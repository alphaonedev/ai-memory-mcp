...
**Code anchors (v0.7.0):** [#1067](https://github.com/alphaonedev/ai-memory-mcp/issues/1067) provider-agnostic LLM substrate (Ollama-native OR any OpenAI-compatible vendor: xAI / OpenAI / Anthropic / Gemini / Kimi / Qwen / Mistral / Groq / Together / Cerebras / OpenRouter / Fireworks / LMStudio / vLLM / llama.cpp server). The v0.8 vLLM first-class backend (§11.4.C) deepens this with PagedAttention for serious inference at the federation node within customer trust boundaries.
...
- **104 MCP tools at `--profile full`** — the **v1.0.0** count, pinned by `Profile::full().expected_tool_count()` in `src/profile.rs` (derived from the per-family `tool_names()` slices; no hand-maintained literal). The callable/bootstrap split is whatever that constant declares, plus the always-on `memory_capabilities`. Per-release history: **74** at v0.7.0 → **100** advertised / 99 callable at v0.8.0 GA (the #1709 coordination tooling — `docs/v0.8.0/release-notes.md:74`) → **101** advertised / 100 callable at v0.9.0 (`docs/v0.9.0/release-notes.md:80`) → **104** advertised / 103 callable at v1.0.0 (`docs/v1.0.0/release-notes.md:84`). The prior wording attributed the 103 count to the v0.8.0 baseline, which shipped 100. **8** advertised at `--profile core` (7 callable Core-family tools + the always-on `memory_capabilities` bootstrap) — the same advertised basis as the 104.
- **102 production HTTP route registrations** / 88 unique URL paths.
- **98 CLI subcommands** under `--features sal`/`sal-postgres`; 96 in default build (the 2-variant gap is `Migrate` + `SchemaInit`, both `#[cfg(feature = "sal")]`; grown via #1720 B2 `Reown` + PE-8 `VerifyAuditTrail` + #1727 `UndoEdit` + v0.9.0 #1859 `Lineage` + v0.9.0 #1827 `Capability` (macaroon capability-token lifecycle) + v1.0.0 #1978 `Watch` (L3 substrate poll-based filesystem-watcher capture daemon); pinned by `ai_memory::EXPECTED_CLI_SUBCOMMANDS_DEFAULT=96` / `EXPECTED_CLI_SUBCOMMANDS_SAL=98` + `tests/cli_subcommand_count_invariant.rs`).
