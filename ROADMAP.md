...
> **Current release:** v1.0.0 (schema v90, 103 MCP tools at `--profile full`), the *defaults stop lying* GA — the version stamp is `Cargo.toml`'s; **no `v1.0.0*` tag is cut yet**, so the newest PUBLISHED tag is **v0.10.0** (released 2026-07-12, schema v80 — the `warn-carrier` release: deprecation WARNs only ahead of the v1.0.0 secure-default flips, with zero default flips, zero schema change and zero behavior change for any correctly-configured deployment; #1972 under the #1940 v1.0.0 epic). v0.10.0 supersedes v0.9.0 (schema v78, 101 MCP tools), a security-hardening and code-review release, which supersedes v0.8.1 (patch release, 2026-06-29) and v0.8.0 (GA, released 2026-06-25; `distributed-coordination`). The prior v0.7.1 patch line (surface area identical to v0.7.0) is itemized in §11.3.1.
