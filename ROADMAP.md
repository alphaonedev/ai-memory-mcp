...
- **26 hook lifecycle events** (17 baseline + 2 transcript-capture additions `PreTranscriptStore`/`PostTranscriptStore` + 5 reflection/compaction additions `PreRecallExpand`/`PreReflect`/`PostReflect`/`PreCompaction`/`OnCompactionRollback` + 2 v0.8.0 #1709 signal events `pre_signal_send`/`post_signal_ack` — per `src/hooks/events.rs::HookEvent`; 17+2+5+2=26. v1.0.0 #2637 REMOVED the never-fired `PreArchive` gate, dropping 27 → 26).
