...
- **23 hook lifecycle events** (15 baseline + 1 transcript-capture event `PostTranscriptStore` + 5 reflection/compaction additions `PreRecallExpand`/`PreReflect`/`PostReflect`/`PreCompaction`/`OnCompactionRollback` + 2 v0.8.0 #1709 signal events `pre_signal_send`/`post_signal_ack` — per `src/hooks/events.rs::HookEvent`; 15+1+5+2=23. v1.0.0 #2637 REMOVED the never-fired `PreArchive` gate (27 → 26); v1.0.0 #2758 REMOVED the never-fired `PreRecall`/`PreSearch`/`PreTranscriptStore` gates — read paths + no production transcript-write path — dropping 26 → 23).
