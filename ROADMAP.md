...
> **Current release:** v1.0.0 (schema v91, 103 MCP tools at `--profile full`), the *defaults stop lying* GA — the version stamp is `Cargo.toml`'s; **no `v1.0.0*` tag is cut yet**, so the newest PUBLISHED tag is **v0.10.0** (released 2026-07-12, schema v80 — the `warn-carrier` release: deprecation WARNs only ahead of the v1.0.0 secure-default flips, with zero default flips, zero schema change and zero behavior change for any correctly-configured deployment; #1972 under the #1940 v1.0.0 epic). v0.10.0 supersedes v0.9.0 (schema v78, 101 MCP tools), a security-hardening and code-review release, which supersedes v0.8.1 (patch release, 2026-06-29) and v0.8.0 (GA, released 2026-06-25; `distributed-coordination`). The prior v0.7.1 patch line (surface area identical to v0.7.0) is itemized in §11.3.1.
...
> **Attribution note ([#2409](https://github.com/alphaonedev/ai-memory-mcp/issues/2409), 2026-07-24).** Every count in this section is the **v1.0.0** state, not the v0.7.0 or v0.8.0 state — the section header previously read "advanced through v0.8.0 GA" while carrying v1.0.0 numbers. Each bullet's *current* value is pinned to a Rust SSOT const and gated by `scripts/check-docs-vs-ssot.sh`; the per-release history below each is narrative.

- **103 MCP tools at `--profile full`** — the **v1.0.0** count, pinned by `Profile::full().expected_tool_count()` in `src/profile.rs` (derived from the per-family `tool_names()` slices; no hand-maintained literal). The callable/bootstrap split is whatever that constant declares, plus the always-on `memory_capabilities`. Per-release history: **74** at v0.7.0 → **100** advertised / 99 callable at v0.8.0 GA (the #1709 coordination tooling — `docs/v0.8.0/release-notes.md:74`) → **101** advertised / 100 callable at v0.9.0 (`docs/v0.9.0/release-notes.md:80`) → **103** advertised / 102 callable at v1.0.0 (`docs/v1.0.0/release-notes.md:84`). The prior wording attributed the 103 count to the v0.8.0 baseline, which shipped 100. **8** advertised at `--profile core` (7 callable Core-family tools + the always-on `memory_capabilities` bootstrap) — the same advertised basis as the 103.
- **96 production HTTP route registrations** / 82 unique URL paths.
- **93 CLI subcommands** under `--features sal`/`sal-postgres`; 91 in default build (the 2-variant gap is `Migrate` + `SchemaInit`, both `#[cfg(feature = "sal")]`; grown via #1720 B2 `Reown` + PE-8 `VerifyAuditTrail` + #1727 `UndoEdit` + v0.9.0 #1859 `Lineage` + v0.9.0 #1827 `Capability` (macaroon capability-token lifecycle) + v1.0.0 #1978 `Watch` (L3 substrate poll-based filesystem-watcher capture daemon); pinned by `ai_memory::EXPECTED_CLI_SUBCOMMANDS_DEFAULT=91` / `EXPECTED_CLI_SUBCOMMANDS_SAL=93` + `tests/cli_subcommand_count_invariant.rs`).
- **22 hook lifecycle events** (15 baseline + 5 reflection/compaction additions `PreRecallExpand`/`PreReflect`/`PostReflect`/`PreCompaction`/`OnCompactionRollback` + 2 v0.8.0 #1709 signal events `pre_signal_send`/`post_signal_ack` — per `src/hooks/events.rs::HookEvent`; 15+5+2=22. v1.0.0 #2637 REMOVED the never-fired `PreArchive` gate (27 → 26); v1.0.0 #2758 REMOVED the never-fired `PreRecall`/`PreSearch` gates (read paths, no op to gate) + the whole transcript hook family `PreTranscriptStore`/`PostTranscriptStore` (no production transcript-write path, so even the notify event never fired) + the now-uninhabited `Transcript` `EventClass`, dropping 26 → 22). **Firing caveat (claims audit, 2026-08-22):** 22 is the count of ADVERTISED events, not of firing ones. All 10 decision-class `Pre*` events are wired — hook-based ENFORCEMENT is real — but 11 of the 22 (`post_store`, `post_recall`, `post_search`, `post_delete`, `post_promote`, `post_link`, `post_consolidate`, `post_governance_decision`, `post_reflect`, `on_index_eviction`, `on_compaction_rollback`) have ZERO production fire sites at v1.0.0: the same advertised-but-inert #2444 class the #2637/#2758 removals addressed for other events, still OPEN for these. See `docs/hook-pipeline.md` for the per-event table.
...
**Open structural gap (§5) — MECHANISM SHIPPED AT v0.9.0; the v1.0.0 default flip was Off→ADVISORY, NOT Off→enforce (corrected 2026-07-24, [#2408](https://github.com/alphaonedev/ai-memory-mcp/issues/2408); supersedes both the "DEFAULT FLIP AT v1.0" reading of this header and the "INERT until attestation lands" framing of an earlier revision).** D3-012 **SHIPPED** at v0.9.0 (#1870 — the schema-v78 `model_attestations` write-once TOFU substrate, loader-observed + operator-signed, `src/storage/model_attest.rs::attested_family_of`), and D3-021 **SHIPPED opt-in-enforce-capable on both backends** (#1767 — `AI_MEMORY_REFLECT_DECORRELATION_QUORUM_N`, refusal ONLY on evidence-backed attested monoculture; claimed-only corpora stay advisory, so enforcement on CLAIMED metadata remains structurally impossible). **What v1.0.0 actually shipped:** [#1952](https://github.com/alphaonedev/ai-memory-mcp/issues/1952) ("defaults stop lying") flipped the compiled default of `AI_MEMORY_REFLECT_DECORRELATION_MODE` from `off` to **`advisory`** — the probe now runs and WARNs by default, and `=off` is the opt-out (`src/config.rs:5476 reflect_decorrelation_mode()` → `unwrap_or_default()` → `ReflectDecorrelationMode::Advisory`, documented at `src/config.rs:4488` and `:5468-5473`). **`enforce` remains OPT-IN at v1.0.0** — the shipped binary does NOT refuse on attested monoculture unless an operator sets the env explicitly. The **enforce-as-default** flip is the tracked **v1.x** lane (D3-021 → D3-031 consolidation-time gate → D3-060 ship-gate), gated behind production reflect family-stamps + a stamp-density probe + an advisory soak; `src/config.rs:4492-4494` states the same v1.x scoping. §25.6's claims discipline and §26.2's kill-test note are reconciled to this. Honest caps unchanged: loader-attested family coverage hard-caps at ~40%; vote-independence stays 0% and permanently *estimable*, never *attestable*.
...
### 25.2 Epoch contract — RESOLVED design (the T1+T4 untracked-hold fork is DISCHARGED, #1878)

An epoch is **three complementary artifacts bound by one SHA-256** (a Checkpoint resolution signature *excludes* `condition`/`metadata`, `src/identity/sign.rs:650-678`, so it cannot carry the payload — the manifest is complementary, not redundant): a **content-signed `epoch_manifest.json`** (signs the WHAT — panel slots, utility weights `frozen_within_epoch=true`, `policy_version`, `prior_epoch_id`, `content_hash`) **bound to an `EpochAdvance` Checkpoint** (attests WHEN+WHO; `resolution = content_hash`, a signed field; rides the FED-RQ-01 checkpoint-federation transport — [#1936](https://github.com/alphaonedev/ai-memory-mcp/issues/1936), **LANDED at v1.0.0 on the SQLite backend**: resolved commit-checkpoints federate over `/sync/push` as a new `checkpoints` subcollection, verified against the resolver's ENROLLED key (fail-closed `AI_MEMORY_FED_REQUIRE_CHECKPOINT_SIG=1`) and applied first-resolution-wins; the v0.9.0 epoch-apply consumer was local-only. Format spine votes `4d3ea1c5` + #1947 decision `00d599ec`) **anchored by a V-4 `epoch.manifest_applied` row** (`payload_hash == content_hash`). `ConditionType::EpochAdvance` is **migration-free** (SAL-enforced, `src/models/checkpoint.rs:13-32`). **The untracked-hold is DISCHARGED ([#1878](https://github.com/alphaonedev/ai-memory-mcp/issues/1878); recorded 2026-07-24 per [#2406](https://github.com/alphaonedev/ai-memory-mcp/issues/2406)).** The schema `docs/contracts/epoch_manifest.schema.json` is git-TRACKED and the VERIFY-ONLY consumer `src/cli/epoch_apply.rs` (`ai-memory epoch-apply <manifest>`) is wired: parse → content-hash pin → operator-Ed25519 `SignableEpochManifest` verify → strict `epoch_seq == prior + 1` → live-`policy_version` binding → one transaction resolving an `EpochAdvance` checkpoint AND appending the V-4 `epoch.manifest_applied` row. The runtime contract is the Rust struct — the schema JSON is NEVER `include_str!`d from `src/`; schema↔struct conformance is asserted in `tests/` (`src/cli/epoch_apply.rs:1-31`). *(D-OPUS-4's ban was predicated on the untracked/no-consumer state; the residual, still-binding perma-ban is §25.6's "implements RQGM" / "co-evolving evaluators shipped" — the L3 optimizer that would PRODUCE a manifest stays permanently external and does not exist in this repo, `scripts/check-l3-boundary.sh` 0-hit.)*
...
- **v1.0.0 (§11.6 federation maturity):** FED-RQ-02/03 federated epoch manifest + cross-node `policy_version` gate · FED-RQ-AGG privacy-preserving aggregate utility (**never raw rates**) · #1707 live recall wire (only after #1706 proves signal) · **F-53 federation E2E — DEFERRED to v1.x** per the §11.6 v1.0.0 disposition ruling (zero implementation; carrier is [#1968](https://github.com/alphaonedev/ai-memory-mcp/issues/1968) OPEN, **not** #1809 — #1809 was the CLOSED docs-drift fix that stopped the per-node at-rest primitive being labelled "E2E", [#2411](https://github.com/alphaonedev/ai-memory-mcp/issues/2411)) · vote-independence empirical estimator.
...
**Allowed (re-cut at the v0.9.0 boundary, 2026-07-09):** "Red-Queen-*principles*-aligned" · "advisory decorrelation probe shipped and is the **v1.0.0 DEFAULT** ([#1952](https://github.com/alphaonedev/ai-memory-mcp/issues/1952) flipped the compiled default Off→**Advisory**, `src/config.rs:5476`); opt-in enforce gate shipped v0.9.0 (#1767), refuses only on ATTESTED monoculture; **enforce-as-default is v1.x, NOT v1.0** — the shipped v1.0.0 binary refuses nothing unless an operator sets `AI_MEMORY_REFLECT_DECORRELATION_MODE=enforce`" *(corrected 2026-07-24, [#2408](https://github.com/alphaonedev/ai-memory-mcp/issues/2408) — the prior "enforce-as-default is v1.0" wording claimed a refusal posture the binary does not have)* · "loader-attested model-family substrate shipped v0.9.0 (#1870, ~40% coverage cap)" · "signed epoch contract with a git-tracked schema and a verify-only local consumer (#1878)". **Removed from the allowed list:** ~~"FED-RQ-01 (in-flight)"~~ — false at any tag; the code never landed ([#1936] is the carrier). **Banned → unlock (unchanged):** "decorrelation enforced" (needs enforce-as-DEFAULT: D3-021 flip + D3-031 + D3-060) · "attests model family" beyond the loader-attested qualified form · "epoch closure shipped" (needs the federated EpochAdvance leg, #1936) · **"implements RQGM"/"co-evolving evaluators shipped" = perma-ban** (category error). **Readiness ladder:** the "15%→~50-60%" figures are planner ESTIMATES, never measurements — ESTIMABLE-labeled, banned from procurement artifacts; family-verify hard-caps at ~40% loader-attested; vote-independence 0% throughout (architectural limit).
...
**Kill-test gate (§16):** if recall still mutates and decorrelation is still claimed-only at v0.9.0, the substrate fails its own kill-test against `git+ripgrep+RAG` — P0-1+P0-2 are the minimum bar. **Verdict recorded 2026-07-09: PASS at v0.9.0** — P0-1 pure recall shipped (v77 fold ledger, #1869) and P0-2's attested-family substrate + evidence-gated quorum enforcement shipped (#1870 + #1767). *(Corrected 2026-07-24, [#2408](https://github.com/alphaonedev/ai-memory-mcp/issues/2408): the earlier "enforce-as-default remains the v1.0 flip" is false — v1.0.0's [#1952](https://github.com/alphaonedev/ai-memory-mcp/issues/1952) flip was Off→**Advisory** (`src/config.rs:5476`), leaving `enforce` opt-in; **enforce-as-default is a v1.x flip** behind D3-021 → D3-031 → D3-060.)*
...
- **Gate 1′ — "defaults stop lying" sub-lane (parallel, tag-blocking; carrier epic [#1952](https://github.com/alphaonedev/ai-memory-mcp/issues/1952), CLOSED):** production reflect family-stamps + stamp-density probe → D3-021 advisory-soak → **enforce-as-default** → D3-031 → D3-060. **Landed scope at v1.0.0 (scope-adjudicated ADVISORY, not enforce):** #1952 flipped the `AI_MEMORY_REFLECT_DECORRELATION_MODE` compiled default **Off→Advisory** only (`src/config.rs:5476`), so the probe runs + WARNs by default while `enforce` stays opt-in; the **enforce-as-default** rung and the D3-031/D3-060 rungs after it are **v1.x**, not v1.0.0 ([#2408](https://github.com/alphaonedev/ai-memory-mcp/issues/2408) reconciliation — §24, §25.6 and §26.2 previously read as if the enforce rung had landed). · `AI_MEMORY_RECALL_TOUCH_SYNC` removal path · fed write-sig (`AI_MEMORY_FED_REQUIRE_WRITE_SIG`, env-table row 94) + signal-sig (`AI_MEMORY_FED_REQUIRE_SIGNAL_SIG`, env-table row 96) WARN→flip — every flip rides the one-cycle deprecation-WARN discipline (#1751 pattern) via the **planned v0.10.0 WARN-carrier release**.
