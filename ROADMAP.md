...
- **v1.0.0 (§11.6 federation maturity):** FED-RQ-02/03 federated epoch manifest + cross-node `policy_version` gate · FED-RQ-AGG privacy-preserving aggregate utility (**never raw rates**) · #1707 live recall wire (only after #1706 proves signal) · **F-53 federation E2E content encryption** ([#1968](https://github.com/alphaonedev/ai-memory-mcp/issues/1968) — the real-work carrier: per-scope envelope keys → ciphertext-only replication, R11; peers today receive plaintext at apply time, at-rest is per-node only. **Corrects the stale citation of CLOSED [#1809](https://github.com/alphaonedev/ai-memory-mcp/issues/1809)**, which was a LOW *docs-drift* fix ("at-rest labeled E2E but is per-node"), NOT the crypto epic. **DEFERRED to v1.x** — a new signed/encrypted federation wire format + per-scope key-distribution model is freeze-hostile at the v1.0.0 GA cut (T1/T3/T4; co-versions with the #1852/#1950 federation-signing cohort); the honest v1.0 posture is per-node at-rest encryption + write-attestation, tracked open under #1968) · vote-independence empirical estimator.
