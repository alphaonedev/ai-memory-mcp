...
- **85 CLI subcommands** under `--features sal`/`sal-postgres`; 83 in default build (the 2-variant gap is `Migrate` + `SchemaInit`, both `#[cfg(feature = "sal")]`; grown to 83/85 via #1720 B2 `Reown` + PE-8 `VerifyAuditTrail` + #1727 `UndoEdit` non-destructive in-place-edit undo; pinned by `ai_memory::EXPECTED_CLI_SUBCOMMANDS_DEFAULT=83` / `EXPECTED_CLI_SUBCOMMANDS_SAL=85` + `tests/cli_subcommand_count_invariant.rs`).
