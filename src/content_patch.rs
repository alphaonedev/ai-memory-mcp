...
mod tests {
    ...
    fn replace_self_overlapping_needle_rejected_as_ambiguous() {
        // Gate-3 review fix: "aa" occurs at OVERLAPPING positions 0 and 1 in
        // "aaa" — an ambiguous target. Non-overlapping `str::matches` counts it
        // as 1 and would silently first-match replace; the "exactly once"
        // contract requires rejecting it. (Deletion form too, to be sure the
        // overlap check runs before the replace.)
        let p = ContentPatch {
    ...
    fn replace_non_self_overlapping_unique_still_ok() {
        // A needle that does not self-overlap and appears exactly once must
        // still replace — the overlap-aware count must not over-reject.
        let p = ContentPatch {
