...
pub async fn prove_possession_via_store(
    ...
    PossessionProof::verify_challenge_response(&taken, agent_id, &pubkey_b64, &signature, &now)
        .map_err(|e| StoreError::InvalidInput {
            detail: format!("refusing to bind: {e}"),
        })
}
...
pub trait MemoryStore: Send + Sync {
    ...
    /// constructor, so an adapter cannot be asked to bind a key whose private
    /// half nobody demonstrated holding. Pre-#3464 this took a self-asserted
    /// key, which let anyone with the admin role bind a key they controlled to
    /// another agent's id and then mint `agent_attested` writes as that agent.
