    ...
    if matches!(app.storage_backend, StorageBackend::Postgres) {
            ...
            for candidate in &chain {
                    ...
                    app.store.get_namespace_standard(&bind_ctx, candidate).await
                {
                    ...
                    match app.store.get(&bind_ctx, &standard_id).await {
                        Ok(m)
                            if crate::visibility::is_readable_on_query(&m, Some(&visibility_caller), Some(m.namespace.as_str())) =>
                        {
                            standards.push(json!({
                                "namespace": candidate,
                                (field_names::STANDARD_ID): standard_id,
        ...
        match app.store.get_namespace_standard(&bind_ctx, &ns).await {
            Ok(Some((standard_id, parent))) => {
                ...
                match app.store.get(&bind_ctx, &standard_id).await {
                    Ok(m) if !crate::visibility::is_readable_on_query(&m, Some(&visibility_caller), Some(m.namespace.as_str())) => {
                        return (
                            StatusCode::OK,
                            Json(json!({
                                "namespace": ns,
