...
pub fn postgres_endpoint_supported(method: &axum::http::Method, path: &str) -> bool {
    ...
    match (method.as_str(), path) {
        ...
        // postgres because it touches NO storage at all: it mints a nonce into
        // the in-process challenge store and returns the transcript, so there
        // is no `app.db` scratch-read hazard for it to fall into. Refusing it
        // on postgres would make the bind it precedes unreachable there —
        // i.e. it would silently disable key enrollment on that backend.
        ...
        // #3064 batch D — I4 transcript replay via SAL
        // `replay_transcript_union` + `fetch_transcript_content`.
        // Never `app.db.lock()`.
