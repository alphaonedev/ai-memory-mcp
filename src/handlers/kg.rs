...
pub async fn entity_register(
    ...
    if matches!(app.storage_backend, StorageBackend::Postgres) {
        ...
        let capability = match crate::handlers::capability_from_headers(&headers, &aid) {
            Ok(c) => c,
            Err(resp) => return resp,
        };
        let ctx = crate::store::CallerContext::for_agent(aid.clone()).with_capability(capability);
            ...
            Ok(Some(rec)) => {
                ...
                let visible = caller_is_admin
                    || match app.store.get(&ctx_admin, &rec.entity_id).await {
                        Ok(m) => crate::visibility::is_readable_on_query(&m, Some(&caller), Some(m.namespace.as_str())),
                        Err(_) => false,
                    };
                if visible {
                    return Json(json!({
        ...
        Ok(Some(rec)) => {
            ...
            let visible = caller_is_admin
                || db::get(&lock.0, &rec.entity_id)
                    ...
                    .flatten()
                    .as_ref()
                    .is_none_or(|m| crate::visibility::is_readable_on_query(m, Some(&caller), Some(m.namespace.as_str())));
            if !visible {
                return Json(json!({
                    "found": false,
                    "entity_id": null,
...
pub async fn kg_timeline(
    ...
    let source_owner: Option<(String, String)> = {
        #[cfg(feature = "sal")]
        if matches!(app.storage_backend, StorageBackend::Postgres) {
            // #3498/#944: inspect the anchor before the handler's owner gate;
            // SAL visibility would erase the required ordinary-row 403 shape.
            // This context is confined to this lookup, never to the traversal.
            let ctx = crate::store::CallerContext::for_admin(&caller);
            match app.store.get(&ctx, &p.source_id).await {
                Ok(mem) => (!crate::visibility::is_substrate_namespace(&mem.namespace)
            ...
            match db::get(&lock.0, &p.source_id) {
                Ok(Some(mem)) => (!crate::visibility::is_substrate_namespace(&mem.namespace)
                    || crate::visibility::is_readable_on_query(
                        &mem,
                        (caller != sentinels::DAEMON_PRINCIPAL).then_some(caller.as_str()),
                        Some(&mem.namespace),
...
/// namespace parameter: only the source anchor is explicit. Failed lookups
