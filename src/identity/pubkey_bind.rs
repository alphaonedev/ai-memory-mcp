...
//! candidate key over a server-issued, single-use, domain-separated
//! transcript) and the crate-internal
//! [`PossessionProof::from_verified_lineage_succession`] (the rotation path,
//! where the CURRENT key-holder's succession signature is the authority).
//! `storage::bind_agent_pubkey` takes `&PossessionProof` by value of its
//! signature, so no present or future call site — on any surface, on either
//! backend — can bind a key without one. That is the control: a structural
//! prevention, not an enumerated set of guarded entry points.
//!
//! # Transcript
//!
...
//! predicate IS the admit-once decision, so a challenge answers exactly one
//! bind even under concurrent submission.
//!
...
pub struct BindChallenge {
    ...
    /// RFC3339 expiry. After this instant the challenge is refused.
...
pub enum BindAuthority {
    /// The candidate key signed a server-issued challenge (the default, and
    /// the only authority reachable from an external surface).
    PossessionProof,
...
/// surfaces) or [`Self::from_verified_lineage_succession`] (crate-internal
/// rotation). The private field is what makes an unproven bind
/// unrepresentable rather than merely discouraged (rust-1.98 ERRORS-09).
#[derive(Debug, Clone)]
...
impl PossessionProof {
    ...
    ///
    /// The caller MUST have already CONSUMED `challenge` from durable storage
    /// (`storage::consume_pubkey_bind_challenge`), which is what makes it
    /// single-use; this function is the cryptographic half only.
    ///
    ...
    pub fn verify_challenge_response(
        ...
        if challenge.agent_id != agent_id || challenge.pubkey_b64 != pubkey_b64 {
        ...
        // Bound the window from ABOVE as well. A daemon-issued challenge is
        // always within the TTL, but the offline lane (`ai-memory agents
        // bind-key --proof-file`) lets the SIGNER choose `expires_at`, and an
        // unbounded one would mint a proof that stays valid forever — a
        // permanent bind capability for anyone who later obtains the file.
        if (expires - now).num_seconds() > BIND_CHALLENGE_TTL_SECS {
            return Err(BindProofError::Expired);
        }
        let key = crate::identity::keypair::decode_public_base64(pubkey_b64)
        ...
        Ok(Self {
            ...
            nonce_b64: Some(challenge.nonce_b64.clone()),
        })
    }
    ...
    /// challenge-response.
    #[must_use]
    pub(crate) const fn from_verified_lineage_succession() -> Self {
        Self {
...
mod tests {
    ...
    fn honest_holder_proves_possession() {
        ...
        let witness =
            PossessionProof::verify_challenge_response(&ch, "ai:alice", &pk, &proof, &now())
                .expect("the key holder must be able to prove possession");
        assert_eq!(witness.authority(), BindAuthority::PossessionProof);
        assert_eq!(witness.nonce_b64(), Some(ch.nonce_b64.as_str()));
    }
    ...
    fn a_proof_for_one_agent_does_not_bind_another() {
        ...
        assert_eq!(
            PossessionProof::verify_challenge_response(&ch, "ai:bob", &pk, &proof, &now())
    ...
    fn malformed_signature_is_refused_not_panicked() {
        ...
        let ch = challenge_for("ai:alice", &pk, &future());
        assert_eq!(
            PossessionProof::verify_challenge_response(&ch, "ai:alice", &pk, "!!!", &now())
