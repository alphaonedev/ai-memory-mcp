...
//! thick 30-field [`Memory`] row, and a machine-checked record of exactly where
...
/// the default `human`/`agent` path UPDATEs in place, and FORGET remains a
/// physical delete-plus-sidecar-tombstone rather than a spine leaf (spec §4).
/// Pinned as a const so the honesty state is machine-checked; the day this
/// flips to `true` is a deliberate edit here + in the spec ledger.
