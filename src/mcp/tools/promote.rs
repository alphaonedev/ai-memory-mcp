...
pub(super) fn handle_promote(
    ...
    if let Some(to_ns) = params[crate::models::field_names::TO_NAMESPACE].as_str() {
        ...
        // ── #3202 — govern the DESTINATION, not just the source ──────────
        // The Task-1.9 block above evaluates `GovernedAction::Promote` against
        // `target.namespace` — the SOURCE. Vertical promotion is a WRITE into
        // `to_ns`: it lands a new row there. Pre-fix the only thing `to_ns` got
        // was `reject_reserved_write_namespace`, so a namespace whose policy
        // said `write: Deny` (or `Approve`) accepted a full copy of any row the
        // caller could read from a descendant — the governance boundary the
        // operator drew around that namespace was bypassed by construction,
        // and the ancestor namespace is normally the MORE protected one.
        // Evaluate the STORE-class gate against the destination before cloning,
        // so a Deny refuses and an Approve queues exactly as a direct
        // `memory_store` into `to_ns` would.
        let dest_agent_id = {
            use crate::models::{GovernanceDecision, GovernedAction};
            ...
            match db::enforce_governance(
                ...
                &dest_agent_id,
                None,
                // The clone is a NEW row in `to_ns` with no prior owner there;
                // an owner-scoped rule must not read the source's author as if
                // it already owned something in the destination.
                None,
                &dest_payload,
            ...
            // #3202 — chain the forensic row under the DESTINATION namespace,
            // so an operator auditing `to_ns` sees the write that landed there.
