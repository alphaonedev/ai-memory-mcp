...
pub fn handle_share(
    ...
    let Some(source) = resolved.filter(|source| {
        ...
        crate::visibility::is_readable_on_query(source, caller, None)
    }) else {
...
mod tests {
    ...
    fn share_allows_owner_and_target_reads_copy_3379() {
        ...
        assert!(crate::visibility::is_readable_on_query(&copy, Some("ai:bob"), Some(&copy.namespace)));
    }
    ...
    #[test]
    fn share_refuses_readable_foreign_collective_3379() {
        let conn = fresh_conn();
        ...
        assert!(crate::visibility::is_readable_on_query(&src, Some("ai:bob"), Some(&src.namespace)));
        let error = handle_share(
            &conn,
            &json!({"source_memory_id": id, "target_agent_id": "ai:carol"}),
            Some("ai:bob"),
        )
        .expect_err("readability does not confer ownership");
        assert_eq!(error, crate::errors::msg::CALLER_DOES_NOT_OWN_MEMORY);
    }

    #[test]
    fn share_never_exposes_substrate_rows_3379() {
        let conn = fresh_conn();
        ...
        let rows: i64 = conn
            ...
            .expect("count rows after refusals");
        assert_eq!(rows, 1, "substrate refusals must not create copies");
    }
