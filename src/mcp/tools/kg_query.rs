...
fn node_visible(mem: &crate::models::Memory, caller: Option<&str>, as_agent: Option<&str>) -> bool {
    if let Some(c) = caller
        && !crate::visibility::is_readable_on_query(mem, Some(c), Some(&mem.namespace))
    {
        return false;
    }
...
fn kg_query_by_source_uri(
    ...
    let roots: Vec<_> = roots
        .into_iter()
        // #3498 (merged at chain assembly): the substrate read rule runs on
        // this path too, so a `_substrate` root is never returned unless the
        // caller named that namespace.
        .filter(|m| {
            crate::visibility::is_readable_on_query(m, caller, namespace)
                && node_visible(m, caller, as_agent)
        })
        .collect();
...
pub fn handle_kg_query(conn: &rusqlite::Connection, params: &Value) -> Result<Value, String> {
    ...
    // Validate both bounds before choosing a path, even when a source-uri
    // query does not use a traversal depth.
...
fn kg_query_from_source(
    ...
    //
    // #3498 + #3386 (merged at chain assembly): step 1 is the substrate read
    // rule `is_readable_on_query` on EVERY hop of the serialized path — it
    // runs even with no caller, because the substrate-namespace exclusion is
    // ambient (#3348) and the path exposes intermediate ids too (#3498); it
    // subsumes the enforced-caller gate `is_visible_to_caller`. Step 2 is the
    // #151 `as_agent` scope step on the target (#3386), which is what finally
    // makes `as_agent` bite on this path. Each step only removes rows, so a
    // self-asserted `as_agent` can never widen past the enforced caller.
