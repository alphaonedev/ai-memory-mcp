...
pub fn decorate_memory_many(
    ...
    rows.iter()
        .map(|(mem, score)| {
            ...
            // #3548 Part A — `content_attestation` is the row's OWN write
            // attestation, read from the `metadata.attest_level` stamp (the
            // write-entry-point precedent, models/memory.rs) — NOT the incident
            // edge. Splitting the two makes the thing each field measures visible
            // in its own name: a self-signed row reached only by an unsigned link
            // now reports content_attestation=self_signed while link_attestation
            // reflects the (weaker) edge.
            let content_attest_level = mem
                .metadata
                .get(crate::models::field_names::ATTEST_LEVEL)
                .and_then(serde_json::Value::as_str)
                .and_then(AttestLevel::from_str);
            obj.insert(
                "content_attestation".to_string(),
                json!(provenance_tier(mem.confidence_source, content_attest_level)),
            );
