...
pub(crate) fn handle_inbox_with_policy(
    ...
    let filtered: Vec<&Memory> = items
        ...
        .collect();
    let messages = filtered.into_iter().map(|m| inbox_message(m)).collect();
...
impl McpTool for NotifyTool {
    ...
    fn docs() -> &'static str {
        "Send message to _inbox/<target>. Sender = caller agent_id. Read via memory_inbox."
    }
...
impl McpTool for InboxTool {
    ...
    fn docs() -> &'static str {
        ...
        "Read _inbox/<agent_id>. access_count==0 is the unread marker \
