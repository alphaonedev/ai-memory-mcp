...
/// `POST /api/v1/agents/{id}/pubkey/challenge`. This external proof is valid
/// only for first bootstrap or same-key reassertion. A distinct replacement
/// is authorized by the current key through the lineage succession path.
