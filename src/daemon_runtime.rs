...
///     in-process TLS and none is configured (fail-closed-now), OR the
///     `asi-hard` posture is engaged and the bind is plaintext NON-loopback
///     (#3200 R3: no hatch under the posture; the loopback exemption stays
///     for the same-host TLS-terminating proxy).
///
...
fn tls_bind_guard(
    ...
    require_tls: bool,
    posture: crate::security_profile::SecurityPosture,
) -> std::result::Result<Option<String>, String> {
    if tls_present {
        return Ok(None);
    }
    ...
    if posture == crate::security_profile::SecurityPosture::AsiHard {
        return Err(format!(
            "{} refuses a plaintext bind to non-loopback {host:?}: memory content and the API \
             key would cross the network in CLEARTEXT. Pass --tls-cert / --tls-key for \
             in-process TLS, or bind to loopback behind a TLS-terminating proxy. \
             AI_MEMORY_ALLOW_PLAINTEXT_NONLOOPBACK is not honoured under this posture (#3200).",
...
pub async fn bootstrap_serve(
    ...
    match tls_bind_guard(
        ...
        if crate::security_profile::is_asi_hard() {
            crate::security_profile::SecurityPosture::AsiHard
        } else {
            crate::security_profile::SecurityPosture::Standard
        },
    ) {
        Ok(None) => {}
        Ok(Some(warning)) => tracing::warn!("{warning}"),
...
mod tests {
    ...
    const STANDARD: crate::security_profile::SecurityPosture =
        crate::security_profile::SecurityPosture::Standard;
    const ASI_HARD: crate::security_profile::SecurityPosture =
        crate::security_profile::SecurityPosture::AsiHard;
    ...
    async fn test_bootstrap_serve_keyword_tier_no_embedder() {
        ...
        const SQLITE_KEYWORD_BASE_TASKS: &[&str] = &[
            "auto-tag worker",
            "garbage-collection loop",
            "offloaded-blob TTL sweep",
            "expired-lease reclaim sweep",
            "FTS5 integrity checker",
            "corpus-size gauge refresher",
            "recall-access fold loop",
            "WAL checkpoint loop",
            "pending-action timeout sweep",
            "transcript lifecycle sweep",
            "agent-quota reset sweep",
        ];
        ...
        let agent_key_refresh_enabled =
            crate::handlers::identity_binding::resolve_agent_key_refresh_interval().is_some();
        ...
        assert!(vi.is_none());
        // #3418 adds one live enrolled-agent-key refresh handle unless the
        // operator explicitly selects the restart-required posture with
        // `AI_MEMORY_AGENT_KEY_REFRESH_SECS=0`. Derive that conditional term
        // from the same resolver that bootstrap uses instead of pinning only
        // today's default. The deferred-audit supervisor has its own shutdown
        // barrier, and the sqlite auto-atomise worker has its own thread handle;
        // neither belongs to `task_handles`. Family-embedding precompute is
        // default-off and the keyword tier has no embedder.
        let expected_task_count =
            SQLITE_KEYWORD_BASE_TASKS.len() + usize::from(agent_key_refresh_enabled);
        assert_eq!(
            bs.task_handles.len(),
            expected_task_count,
            "base task inventory {SQLITE_KEYWORD_BASE_TASKS:?}; agent-key refresh enabled: \
             {agent_key_refresh_enabled}"
        );
