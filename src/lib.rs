...
pub mod kg;
// v1.0.0 #3717 — the key-role table: every role a deployment needs, its
// typed on-disk state, and the ONE plan `keys init` / `keys status` / the
// doctor consume (refuse-before-write on a lost private half).
pub mod keys;
