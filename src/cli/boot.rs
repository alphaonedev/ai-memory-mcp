...
/// `db::CURRENT_SCHEMA_VERSION` (31 in v0.7.0 — v21 from K2's
...
/// substrate-native recursive-learning mission, v30 from L1-1's
/// `memories.memory_kind TEXT NOT NULL DEFAULT 'observation'` column
/// backing the typed `MemoryKind::Reflection` enum, and v31 from
/// L1-6's `Goal` MemoryKind variant + trigger-based closed-set
/// guard for the Boundary §16.2 substrate refusal). When a DB's
/// `schema_version` exceeds this, the binary is too old for a newer
/// DB and we surface a warning. v0.6.3.1 (PR-9h / issue #487 PR
/// #497 req #72).
pub const MAX_SUPPORTED_SCHEMA: u32 = 31;
