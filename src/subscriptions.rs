...
fn validate_url_with(url: &str, allow_loopback: bool) -> Result<()> {
    ...
    if scheme != "https" && scheme != "http" {
        return Err(anyhow!("webhook URL scheme must be http(s): {url_display}"));
    }
    ...
    let host: String = if let Some(stripped) = host_port.strip_prefix('[') {
        ...
        match stripped.find(']') {
            ...
            None => return Err(anyhow!("malformed IPv6 URL host: {url_display}")),
        }
    } else {
        // IPv4 / hostname.
        host_port
    ...
    if scheme == "http" && !is_loopback {
        ...
        if let Some(ip) = parsed_ip {
            if !is_loopback_normalized(ip) {
                return Err(anyhow!(
                    "webhook URL must be https for non-loopback host: {url_display}"
                ));
            }
        } else {
            return Err(anyhow!(
...
pub fn replay_subscription_events(
    ...
    let rows = stmt.query_map(params![subscription_id, since], |row| {
