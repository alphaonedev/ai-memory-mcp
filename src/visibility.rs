...
pub fn is_visible_by_fields(
    ...
    match classify_scope(id, namespace, metadata) {
        ScopeArm::OwnerPrivate => private_visible(namespace, metadata, caller),
        ScopeArm::Broad => true,
...
pub fn is_visible_to_scope_agent(mem: &Memory, as_agent: &str, caller: Option<&str>) -> bool {
    match classify_scope(&mem.id, &mem.namespace, &mem.metadata) {
        ScopeArm::OwnerPrivate => {
            caller.is_some_and(|c| private_visible(&mem.namespace, &mem.metadata, c))
        }
        ScopeArm::Broad => true,
...
mod tests {
    ...
    #[test]
    fn scope_agent_preserves_legacy_inbox_recipient_3386() {
        let mut m = mem_with_metadata(json!({
            "agent_id": "ai:alice",
            "recipient_agent_id": "ai:bob",
        }));
        m.namespace = format!("{}ai:bob", crate::LEGACY_INBOX_NAMESPACE_PREFIX);
        assert!(is_visible_to_scope_agent(&m, "ai:bob", Some("ai:bob")));
        assert!(!is_visible_to_scope_agent(&m, "ai:bob", Some("ai:eve")));
        assert!(!is_visible_to_scope_agent(&m, "ai:bob", None));
        // The existing private-inbox fallback is namespace-bound.
        m.namespace = "team/notes".to_string();
        assert!(!is_visible_to_scope_agent(&m, "ai:bob", Some("ai:bob")));
    }
