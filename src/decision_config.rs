...
//! misconfigured silently. Absorbs #3808 (the dead `[llm.auto_tag]`
//! sibling resolver, retired in the same commit).
//!
...
//!   replaces, which inherited on a backend-name match alone.
...
///
/// Per-field ladder, mirroring the `[llm.auto_tag]` parent-fallback
/// shape this resolver replaces (#3808):
///
...
/// Parse-time refusals for `[decision]` and for the retired
/// `[llm.auto_tag]` endpoint keys (#3808). Called from
...
/// #3808 — `[llm.auto_tag]`'s `backend` / `base_url` / `api_key_env` /
/// `api_key_file` were documented as a second endpoint but had no
/// reader: `resolve_llm_auto_tag` had zero callers and production
/// threads the legacy flat `auto_tag_model` string. They are now REFUSED
/// by name rather than accepted and ignored.
///
/// Refusal rather than a WARN, because an accepted-and-ignored key is a
/// config that lies, and (had it been wired as written) the second
/// endpoint would also have bypassed the `build_llm_client` egress gate.
/// The repair is one deleted line, or the same endpoint expressed under
/// `[decision]`, which IS resolved and will be gated at the boot
/// chokepoint.
///
/// `[llm.auto_tag].model` is deliberately STILL ACCEPTED: `ai-memory
/// config migrate` writes exactly that key when it lifts a legacy
/// `auto_tag_model`, so refusing it would make the repair tool emit a
/// config the loader rejects — a lockout, not a control.
fn validate_retired_auto_tag(cfg: &AppConfig) -> anyhow::Result<()> {
    let Some(auto_tag) = cfg.llm.as_ref().and_then(|l| l.auto_tag.as_ref()) else {
        return Ok(());
    };
    for (key, present) in [
        ("backend", auto_tag.backend.is_some()),
        (config_keys::BASE_URL, auto_tag.base_url.is_some()),
        (config_keys::API_KEY_ENV, auto_tag.api_key_env.is_some()),
        (config_keys::API_KEY_FILE, auto_tag.api_key_file.is_some()),
    ] {
        if present {
            anyhow::bail!(format!(
                "[llm.auto_tag].{key} is not honoured and is refused (#3808): the \
                 `[llm.auto_tag]` sibling endpoint has never had a reader, so this \
                 key has been silently ignored, and a second inference endpoint \
                 configured here would bypass the boot egress gate. Remove the key, \
                 or configure the second endpoint under `[{SECTION}]` (#3806), which \
                 is resolved and gated. `[llm.auto_tag].model` is unaffected."
            ));
        }
    }
    Ok(())
...
mod tests {
    ...
    // ---- #3808: the retired [llm.auto_tag] endpoint keys ------------
    #[test]
    fn retired_auto_tag_endpoint_keys_are_refused_by_name_and_model_still_parses() {
        for key in [
            "backend = \"ollama\"",
            ...
            let err = expect_refusal(&cfg, "this section must be refused");
            assert!(err.contains("[llm.auto_tag]."), "{key}: {err}");
            assert!(
                err.contains("3808"),
                "the refusal must cite the defect: {err}"
            );
        }

        // PRESENCE control — `model` (what `config migrate` writes) is
        // still accepted, so the repair tool's output still loads.
        let cfg = parse("[llm]\nbackend = \"ollama\"\n\n[llm.auto_tag]\nmodel = \"gemma3:4b\"\n");
        assert!(
            validate(&cfg).is_ok(),
            "[llm.auto_tag].model must still parse"
        );
    }
