...
// suites after the 3017ea05 coverage red) — #3777: the predicate lives in
// `tests/common/lane_db.rs`, the ONE guard every guarded pg cell calls; the
// harness re-exports it so the wake-hub suites keep their spelling.
// ---------------------------------------------------------------------------

#[path = "../common/lane_db.rs"]
mod lane_db;
// Each wake-hub binary includes this harness; the ones without a pg cell use
// none of these names (the same reason the harness is `allow(dead_code)`).
#[allow(unused_imports)]
pub use lane_db::{
    SHARED_LIVE_STORE_PORT, assert_lane_database, database_name, is_shared_live_store, port_of,
};
