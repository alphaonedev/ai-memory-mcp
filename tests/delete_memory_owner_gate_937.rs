...
async fn bob_cannot_delete_alice_memory_937() {
    ...
    // #3426 / #3339 — was FORBIDDEN. Alice's row carries no `metadata.scope`
    // key, so it is PRIVATE and bob cannot read it; a 403 here confirmed the
    // id EXISTS while `GET` answered 404, making the write path an existence
    // oracle. The refusal is now the read path's own answer, which is what
    // the postgres branch already returned (its handlers read through a
    // visibility-scoped `store.get`). The DENIAL itself is unchanged — see
    // the row-still-exists assertion below.
    assert_eq!(
        status,
