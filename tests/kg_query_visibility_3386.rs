...
#[test]
fn by_source_uri_refuses_negative_bounds() {
    let _p = Posture::enforced(ALICE);
    let conn = uri_fixture();

    for bound in ["limit", "max_depth"] {
        let err = handle_kg_query(&conn, &json!({"by_source_uri": DOC_URI, bound: -5}))
            .expect_err("a negative bound must refuse on this path too");
        assert!(
            err.contains(bound) && err.contains("non-negative"),
            "got: {err}"
        );
    }
}
