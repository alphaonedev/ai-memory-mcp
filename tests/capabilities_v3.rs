...
fn cap_v3_summary_core_profile_counts_and_names_recovery_paths() {
    ...
    // memory tool). Total memory tools = 52 (53 - 1 bootstrap).
    assert!(
        summary.starts_with("7 of 52 memory tools"),
        "core profile summary should open with \"7 of 52 memory tools\" (Round-2 F13; \
         v0.7.0 Wave-2 fix B2 (S6-M1) added memory_reflection_origin to Family::Power, \
         bumping the substantive total from 51 to 52); got: {summary}"
    );
    assert!(summary.contains("(core)"), "must label the profile as core");
    assert!(
        summary.contains("45 are listed in this manifest"),
        "core profile must report 45 unloaded (52 - 7); got: {summary}"
    );
...
fn cap_v3_summary_full_profile_reports_all_visible() {
    ...
    // total from 50 to 51.
    assert!(
        summary.starts_with("52 of 52 memory tools"),
        "full profile summary should open with \"52 of 52 memory tools\" (Round-2 F13; \
         v0.7.0 Wave-2 fix B2 (S6-M1) added memory_reflection_origin); got: {summary}"
    );
...
fn cap_v3_summary_graph_profile_counts() {
    ...
    assert!(
        summary.starts_with("18 of 52 memory tools"),
        "graph profile = 7 core (v0.7 B1+B2) + 11 graph (v0.7 J7) = 18 memory tools \
         (Round-2 F13: bootstrap excluded; v0.7.0 Wave-2 fix B2 (S6-M1) added \
         memory_reflection_origin to Family::Power, bumping the substantive total \
         from 51 to 52); got: {summary}"
    );
    assert!(summary.contains("(graph)"));
    assert!(summary.contains("34 are listed in this manifest"));
}
...
fn cap_v3_describe_core_profile_is_plain_english_with_loaded_names() {
    ...
    assert!(
        describe.contains("45 more"),
        "core profile must report 45 unloaded (52 - 7); v0.7.0 Wave-2 fix B2 \
         (S6-M1) added memory_reflection_origin bumping the substantive total \
         to 52; got: {describe}"
    );
...
fn cap_v3_describe_full_profile_uses_nothing_more_form() {
    ...
    assert!(
        describe.starts_with("I can directly use all 52 memory tools right now ("),
...
fn cap_v3_describe_graph_profile_uses_preview_ellipsis() {
    ...
    assert!(describe.contains("(store, recall, list, get, search, ...)"));
    assert!(describe.contains("34 more"));
}
...
fn cap_v3_response_carries_tools_array_with_51_entries() {
    ...
    assert_eq!(
        tools.len(),
        53,
        "v3 must surface all 53 tools regardless of profile (v0.7.0 I4 added \
         ...
         Task 4/8 added memory_reflect; v0.7.0 Wave-2 fix B2 (S6-M1) added \
         memory_reflection_origin); got {}",
        tools.len()
    );
