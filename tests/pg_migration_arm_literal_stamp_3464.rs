...
fn sqlite_migrations_source() -> String {
    let path = Path::new("src/storage/migrations.rs");
    std::fs::read_to_string(path).unwrap_or_else(|e| panic!("read {}: {e}", path.display()))
}

fn postgres_schema_source() -> String {
    let path = Path::new("src/store/postgres_schema.sql");
    std::fs::read_to_string(path).unwrap_or_else(|e| panic!("read {}: {e}", path.display()))
}
...
#[test]
fn postgres_bootstrap_contains_only_safe_v97_relations_3464() {
    let schema = postgres_schema_source();
    for needle in [
        ...
        "CONSTRAINT agent_pubkey_history_pubkey_canonical",
        "idx_agent_pubkey_history_one_open",
        "idx_agent_pubkey_history_key_once",
        "CREATE TABLE IF NOT EXISTS agent_pubkey_challenges",
        "idx_agent_pubkey_challenges_expires",
    ] {
        assert!(
            schema.contains(needle),
            "postgres bootstrap missing v97 pin: {needle}"
        );
    }
    assert!(!schema.contains("CREATE TRIGGER agent_pubkey_history_authoritative_v97"));
    assert!(!schema.contains("CREATE OR REPLACE FUNCTION reconcile_agent_pubkey_from_history_v97"));
}
