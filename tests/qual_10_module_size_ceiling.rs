...
const MODULE_SIZE_CEILINGS: &[(&str, usize)] = &[
    ...
    ("src/storage/migrations.rs", 7_400), /* 2026-09-15 (Consolidation Unit 1 #3690, base 1ec64196b): the v100 rung (`MIGRATION_V100_SQLITE` include + `migrate_v100` + its tip arm) and the rule-(f) comment on the bootstrap title-slot index grow this file to MEASURED 7_311 on this branch alone; #3655-v2 (v99) adds its own rung beside it, so the stacked tree needs re-measuring. Never lower. Ceiling 7_300 -> 7_400 (+89 headroom, lockstep). PRIOR: 2026-09-03 (#3344 v96 embed_skip rung merged onto the #3419 v95 rung): the merge result lands migrations.rs at 7_007 — neither side alone breached 6_930; 7_300 leaves room for the #3464 v97 rung. Prior: */
    ...
    ("src/llm.rs", 7_050), /* MERGER BUMP 2026-09-14 on RESOLVED TREE 7efa7b4cf. wc -l = 6_971, +21 over 6_950. #3627 bumped its OWN branch ceiling from its own actual 6_829; the resolved figure is 142 higher because the reconciliation branch and the stack also touch this file. A BRANCH CANNOT MEASURE ITS OWN STACK. 6_950 -> 7_050 (+79 headroom). */
];
