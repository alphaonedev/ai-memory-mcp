...
const MODULE_SIZE_CEILINGS: &[(&str, usize)] = &[
    ...
    ("src/store/postgres.rs", 37_850), /* 2026-08-26 (#3267 x #3240 cross-merge re-measure): release/v1.0.0 moved again under PR #3267 — Grok's #3240 (pg SAL-parity cluster) grew this file to 36_999 on the release side while the #3267 review-fix wave grew it to 37_168 on the branch side; the merge of c3344757 x 0782e6ce auto-merges postgres.rs cleanly and MEASURES 37_766 (wc -l on the resolved tree, per the "a clean auto-merge is UNVERIFIED — always re-measure" rule). Ceiling max(37_080, 37_400) -> 37_850 (+84 headroom over the merged measure). Resolved in the merge commit itself; both parent rationales kept below. Never lower the 36_600 floor. PRIOR (release side, #3240): 2026-08-27 (#3240): MEASURED 36_999 after SAL-parity cluster rebased onto #3237 (9cc3df51). Never lower the 36_600 floor. Ceiling 36_600 -> 37_080 (+81 headroom). Helpers live in postgres_parity.rs; this file still grew on the export/gc/record-stop call sites. PRIOR (branch side, #3267): 2026-08-26 (#3264 adversarial-review fix wave, merge-time re-measure): the two independent reviews of PR #3267 landed B1 (drop the preemptive `rolsuper` refusal; add `preemptive_refusal_detail` + the WARN arm + the managed-Postgres rationale), B2 (`installed` from `pg_extension`, four probe round trips folded into one statement), S1 (`render_database_for_operator` + `database_name_is_plain`) and two new EOF unit tests, plus the corrected comment blocks the reviewers cited. Branch file measures 37_168 post-fmt; the CLEAN AUTO-MERGE with the release tip that #3237 moved lands it at 37_316 (`git merge-tree --write-tree 9cc3df51 b8393a63`, re-measured per the "a clean auto-merge is UNVERIFIED" rule the entry below states). Ceiling 37_150 -> 37_400 (+84 headroom over the merged measure). Never lower the 36_400 floor. PRIOR: 2026-08-26 (#3264 merge-time re-measure): the release tip moved under this PR — #3237 (sqlite SAL parity) grew postgres.rs on `release/v1.0.0` while this branch added the preflight arm, so the clean auto-merge of the two lands the file at 37_091 (measured via `git merge-tree` of 9cc3df51 x 90d9a0f8, exactly the "clean auto-merge is UNVERIFIED — always re-measure" case the entry above warns about). Ceiling 36_970 -> 37_150 (+59 headroom). No new code in this commit — lockstep bump only. Never lower the 36_400 floor. PRIOR: 2026-08-26 (#3264 pgvector bootstrap preflight): the classified fail-closed CREATE EXTENSION diagnostic — the pgvector/role/ag_catalog probe SQL + remedy consts, the `PgvectorPreflight` decision table (`classify_pgvector_preflight` / `classify_init_sql_error`, both pure + unit-tested), `probe_pgvector_preflight` / `probe_extension_version`, the bootstrap preflight arm and the SQLSTATE 42501/0A000 mapping on the `raw_sql` error path, plus the EOF-placed decision-table test module (EOF so the check-hardcoded-literals test-boundary heuristic is not moved ~20k lines earlier). Measured 36_943 post-fmt; ceiling 36_600 -> 36_970 (+27 headroom). Never lower the 36_400 floor. PRIOR: 2026-08-26 (#3243 Fable items 1/2/4): inbox-target arm + FOR UPDATE owner probe + owner-predicated archive INSERT/DELETE. Never lower the 36_400 floor. PRIOR: 2026-08-25 (#3243 rebase onto #3252): never lower the 36_400 floor. */
