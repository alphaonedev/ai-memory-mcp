...
const MODULE_SIZE_CEILINGS: &[(&str, usize)] = &[
    ...
    ("src/mcp/mod.rs", 16_680), /* 2026-08-22 (#3171/#3204): tools/call envelope pins (non-object `arguments` → -32602; absent `arguments` still means `{}`; reserved-sentinel `agent_id` cannot stamp the audit actor) land the file at 16_564+~70; ceiling 16_580 -> 16_680 (+headroom, lockstep). PRIOR: 2026-08-16 #2983-#2987 QUAL-10 (Batman auto-atomise remediation, 5-agent vote 4d3ea1c5): the bounded single-consumer atomise worker wiring in `run_mcp_server` (the drain-time atomiser cell + its reload re-seed + the #2985 curator-less boot WARN), the `atomise_queue` field on `ToolDispatchCtx` + `handle_request`, the `AtomiseWiring` forward in `dispatch_memory_store`, and the `handle_store_with_atomise_for_tests` injection entry point land the file at 16_495; ceiling 16_400 -> 16_580 (+85 headroom, lockstep). PRIOR: 2026-08-04 #2544 (CB-15): the namespace-standard lifecycle/expiry
