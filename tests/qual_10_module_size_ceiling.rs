...
const MODULE_SIZE_CEILINGS: &[(&str, usize)] = &[
    ...
    ("src/store/postgres.rs", 33_920), /* 2026-08-17 (#3045 coordination-plane rebased onto #3044): the pg twins of checkpoint_resolve first-resolution-wins CAS (#2995) + action_add_edge self-edge/ordering-cycle guard (#3008) land on top of the #3014/#2993 attestation-funnel changes. Combined measured 33_859; ceiling 33_860 -> 33_920 (+61 headroom). PRIOR: 2026-08-16 (#3014/#2993): attestation-funnel gate + alias secret-screen on the pg twins — `PostgresStore::{entity_register,reflect,consolidate}` cross `gate_unsigned_surface_attestation` / global-strict refuse + `attest_level="claimed"` stamp, and `entity_register` screens aliases with `screen_for_caller`/`redact_for_storage`. Measured 33_805; ceiling 33_760 -> 33_860 (+55 headroom). PRIOR: 2026-08-15 (#2948, 5-agent vote 4d3ea1c5): K3 twin of the create-funnel append-only fix — the `pg_probe_upsert_prior_version` helper + armed-path COW-SUPERSEDE leaf emission on the 7 local upsert funnels (store / store_batch / store_with_embedding_inner / capture_turn_idempotent / recover_turn_idempotent / reflect_with_hooks / consolidate-destination). Measured 33_675; ceiling 33_480 -> 33_760 (+85 headroom). PRIOR: 2026-08-15 forensic-audit-trail PR-3 (L4 lane 1): the postgres append re-signs daemon-signed rows identity-bound over the ALREADY-TRUNCATED timestamp (#1925/#2203 pg parity) + the `verify_audit_trail` twin threads the AI_MEMORY_AUDIT_PUBKEY pin and folds the shared `compute_signature_verdict` (K3 parity). Measured 33_431; ceiling 33_480 (+49 headroom). PRIOR (#2392-rebased-onto-#2567): #2392 tag-fold into the pg tsv column rebased on #2567 embedder-constructibility gate; combined measured 33_315, ceiling 33_390. */
