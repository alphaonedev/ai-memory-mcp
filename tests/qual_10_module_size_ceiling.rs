...
const MODULE_SIZE_CEILINGS: &[(&str, usize)] = &[
    ...
    ("src/storage/mod.rs", 29_260), /* 2026-08-22 (#3115): statement-atomic delete wrapper (owns_tx BEGIN/COMMIT/ROLLBACK around `delete_inner`) + ARCHIVE_REASON_DEFAULT SSOT bind at both archive funnels. Measured 29_196; ceiling 29_160 -> 29_260 (+64 headroom). PRIOR: 2026-08-20 (#2542 GA Wave-2, review rework): the ownership-based governance-chain exclusion — `namespace_standard_owner` / `parent_link_governance_entitled` / `warn_governance_graft_excluded` helpers + the `ChainView::Governance` entitled-parent walk (drops a cross-tenant `parent_namespace` link with a WARN). Measured 29_101; ceiling 29_040 -> 29_160 (+59 headroom). PRIOR: 2026-08-20 (#2954 GA Wave-2): the federation newer-wins COW-SUPERSEDE wiring in `insert_if_newer` — the `FederationMergePreimage` struct + `probe_federation_merge_preimage` (armed-only, under the extended owns_tx BEGIN IMMEDIATE) + the extracted `emit_federation_newer_wins_supersede_leaf_if_enabled` won-and-changed helper. Measured 28_949 post-fmt; ceiling 28_850 -> 29_040 (+91 headroom). PRIOR: 2026-08-19 (#2515, 5-agent vote #3089 Vote C): the `insert_inner` expires_at arm gains the #2335 tier-aware lattice FLOOR (scalar MAX over the COALESCE'd pair, replacing the bare COALESCE that shortened a live TTL on re-store) + the multi-line rationale comment. Measured 28_790; ceiling 28_780 -> 28_850 (+60 headroom). PRIOR: 2026-08-04 #2718/CB-14 QUAL-10: `memories_updated_since_counted` (raw pre-drop row count for the F7 tie-group guard) + `sync_state_merge_authorized` (per-key-authorized #2670-class fold) + 2 unit tests. Measured 28_160; ceiling 28_120 -> 28_200 (+40 headroom). 2026-08-10 (#2860, 5-agent vote 4d3ea1c5): the `set_row_metadata` free fn (federated-consolidate finalize metadata persist). Measured 28_211; ceiling 28_200 -> 28_300 (+89 headroom). 2026-08-10 (#2771, 5-agent vote 4d3ea1c5): `insert_no_overwrite` + the `insert_inner` `fail_on_conflict` fail-closed create arm (`ON CONFLICT DO NOTHING` -> typed ConflictError; single-sourced column list). Measured 28_363; ceiling 28_300 -> 28_450 (+87 headroom). 2026-08-11 (#2887, 5-agent vote 4d3ea1c5): the `InsertConflictArm` enum + `insert_restore_same_id` + the `insert_inner` `RestoreSameId` CAS arm (`ON CONFLICT DO UPDATE ... WHERE memories.id = excluded.id` -> restore-safe lost-update close for the reversible-rollback paths). Measured 28_455; ceiling 28_450 -> 28_550 (+95 headroom). 2026-08-14 (#2935, 5-agent vote 4d3ea1c5): consolidation-laundering fix — confidence=min(source confidences) + memory_kind=Claim at db::consolidate, both output builders, plus the regression assert. Measured 28_608; ceiling 28_550 -> 28_680 (+72 headroom). 2026-08-15 (#2948, 5-agent vote 4d3ea1c5): route the `insert_inner` `ON CONFLICT DO UPDATE SET content` overwrite through the append-only ledger — the armed-path version pre-probe + unified owns_tx guard + the extracted `emit_upsert_supersede_leaf_if_enabled` COW-supersede helper. Measured 28_713; ceiling 28_680 -> 28_780 (+67 headroom). */
    ...
    ("src/store/postgres.rs", 34_720), /* 2026-08-22 (#3110): visibility-gate comments + caller_opt SQL arm on search_with_source_uri. Measured 34_645; ceiling 34_620 -> 34_720 (+75 headroom). PRIOR: 2026-08-20 (#2542 GA Wave-2, review rework): the pg twin of the ownership-based governance-chain exclusion — `pg_namespace_standard_owner_{pool,in_tx}` + `pg_warn_governance_graft_excluded` + the `governance`-parameterized `pg_namespace_chain` / `build_namespace_chain_in_tx` (entitled-parent walk); the trait `build_namespace_chain` reverts to the pure LOOKUP chain (Finding 4). Measured 34_562; ceiling 34_500 -> 34_620 (+58 headroom). PRIOR: 2026-08-20 (#3055, 5-agent vote #3089 cluster C-fed-integrity-representation), rebased onto #2954: the AGE schema-placement repair — the `after_connect` `normalize_app_search_path` fix (reorders the session search_path so the first real app schema — `public` in prod, the #1381 per-test schema in tests — precedes `"$user"` and any `ag_catalog`, with `ag_catalog` kept LAST, WITHOUT hard-coding `public`; closes the CVE-2018-1058 `$user`-precedence split-brain) + its unit tests + the `$user`/prod regression tests, PLUS the pre-bootstrap `relocate_app_tables_to_public` self-heal for the already-affected installed base (guarded `ALTER TABLE ... SET SCHEMA` over the 37-table + 6-sequence allowlist), the SQL consts, and the connect() call site + rationale comments. Measured 34_444; ceiling 34_200 -> 34_500 (+56 headroom). PRIOR: 2026-08-20 (#2954 review-fix): `pg_advisory_lock_title_namespace` (xact-scoped `pg_advisory_xact_lock(hashtext,hashtext)`) acquired in BOTH `pg_probe_upsert_prior_version` (#2948 create funnel) and the #2954 federation probe to serialize concurrent same-(title,namespace) applies — closes the fresh-insert TOCTOU that could MISS a supersede leaf — plus `duration_trunc` µs-alignment of the won-and-changed predicate to the pg timestamptz. Measured 34_174; ceiling 34_160 -> 34_200 (+26 headroom). PRIOR: 2026-08-20 (#2954 GA Wave-2): the federation newer-wins COW-SUPERSEDE wiring in `apply_remote_memory` — the `PgFederationMergePreimage` struct + `pg_probe_federation_merge_preimage` (armed-only `SELECT … FOR UPDATE` inside the tx) + the won-and-changed `pg_emit_revision_leaf_if_enabled` call at the upsert site. Measured 34_099 post-fmt; ceiling 34_060 -> 34_160 (+61 headroom). PRIOR: 2026-08-19 (#2515, 5-agent vote #3089 Vote C): the five local upsert funnels (store / store_batch / capture_turn_idempotent / recover_turn_idempotent / store_with_embedding_inner) gain the #2335 tier-aware lattice FLOOR (GREATEST over the COALESCE'd pair, replacing the bare COALESCE that shortened a live TTL on re-store) + the shared multi-line rationale comment. Measured 34_005; ceiling 33_980 -> 34_060 (+55 headroom). PRIOR: 2026-08-19 (#3070/#3074 enterprise-federation pg-audit): the semantic recall pool gains the `target_agent_id` inbox carve-out (parity with the FTS pool + sqlite), and the bootstrap/migrate/index-build advisory-lock connections are CLOSED (via the new `close_timeout_relaxed_conn` helper) instead of returned to the pool with a cleared `statement_timeout` (#3074). Measured 33_922; ceiling 33_920 -> 33_980 (+58 headroom). PRIOR: 2026-08-17 (#3045 coordination-plane rebased onto #3044): the pg twins of checkpoint_resolve first-resolution-wins CAS (#2995) + action_add_edge self-edge/ordering-cycle guard (#3008) land on top of the #3014/#2993 attestation-funnel changes. Combined measured 33_859; ceiling 33_860 -> 33_920 (+61 headroom). PRIOR: 2026-08-16 (#3014/#2993): attestation-funnel gate + alias secret-screen on the pg twins — `PostgresStore::{entity_register,reflect,consolidate}` cross `gate_unsigned_surface_attestation` / global-strict refuse + `attest_level="claimed"` stamp, and `entity_register` screens aliases with `screen_for_caller`/`redact_for_storage`. Measured 33_805; ceiling 33_760 -> 33_860 (+55 headroom). PRIOR: 2026-08-15 (#2948, 5-agent vote 4d3ea1c5): K3 twin of the create-funnel append-only fix — the `pg_probe_upsert_prior_version` helper + armed-path COW-SUPERSEDE leaf emission on the 7 local upsert funnels (store / store_batch / store_with_embedding_inner / capture_turn_idempotent / recover_turn_idempotent / reflect_with_hooks / consolidate-destination). Measured 33_675; ceiling 33_480 -> 33_760 (+85 headroom). PRIOR: 2026-08-15 forensic-audit-trail PR-3 (L4 lane 1): the postgres append re-signs daemon-signed rows identity-bound over the ALREADY-TRUNCATED timestamp (#1925/#2203 pg parity) + the `verify_audit_trail` twin threads the AI_MEMORY_AUDIT_PUBKEY pin and folds the shared `compute_signature_verdict` (K3 parity). Measured 33_431; ceiling 33_480 (+49 headroom). PRIOR (#2392-rebased-onto-#2567): #2392 tag-fold into the pg tsv column rebased on #2567 embedder-constructibility gate; combined measured 33_315, ceiling 33_390. */
