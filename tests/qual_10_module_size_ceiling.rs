...
const MODULE_SIZE_CEILINGS: &[(&str, usize)] = &[
    ...
    // 2026-09-03 (#3476, second fleet bump) — 32_990 -> 33_600: #3420 #3345 #3382 #3464 each carry a
    // lockstep bump of this entry with different values and conflict pairwise at merge. One fleet
    // value; branches take this line on rebase and drop their own.
    // 2026-09-03 (#3476, fourth fleet bump) — 33_600 -> 34_000: the wave stacks #3463 (+122) #3464 #3382 (+182)
    // #3345 on this module; one fleet value with headroom for all of them.
    // 2026-09-10 (#3587 U5a, swarm anti-drift scaffolding): 34_000 -> 35_200 so
    // U1 `archive_as_superseded` (four-funnel write path + in-module tests) and
    // U2 line-file `transcript_line_dedup` consumers can land without a ceiling
    // fight. This unit has no behaviour change. MEASURED 33_848 at U5a land
    // (152 headroom under 34_000 — audit-C F12). #3297 re-MEASURED 34_014
    // on 1880b1cc (`wc -l`; ceiling 35_200 unchanged, 1_186 headroom).
    // Never lower. Ceiling 34_000 -> 35_200
    // (+1_352 headroom for the remaining #3587 units, lockstep).
    // 2026-09-17 (#3758 SET-namespace-standard rebind gate): the shared
    // `namespace_standard_binding` reader (the ONE read both the SET and the
    // CLEAR admission consult, so the two ops cannot drift on what "currently
    // bound" means) + its dated comment add 41 lines; on the chain-8 landing
    // tip 8b56f33e6 (35_180 after the six landed commits) that crosses the
    // ceiling. MEASURED 35_221 on the assembled tree. Never lower.
    // Ceiling 35_200 -> 35_300 (+79 headroom, lockstep — the crossing commit
    // bumps).
    ("src/storage/mod.rs", 35_300), /* 2026-09-01 (#3388 pending_reject approver gate): the shared `evaluate_approver_eligibility` predicate + `ApproverEligibility` / `RejectOutcome` enums + `reject_with_approver_type` — the ONE eligibility gate `approve_with_approver_type` and the new reject twin both call, so the veto path can no longer lack (or drift from) the self-approval / named-approver / registered-agent rules approve enforces — plus their dated comments, net of the four per-arm blocks they replace, grow this file to MEASURED 32_887. Never lower the 30_680 floor. Ceiling 32_790 -> 32_990 (+103 headroom, lockstep). PRIOR: 2026-08-31 (#3327 GA-review polish batch g4): the fail-closed hardening of three storage funnels — Sec-F2 (`sever_namespace_standards` now PROPAGATES the `SUBSTRATE_NAMESPACE_STANDARD_SEVERED` audit-append error, matching the #3290 pg twin, so a governance-binding sever cannot commit without its tamper-evident row), Sec-F4 (`swarm_rewind`'s already-Contaminated root branch now checks its in-place marker CAS row count and rolls back on 0, mirroring the sibling `contaminate_row`/`Vanished` fail-closed path so a concurrent state-move can no longer commit a signed rewind whose idempotency marker never persisted), and Sec-F6 (`swarm_rewind_audit_payload` returns `Result` and propagates the serialization error instead of `.unwrap_or_default()`, so a bad payload aborts rather than committing a wrong-hash audit row) — plus their dated comments grow this file to MEASURED 32_691. Never lower the 30_680 floor. Ceiling 32_685 -> 32_790 (+99 headroom, lockstep). PRIOR: 2026-08-31 (#3281 invalidate_link audit hash parity): the shared `canonicalize_valid_until_stamp` helper — the ONE canonical µs-RFC3339 form BOTH the sqlite and postgres invalidation paths stamp + hash into the `memory_link.invalidated` audit-leaf pre-image (was: raw wire string on sqlite vs a chrono re-render on pg → divergent leaf hash) — grows this file to MEASURED 32_585. Never lower the 30_680 floor. Ceiling 32_550 -> 32_685 (+100 headroom, lockstep). PRIOR: 2026-08-31 (#3322 #3266 MVG memory_swarm_rewind): the atomic/resumable/reversible `swarm_rewind` orchestration (root+cascade contamination via the extracted shared `contaminate_row` helper, operator routine-freeze, signed `swarm.rewind` audit append inside the one transaction, #3323 lineage cost report) plus the `SwarmRewindReport` / `SwarmRewindCost` structs land storage/mod.rs at MEASURED 32_448. Never lower the 30_680 floor. Ceiling 32_100 -> 32_550 (+102 headroom, lockstep). PRIOR: 2026-08-30 (#3324 #3266 MVG contaminated lifecycle auto-propagation): the transactional auto-stamp `stamp_contaminated_descendants` (+ `ContaminationStampReport` + `CONTAMINATION_METADATA_KEY`) that promotes the read-only transitive-suspect walk to a bounded/cycle-safe/idempotent/reversible WRITE tainting derives_from descendants on invalidate, rebased onto #3323 (which took v93 + touched this file), grows it to MEASURED 31_998. Never lower the 30_680 floor. Ceiling 31_900 -> 32_100 (+102 headroom). PRIOR: 2026-08-30 (#3188 namespace governance parity): the fail-closed rewrite of auto_detect_parent (direct standard_id-IS-NOT-NULL probe replacing the fault-swallowing get_namespace_standard().ok().flatten()) plus four #3188 sqlite regression tests (auto_detect happy/none/fail-closed + parent=None inferred-parent bind) grow this file to MEASURED 31_829. Never lower the 30_680 floor. Ceiling 31_800 -> 31_900 (+71 headroom). PRIOR: 2026-08-30 (#3279 since/until instant parity): the CREATED_AT_INSTANT_FMT const + created_at_instant_window helper and the strftime-both-sides rewrite of the five Filter/recall-driven created_at since/until predicates (build_list_query, search_with_source_uri, recall, recall_hybrid FTS + semantic) plus bound canonicalization grow this file to MEASURED 31_711. Never lower the 30_680 floor. Ceiling 31_700 -> 31_800 (+89 headroom). PRIOR: 2026-08-30 (#3196 find_paths traversal budget): the bounded level-by-level BFS engine (FindPathsTraversal + build_undirected_adjacency + fetch_frontier_edges) replacing the recursive CTE, plus six 3196 regression tests, grow this file to MEASURED 31_640. Never lower the 30_680 floor. Ceiling 31_300 -> 31_700 (+60 headroom). PRIOR: 2026-08-27 (#3272/#3286 erasure primitives): C-1 unconditional SEVER hoisted in delete_inner, C-2 restore-only forget-tombstone clear in insert_inner, C-3 DLQ/dedup purge moved into the shared tombstone_and_erase primitive, plus four sqlite regression tests, grow this file to MEASURED 31_134. Never lower the 30_680 floor. Ceiling 31_000 -> 31_300 (+166 headroom). PRIOR: 2026-08-27 (#3235 rebase onto 19892abf / #3267): auto-merge of boot-health cluster onto #3267+#3240+#3237 MEASURED 30_882. Never lower the 30_680 floor from #3237. Ceiling 30_680 -> 31_000 (+118 headroom). PRIOR (release): 2026-08-26 (#3237 rebase onto #3242): measured 30_619 after #3178 windowed link + #3176 evaluate_level Result + #3203 fence + #3204 µs stamp stacked on the 30_380 floor. Never lower. Ceiling 30_380 -> 30_680 (+61 headroom, lockstep). PRIOR: 2026-08-25 (#3253 rebase onto #3255): never lower the #3224 floor 30_380. PRIOR (PR): 2026-08-26 (#3235 rebase onto e9bf9dea): never lower. Take max(release 30_380 from #3253, PR 30_640 from #2402/#2614/#2606/#2626). PRIOR: 2026-08-25 (#3235 rebase onto 6187e189): MEASURED 30_555; ceiling 30_380 -> 30_640 (+85 headroom). */
    // 2026-07-21 (#1802 R-05 S1) — NEW submodule extracted from
    // storage/mod.rs (doctor / observability probes). Measured 698;
    // ceiling 800 (+102). 2026-09-12 (#3655 sync watermark probe): the
    // `SyncPeerWatermark` / `SyncWatermarks` types + `doctor_sync_peer_watermarks`
    // (propagates the query failure, counts + names malformed rows, ages the
    // three cursors against the probe time) replace the 24-line
    // `doctor_max_sync_skew_secs`; MEASURED 816. Never lower the 800 floor.
    // Ceiling 800 -> 900 (+84 headroom, lockstep).
    ("src/storage/doctor.rs", 1_000),
    ...
    // 2026-09-02 (#3476) — 17_150 -> 17_400, ONE fleet bump for the v1.0.0 fix wave: the tip sat at
    // 17147/17_150 after #3356 and every branch adding a dispatch arm (#3357, #3379-#3383, ...)
    // was bumping it privately with conflicting values. Branches take this line on rebase and drop
    // their own. The module split is tracked separately.
    // 2026-09-14 (#3393) — 17_400 -> 17_447, MEASURED on this branch (43dd2d959, `wc -l`): the
    // observe_capture_nag_keys_on_caller_not_body_claimed_agent_id_3393 pin (+89 over the head's
    // 17_358) lands in the in-module test block, and the branch's own gate was RED at 17_447 —
    // approved once without running the gate that bounds the file (Conductor comment
    // 5662252639), so this bump is exact-to-actual, zero headroom by design: new pins for this
    // module go in their own tests/ files (#3373 / #3392 / #3730 already do). RESOLVED-TREE NOTE:
    // the merge queue stacks #3373 (+40) on this branch (+89) on a shared +28 — three additive
    // deltas, measured by the Conductor at 17_515 on the resolved order — so whoever merges LAST
    // re-measures this file on the resolved tree and re-cuts to that actual; 17_447 is the
    // branch truth, not the chain's. Never lower.
    ("src/mcp/mod.rs", 17_600), /* #3393-v2 QUAL-6 fix: `dispatch_memory_capture_turn` renders the now anyhow-typed `handle_capture_turn_mcp` into the legacy String envelope at the dispatch boundary (+1 `.map_err` line, +2 comment lines); MEASURED 17_450, ceiling 17_447 -> 17_450 exact, lockstep, never lower. PRIOR: #3297 re-MEASURED 17_345 on 1880b1cc (`wc -l`; ceiling 17_400 unchanged, 55 headroom). PRIOR: 2026-08-31 #3329 QUAL-10: the non-blocking boot embedding backfill — the `backfill_on_boot` dispatch in `run_mcp_server`, `run_embedding_backfill_bounded` + `BackfillProgress` + `BOOT_BACKFILL_BLOCKING_BUDGET` + `spawn_boot_backfill`, plus the bounded/background backfill regression tests — lands mcp/mod.rs at MEASURED 17_073. Ceiling 16_890 -> 17_150 (+77 headroom, lockstep, never lower). PRIOR: 2026-08-29 B15 QUAL-10: MEASURED 16_809 after Wave-2 B6 MCP fail-closed allowlist. Ceiling 16_760 -> 16_890 (+81 headroom, lockstep, never lower). PRIOR: 2026-08-24 (#3220 rebase onto 72e4c100): MEASURED 16_683. Ceiling 16_680 -> 16_760 (+77 headroom). PRIOR: 2026-08-22 (#3171/#3204): tools/call envelope pins (non-object `arguments` → -32602; absent `arguments` still means `{}`; reserved-sentinel `agent_id` cannot stamp the audit actor) land the file at 16_564+~70; ceiling 16_580 -> 16_680 (+headroom, lockstep). PRIOR: 2026-08-16 #2983-#2987 QUAL-10 (Batman auto-atomise remediation, 5-agent vote 4d3ea1c5): the bounded single-consumer atomise worker wiring in `run_mcp_server` (the drain-time atomiser cell + its reload re-seed + the #2985 curator-less boot WARN), the `atomise_queue` field on `ToolDispatchCtx` + `handle_request`, the `AtomiseWiring` forward in `dispatch_memory_store`, and the `handle_store_with_atomise_for_tests` injection entry point land the file at 16_495; ceiling 16_400 -> 16_580 (+85 headroom, lockstep). PRIOR: 2026-08-04 #2544 (CB-15): the namespace-standard lifecycle/expiry
    ...
    // 2026-09-03 (#3476, second fleet bump) — 40_800 -> 41_200: #3420 #3345 #3382 #3464 each carry a
    // lockstep bump of this entry with different values and conflict pairwise at merge. One fleet
    // value; branches take this line on rebase and drop their own.
    // 2026-09-03 (#3476, fourth fleet bump) — 41_200 -> 41_600: the wave stacks #3463 (+122) #3464 #3382 (+182)
    // #3345 on this module; one fleet value with headroom for all of them.
    // 2026-09-14 (#3718 review follow-up) — 42_500 -> 42_600: the seal-path sealed-rows guard
    // (`seal_content_guarded` + the connection-taking `seal_content_for_insert` / `UpsertSeal::seal`)
    // lands +58 net on this module; MEASURED 42_503 on ed23a3709, ceiling 42_600.
    ("src/store/postgres.rs", 43_400), /* 2026-09-23 (#3883/#3882 AGE orphaned-graph structural classification, 5-agent vote 4d3ea1c5 option A): the orphan reason/prefix/signature consts + `is_age_graph_orphan` + the two `ag_catalog.ag_graph` registry probes (tx-savepoint-wrapped + pool) + the shared `record_failed_age_projection` absorb helper + the A2 orphan probe in `project_link_into_age` + the A4 third-reason arms on `warn_age_fallback`/`_pair` + the A3 drainer self-heal + the A6/#3882 `ensure_memory_graph` boot orphan WARN + the #2511 orphan assertion + the sqlite-free classifier unit test bring this file to MEASURED 43_164 on the REBASED tree (carrier eee7bf2e2, `wc -l` = `content.lines().count()`, the qual_10 gate instrument). RE-MEASURED after the rebase per the #2627/#3520 no-carried-ceiling lesson: the pre-rebase base 5a9b50731 measured 43_195, but Boids items 1+2 shrank the carrier ~31 lines, so the rebased figure is 43_164 -- a ceiling carried across a rebase is UNVERIFIED until re-measured. At 43_164 the file FITS under the 43_400 ceiling with 236 headroom -- the additions did NOT overflow it. The #3883 drainer-gate follow-up (gate_record_stop at the top of drain_kg_projection_outbox + the interval-loop record-stop warn-suppression, #3883/#3942) brings this file to 43_199, still under 43_400 with 201 headroom. They land inline because each reads the enclosing tx/loop state (the projection savepoints, the create_graph error branch, the per-edge ids) and cannot be extracted without threading the transaction out of this file. The bump 43_200 -> 43_400 RESTORES the ceiling's headroom policy (+236 over the rebased measure 43_164, lockstep, never lower), it does not react to an overflow. PRIOR: 2026-09-15 (Consolidation Unit 1 #3690 #3695 #3691 #3693 #3626 #3699, RE-CUT onto the corrected tree 23d414b17, which had taken the merger bump 42_600 -> 42_700 at MEASURED 42_623): the branch alone measured 42_981 on 1ec64196b and asked for 43_100; the STACKED tree measures 43_063 — 37 lines of headroom, below the ~70 this file keeps so the next unrelated line cannot red a branch that did nothing wrong. Ceiling 43_100 -> 43_200 (+137 headroom, lockstep, never lower). The branch-alone measurement: the title-slot admission helpers (`pg_title_slot_holder` / `pg_same_id_hidden_row_under_key` / `pg_title_slot_admission`), the `TITLE_SLOT_CONFLICT_TARGET` + merge-backstop rewrite of the eight `ON CONFLICT (title, namespace)` literals (now `format!`-built), the by-id tombstone restore arm, the guarded consolidation tombstone UPDATE, the visible-only `find_contradictions` / `find_by_title_namespace`, the #3626 owner-drop suffix on every arm, `migrate_v100`, and the #3699 cross-id merge WARN grow this file to MEASURED 42_981 on this branch alone. A merger bump to 42_700 was taken on the corrected #3654/#3655 tree (measured 42_623 there); the two stack, so the fleet value must be re-measured on the combined tip. Never lower. Ceiling 42_600 -> 43_100 (+119 headroom, lockstep). PRIOR: #3297 re-MEASURED 42_414 on 1880b1cc (`wc -l` after the AGE-1.8.0 comment rewrite; ceiling 42_500 unchanged, 86 headroom). PRIOR: 2026-09-08 (#3423 reflect owner + `reflects_on` edge attestation parity, REBASED onto release/v1.0.0 02580a51): the postgres `reflect_with_hooks` `memory_links` INSERT now SIGNS each `reflects_on` edge with `hooks.active_keypair` over the same `SignableLink` CBOR the sqlite twin signs (pre-image + `attest_level`/`observed_by` binds, stamped once through #3422 `canonical_created_at_stamp`), +61 lines that cannot live in a submodule: the block signs INSIDE the open `tx` and reads the per-edge `src_id` of the enclosing loop, so extracting it would have to thread the transaction out of this file. BUMPED ONLY BY THE REBASE, and that is the whole point of re-measuring: on its own base (70c20c8d) this file was 40_584 and the branch landed 40_645 under the then-ceiling 40_680, needing NO bump — the tip has since grown to 42_344 with only 56 headroom under 42_400, so the identical +61 now overflows by 5. A ceiling carried across a rebase is UNVERIFIED until re-measured (the #2627 / #3520 lesson). MEASURED 42_405 on the rebased tree. Ceiling 42_400 -> 42_500 (+95 headroom, lockstep, never lower). PRIOR: 2026-09-07 (#3520 pg deadlock retry + bootstrap catalog pre-check, REBASED onto integration/f1 5fef2eb8): the six destructive-funnel bodies (`size_gc` / `run_gc` / `forget` / `archive_restore` / `archive_by_ids` / `delete`) rewrapped in the shared `tx_retry::TxRetry` loop with their data-integrity rationale, the bootstrap catalog pre-check wiring in `connect_with_dim`, the two new submodule declarations and the `PG_BOOTSTRAP_TEST_LOCK` cfg(test) guard land this file at MEASURED 42_311 on the rebased tree. RE-DERIVED after the rebase rather than carried forward: the pre-rebase branch measured 42_419 and asked for 42_500, but #3527 moved 161 lines of chain-append allocation OUT into `src/store/postgres/chain_append.rs`, so the merged file needs far less — the #2627 lesson that a ceiling arriving through a merge is UNVERIFIED until re-measured, applied to a rebase. The retry driver and the DDL filter themselves live in NEW modules (`src/store/postgres/tx_retry.rs`, `src/store/postgres/bootstrap_ddl.rs`) per the submodule-over-bump rule; what lands here is only the per-funnel wrapping that cannot live elsewhere. Never lower. Ceiling 42_250 -> 42_400 (+89 headroom, lockstep). PRIOR: 2026-09-06 (#3525 pg connect-lock probe ladder): the probe schedule moved OUT of this file into the new `src/store/pg_migration_lock.rs` (MIGRATION_LOCK_POLL_BASE_MS / _MAX_MS and the MigrationLockProbeLadder that walks them), so this file NETS SMALLER at MEASURED 42_183. Ceiling deliberately left at 42_250 rather than tightened: the shrink is a module extraction, not headroom earned by this file, and re-tightening would only have to be undone by the next pg change. PRIOR: 2026-09-06 (#3519 pg bootstrap/migrate advisory-lock wait): the poll-based `acquire_migration_advisory_lock` helper (bounded doubling `pg_try_advisory_lock` backoff, progress logging, fail-closed refusal), its five budget/cadence consts, the shared `prepare_migration_lock_connection` / `release_migration_advisory_lock` pair (which also closes the #3074 leak on the acquire-failure exits), the two rewired lock sites, the `the_lock_wait_budget_tracks_the_index_build_budget_3519` unit test and the widened CIC design note grow this file to MEASURED 42_187. Never lower. Ceiling 42_000 -> 42_250 (+63 headroom, lockstep). PRIOR: 2026-09-06 (Master lockstep on the COMBINED tip 486db24f = 87f86a0a + #3345 + integration/f1 (#3064 #3075 …) + #3516): MEASURED 41_904 on the merged tree. Each lane measured on its own base (#3075 saw 41_729; the #3345 amendment reverted to 41_800 after its rollup rewrite) but the growth STACKS across the merge, so the fleet value is taken on the combined tip at the final gate. Ceiling 41_800 -> 42_000 (+96 headroom, lockstep, never lower). PRIOR: 2026-09-05 (#3424 kg/query wire-shape parity): the bounded `hydrate_kg_query_rows` step (targets via ANY, last-hop edges via UNNEST on the full (source_id,target_id,relation) key) in both AGE and CTE traversals grows this file to MEASURED 41_676. Never lower the 39_350 floor. Ceiling 41_600 -> 41_800 (+124 headroom, lockstep). PRIOR: 2026-09-02 (#3419 attested-write replay ledger): the v95 `MIGRATION_V95_ATTESTED_WRITE_LEDGER` include + `migrate_v95` + its dispatch arm, the settled-arm literal-94 stamp fix on `migrate_v94`, and the `admit_attested_write` SAL impl (the admit-once `INSERT ... ON CONFLICT DO NOTHING` whose PRIMARY KEY IS the decision, plus the bounded-retention prune) grow this file to MEASURED 40_692. Never lower the 39_350 floor. Ceiling 40_680 -> 40_800 (+108 headroom, lockstep). PRIOR: 2026-08-31 (#3041 pg-twin equal-instant acyclicity): the postgres structural equal-instant cycle guard `lineage_would_close_cycle_pg` (a lean id-only recursive-CTE ancestor walk to the fail-CLOSED `LINEAGE_CYCLE_CHECK_MAX_DEPTH` ceiling) + the rewired three-arm EQUAL branch in `validate_link_pre_create_pg` Pass 0 (closing the pg fail-OPEN twin of the sqlite #3041 fix, plus the >5-hop completeness gap) grow this file to MEASURED 40_575. Never lower the 39_350 floor. Ceiling 40_500 -> 40_680 (+105 headroom, lockstep). PRIOR: 2026-08-31 (#3259 pg promote to_namespace clone): the decide-only destination-STORE gate twin (`pg_refuse_unapproved_destination_store`), the pool ns-owner resolver (`pg_resolve_namespace_owner`), the cross-namespace clone (`pg_promote_to_namespace`) calling the SHARED storage helpers, and the rewired `execute_pending_action` promote arm grow this file to MEASURED 40_362. Never lower the 39_350 floor. Ceiling 40_200 -> 40_500 (+138 headroom, lockstep). PRIOR: 2026-08-30 (#3323 per-lineage token/cost accounting): `migrate_v93` (token_cost_counters DDL retrofit) + the v93 dispatch arm + the two `store`-family write hooks (`record_write_pg`) + the `recall_hybrid` hook (`record_recall_pg`) + the settled v91/v92 literal-stamp fixes grow this file to MEASURED 40_049. Never lower the 39_350 floor. Ceiling 40_050 -> 40_200 (+151 headroom, lockstep). PRIOR: 2026-08-30 (#2555 schema_version poison guard): the poison pre-check in the bootstrap downgrade guard + `migrate_v92` (pg CHECK retrofit: pg_constraint probe, out-of-band pre-flight, ADD CONSTRAINT doc-twin) + the v92 dispatch arm + the `MIGRATION_V92_SCHEMA_BOUND` const grow this file to MEASURED 39_925. Never lower the 39_350 floor. Ceiling 39_850 -> 40_050 (+125 headroom, lockstep). PRIOR: 2026-08-30 (#3172 schema-masks-data-loss gate): the postgres twin `enforce_lineage_watermarks` + `record_lineage_watermark_pg` methods and their connect-bootstrap call grow this file to MEASURED 39_753. Never lower the 39_350 floor. Ceiling 39_650 -> 39_850 (+97 headroom, lockstep). PRIOR: 2026-08-29 (B17 #3230/#3231/#3250): effective-tier expires_at CASE on both update funnels, capture_turn in-tx advisory-lock re-probe, v91 archived_memory_links CID columns + migrate_v91, grow this file to MEASURED 39_517. Never lower the 39_350 floor. Ceiling 39_500 -> 39_650 (+133 headroom). PRIOR: 2026-08-27 (#3291 SignableLink created_at): attest memory_links.created_at in the link pre-image — truncate-before-sign+INSERT in link_internal (HALT R1b S52), SELECT created_at on kg_invalidate prior-row + verify_link (all three SELECT variants), grow this file to MEASURED 39_351. Never lower the 39_350 floor. Ceiling 39_350 -> 39_500 (+149 headroom). PRIOR: 2026-08-27 (#3272/#3286 erasure primitives): C-1 unconditional namespace_meta SEVER in pg_hard_delete_in_tx, C-2 restore-only forget-tombstone clear in store_with_embedding_inner, C-3 shared pg_purge_dlq_dedup_in_tx routed from delete/evict/forget, plus three live-PG regression tests, grow this file to MEASURED 39_218. Never lower the 38_800 floor. Ceiling 39_000 -> 39_350 (+132 headroom). PRIOR: 2026-08-27 (#3270/#3271/#3275/#3296 authz cluster): the SAL caller-owns fixes add archive_restore owner predicate + delete_link owner gate + archive_by_ids rows_affected/sort + three pg regression tests, growing this file to MEASURED 38_907. Never lower the 38_800 floor from the #3235 rebase. Ceiling 38_800 -> 39_000 (+93 headroom). PRIOR (#3235 rebase onto 19892abf / #3267): postgres.rs auto-merged (boot-health cluster + pgvector preflight + SAL-parity). MEASURED 38_654 (wc -l on the resolved tree; a clean auto-merge is UNVERIFIED — always re-measure). Never lower the 37_850 floor from #3267×#3240. Ceiling 37_850 -> 38_800 (+146 headroom). PRIOR (release, #3267 x #3240): 2026-08-26 (#3267 x #3240 cross-merge re-measure): release/v1.0.0 moved again under PR #3267 — Grok's #3240 (pg SAL-parity cluster) grew this file to 36_999 on the release side while the #3267 review-fix wave grew it to 37_168 on the branch side; the merge of c3344757 x 0782e6ce auto-merges postgres.rs cleanly and MEASURES 37_766. Ceiling max(37_080, 37_400) -> 37_850 (+84 headroom over the merged measure). Never lower the 36_600 floor. PRIOR (PR, #3235): 2026-08-26 (#3235 rebase onto e9bf9dea): never lower. Take max(release 36_600 from #3243, PR 37_110 from #2614/#2402/#2626). PRIOR: 2026-08-25 (#3235 rebase onto 6187e189): MEASURED 37_030; ceiling 36_200 -> 37_110 (+80 headroom). */
    ...
    ("src/config.rs", 15_600), /* 2026-09-21 #3811: MEASURED 15_504; ceiling 15_500 -> 15_600 (+96 headroom, lockstep). Growth is test coverage for a credential-mis-delivery fix: +102 test lines and -27 resolver lines, net +75 from base 15_429. Preserve the regression coverage; Master authorized this bump at 18:06Z. PRIOR: #3297 re-MEASURED 15_213 on 1880b1cc (`wc -l`; ceiling 15_500 unchanged, 287 headroom). PRIOR: 2026-09-10 (#3587 U5a, swarm anti-drift scaffolding): 15_120 -> 15_500 so U3's two `[curator]` keys (`stale_ruling_days`, `stale_ruling_notify_min_interval_secs`, 20–40-line field docs each) and U1's `[autonomy] supersede_on_contradiction = "propose"` section can land without a ceiling fight. This unit has no behaviour change. MEASURED 15_028 (92 headroom under 15_120 — audit-C F12). Never lower. Ceiling 15_120 -> 15_500 (+472 headroom, lockstep). PRIOR: 2026-09-06 (#3523 ONE $HOME mutex): splitting `test_env_lock` into the acquirer + the `test_env_mutex` OWNER — the single process-env `Mutex<()>` that `crate::test_support::env_lock()` now delegates to, ending the FOURTH instance of the per-module-mutex defect $HOME suffered at #1998 -> #2115 -> #2127 — plus the doc block recording why the raw mutex (not just a guard) is exposed (so the singleton can be proven by OBSERVATION: hold one wrapper, a probe thread must fail `try_lock` on the other) lands config.rs at MEASURED 15_016; ceiling 15_000 -> 15_120 (+104 headroom, lockstep, never lower). PRIOR: 2026-09-05 (#3385 archive_on_gc v2 key): the `DEFAULT_ARCHIVE_ON_GC` const + the `warn_deprecated_flat_archive_on_gc` one-shot deprecation WARN + the v2-aware `effective_archive_on_gc` / `archive_on_gc_source` resolver pair (and their precedence/data-loss docs) land config.rs at MEASURED 14_914; ceiling 14_880 -> 15_000 (+86 headroom, lockstep, never lower). PRIOR: 2026-09-02 (#3467 EPIC #3466 wake-hub core): the `[wake_hub]` config block (`WakeHubConfig` + allowlist + parity test) lands config.rs at 14_773; ceiling 14_880 (+107 headroom). Prior: */
    /* 2026-08-31 #3329 QUAL-10: the `BackfillOnBoot` enum + `resolve_embed_backfill_on_boot` resolver + `ENV_EMBED_BACKFILL_ON_BOOT`, the macOS config-path resolver (`resolve_config_path_choice` + `ConfigPathChoice`/`ConfigPathWarning` + the two macOS shadow/only WARN helpers), plus the #3329 unit-test matrix (config-path + backfill-on-boot) land config.rs at MEASURED 14_640. Ceiling 14_140 -> 14_720 (+80 headroom, lockstep, never lower). PRIOR: 2026-08-29 B15 QUAL-10: MEASURED 14_062 after Wave-2 B3 config-seed. Ceiling 14_040 -> 14_140 (+78 headroom, lockstep, never lower). PRIOR: 2026-08-25 (#3235 rebase onto 6187e189): MEASURED 13_960 after #2626 AI_MEMORY_EMBED_DIM stacked on #3205's 13_880 floor; never lower. Ceiling 13_880 -> 14_040 (+80 headroom). PRIOR: 2026-08-23 #3205 QUAL-10: api_key_file single-handle TOCTOU (`enforce_api_key_file_perms` takes `&File`) lands config.rs at 13_802; ceiling 13_800 -> 13_880 (+78 headroom, lockstep). PRIOR: 2026-08-22 #3166/#3167/#3002 QUAL-10: the fail-closed boot config resolver (`EX_CONFIG` const, the shared `skip_config()` truthy-grammar helper, `load_for_boot` / `try_load_from_optional` / the extracted `from_toml_contents` parse+validate tail, the `ErrorKind::NotFound` split in `load_from`) plus its four path-explicit regression tests, AND the #3002 migration-safety guard on `config_path()` (`LEGACY_CONFIG_DIR` + `warn_legacy_config_root_once` + the two legacy/XDG resolution tests) land config.rs at 13_715; ceiling 13_360 -> 13_800 (+85 headroom, lockstep). PRIOR: 2026-08-11 #2905 §5.3 CI-fix QUAL-10: added the `run_env_isolated_child_or_spawn` subprocess env-isolation test helper (+ `TEST_ENV_ISOLATION_ROLE_ENV`) so the §5.3 posture tests' process-global `set_var`s stop leaking into concurrent http_create handler tests (memory 2905-posture-test-env-leak). Measured 13_267; ceiling 13_220 -> 13_340 (+73 headroom). 2026-08-18 #3002: config_path() XDG-awareness doc + the `config_path_honors_xdg_config_home` regression test push config.rs to 13_350; ceiling 13_340 -> 13_360 (+10 headroom). */
    ...
    // 2026-09-14 (#3711 rebased over #3705, resolved tree) — 4_950 -> 5_050: BOTH branches asked
    // for 4_950 on their own bases (#3705 +TLS floor, #3711 +dlq_reason/origin-only sinks) and
    // the two growths STACK on the resolved tree; MEASURED 4_962 by `wc -l` here (the Conductor's
    // rule: take the maximum, then set the ceiling from the measured actual, never carry either
    // branch's number). Ceiling 5_050 (+88 headroom, lockstep, never lower).
    ("src/subscriptions.rs", 5_260), /* 2026-09-24 (#3941 snapshot the server-wide HMAC secret at dispatch): capturing `active_hooks_hmac_secret()` once on the caller thread before `rt.spawn` (so a sibling flip cannot reach a queued worker's lazy read) + its rationale doc + the current_thread red-first pin's src side grow the file to MEASURED 5_200 on the branch (5_188 on the base 2d959cab2 / c1a0ca59a). The PRIOR +89 (5_050 -> 5_200 at #3744, MEASURED 5_111) had already been consumed to 5_188 by landings between, leaving only +12, so this is a due lockstep bump not a fresh one. Ceiling 5_200 -> 5_260 (+60 headroom, lockstep, never lower). PRIOR: 2026-09-15 (#3744 userinfo strip): `authority_without_userinfo` — the ONE authority-bounding helper both SSRF guards now take their host from, so `https://a:b@169.254.169.254/` can no longer read its "host" as the username and walk past the private-range / loopback checks at registration — plus its three unit pins beside the existing SSRF cells (registration refusals naming the real host incl. IPv6 / link-local / percent-encoded userinfo; the DNS guard's second line stated explicitly; the helper's own contract) and the doc comments that carry the reasoning grow this file to MEASURED 5_111 on the branch alone (4_991 on the base 1ec64196b; identical on the wave-2 stack — a branch-level lockstep bump, not a merger bump). Never lower. Ceiling 5_050 -> 5_200 (+89 headroom, lockstep). PRIOR: 2026-09-13 (#3705): `validate_url_with` refuses every http:// target first (loopback included), the dispatcher trusts an operator-installed root (`install_dispatch_root_certificate` / `dispatch_root_certificate`, `[subscriptions].ca_cert`), and the eleven dispatch tests moved from a plaintext wiremock to an in-process TLS receiver; MEASURED 4_891; ceiling 4_850 -> 4_950 (+59 headroom, lockstep, never lower). PRIOR: 2026-09-05 #3465 QUAL-10 (agent-facing notify push): the `agent_notified` webhook-event const + its `WEBHOOK_EVENT_TYPES` entry + the `AgentNotifiedEventDetails` details struct (recipient / correlation id / sha256 content digest — never the body), plus the `webhook_event_consts_appear_in_array` unit test the `webhook_events` module doc has claimed since v0.7 but which did not exist anywhere in the tree (it parses `mod webhook_events` out of this file, so it cannot drift the way a restated list would), land the file at MEASURED 4_786 on the 9cc9cf1b base; ceiling 4_700 -> 4_850 (+64 headroom, lockstep, never lower). PRIOR: 2026-08-30 #3191 QUAL-10 (coordination-plane atomicity): F-4 atomic conditional DLQ INSERT (`INSERT ... SELECT ... WHERE (count) < cap`) replacing the two-statement probe-then-insert, and F-5 fail-closed audit-failure DLQ routing in the dispatch worker (route to DLQ + refuse to dispatch when the per-delivery audit write fails), plus their rationale docs, land the file at MEASURED 4_637; ceiling 4_600 -> 4_700 (+63 headroom, lockstep, never lower). PRIOR: 2026-08-22 #2592 QUAL-10 (silent-default cluster): `DISPATCH_SCAN_TRUNCATED_SUB_ID` + `record_dispatch_scan_truncation` (WARN + metric + durable subscription_dlq row for a subscriber scan cut at SUBSCRIPTION_DISPATCH_LIMIT) and their rationale docs land the file at 4_585; ceiling 4_520 -> 4_600 (+15 headroom, lockstep). PRIOR: 2026-07-31 #2445 — 2-line disposition note on the ordering-covered raw open; measured 4_502 PRIOR (rebased #3711 over #3705, same 4_950 asked by both): 2026-09-13 #3684/#3724 QUAL-10 (credential-to-sink family): the `dlq_reason` closed-vocabulary module (nine consts with their rationale), the origin-only `target` rendering in `send()` and the two SSRF validators, and the per-failure operator-log lines that keep the receiver's raw text OUT of the stored `subscription_dlq.last_error` land the file at MEASURED 4_881 (`wc -l`). The vocabulary lives here because `send()` is the ONE producer of the stored reason and the consts must sit beside it; the renderer and the transport classifier themselves live in the NEW `src/url_display.rs` (submodule-over-bump). Never lower. Ceiling 4_850 -> 4_950 (+69 headroom, lockstep). PRIOR: 2026-09-05 #3465 QUAL-10 (agent-facing notify push): the `agent_notified` webhook-event const + its `WEBHOOK_EVENT_TYPES` entry + the `AgentNotifiedEventDetails` details struct (recipient / correlation id / sha256 content digest — never the body), plus the `webhook_event_consts_appear_in_array` unit test the `webhook_events` module doc has claimed since v0.7 but which did not exist anywhere in the tree (it parses `mod webhook_events` out of this file, so it cannot drift the way a restated list would), land the file at MEASURED 4_786 on the 9cc9cf1b base; ceiling 4_700 -> 4_850 (+64 headroom, lockstep, never lower). PRIOR: 2026-08-30 #3191 QUAL-10 (coordination-plane atomicity): F-4 atomic conditional DLQ INSERT (`INSERT ... SELECT ... WHERE (count) < cap`) replacing the two-statement probe-then-insert, and F-5 fail-closed audit-failure DLQ routing in the dispatch worker (route to DLQ + refuse to dispatch when the per-delivery audit write fails), plus their rationale docs, land the file at MEASURED 4_637; ceiling 4_600 -> 4_700 (+63 headroom, lockstep, never lower). PRIOR: 2026-08-22 #2592 QUAL-10 (silent-default cluster): `DISPATCH_SCAN_TRUNCATED_SUB_ID` + `record_dispatch_scan_truncation` (WARN + metric + durable subscription_dlq row for a subscriber scan cut at SUBSCRIPTION_DISPATCH_LIMIT) and their rationale docs land the file at 4_585; ceiling 4_520 -> 4_600 (+15 headroom, lockstep). PRIOR: 2026-07-31 #2445 — 2-line disposition note on the ordering-covered raw open; measured 4_502 */
    /* 2026-09-10 (#3587 U5a, swarm anti-drift scaffolding): 3_600 -> 3_900
     * so U4 `install claude-code --hook capture` (Stop entry builder +
     * command fn + apply/remove pair + docs; audit-B ≈ 90–140 LOC) and the
     * operator-ruled Codex capture leg can land without a ceiling fight.
     * This unit has no behaviour change. MEASURED 3_558 at U5a land
     * (42 headroom under 3_600 — audit-C F12). #3297 re-MEASURED 3_715
     * on 1880b1cc (`wc -l`; ceiling 3_900 unchanged, 185 headroom).
     * Never lower. Ceiling 3_600 -> 3_900 (+342
     * headroom, lockstep). PRIOR: 2026-09-02 (#3432) QUAL-10 lockstep: the two dry-run-diff redaction
     * regression tests (a client config whose `mcpServers.<x>.env` block
     * holds an API key must not have it printed, and `--apply` must still
     * write the REAL key, not the mask) plus their fixture land the file at
     * MEASURED 3_558; ceiling 3_500 -> 3_600 (+42 headroom, lockstep, never
     * lowered). */
    ...
    ("src/storage/migrations.rs", 7_400), /* 2026-09-13 (#3655 review rework, v99 `sync_peer_contact`): the v99 include_str const, `migrate_v99`, the literalised v98 arm plus the new tip arm land migrations.rs at MEASURED 7_309 (`wc -l` after `cargo fmt`). Ceiling 7_300 -> 7_400 (+91 headroom, lockstep, never lower). PRIOR: */
    /* 2026-09-03 (#3344 v96 embed_skip rung merged onto the #3419 v95 rung): the merge result lands migrations.rs at 7_007 — neither side alone breached 6_930; 7_300 leaves room for the #3464 v97 rung. Prior: */
    /* 2026-09-02 (#3419 attested-write replay ledger): the v95 `MIGRATION_V95_SQLITE` include + the `attested_write_ledger` bootstrap-SCHEMA table + index + the v95 ladder arm (and the literalisation of the settled v94 arm) grow this file to MEASURED 6_828. Ceiling 6_780 -> 6_930 (+102 headroom, lockstep, never lower). PRIOR: 2026-08-30 (#2555 schema_version poison guard): the `MAX_SCHEMA_VERSION` const + `max_schema_version()` accessor, the bootstrap-SCHEMA CHECK, the poison pre-check in `migrate()`, the v92 rebuild arm + `MIGRATION_V92_SQLITE` const, and the `schema_version_ceiling_ssot_2555` / `fresh_schema_version_is_bounded_2555` unit tests grow this file to MEASURED 6_690. Ceiling 6_640 -> 6_780 (+90 headroom, lockstep, never lower). PRIOR: 2026-08-30 (#3172 schema-masks-data-loss gate): the `lineage_integrity_watermark` bootstrap SCHEMA table grows this file to MEASURED 6_541 (the enforce gate itself lives in db::open/connection.rs + schema_integrity.rs, not here). Ceiling 6_540 -> 6_640 (+99 headroom, lockstep, never lower). PRIOR: 2026-08-25 (#3221 rebase onto 5cb2dc31): MEASURED 6_463. Ceiling 6_400 -> 6_540 (+77 headroom). Never lower a floor. */
