...
const MODULE_SIZE_CEILINGS: &[(&str, usize)] = &[
    ...
    ("src/daemon_runtime.rs", 11_700), /* 2026-07-31 #2490 export/import return an exit code so a PARTIAL export or an incomplete import cannot present as success; the two dispatch arms grow from a bare tail-call to the `match ... { 0 => Ok(()), code => exit(code) }` shape the `Command::Config` precedent already uses, landing the file at 11_647; ceiling 11_640 -> 11_700 (+53 headroom, lockstep). */
