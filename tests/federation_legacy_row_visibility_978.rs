...
fn reset_env() {
    unsafe {
        ...
        // #1789 — these tests pin the #978 legacy-row visibility filter on
        // /sync/since with an unenrolled X-Peer-Id and no signature, hitting
        // the (None,None) arm. v0.8 flipped that arm's secure default to
        // strict (401); opt back to permissive to preserve each test's
        // visibility-filter intent.
