...
use ai_memory::curator::{
    ...
    STALE_RULING_STATE_HASH_KEY, STALE_RULING_STATE_NAMESPACE, STALE_RULING_STATE_TITLE,
    run_stale_ruling_sweep,
};
...
/// A ruling that is not LIVE in the lifecycle sense is never reported, so its
/// id never leaves the store in a digest: tombstoned (retained history),
/// quarantined (#1948, hidden from every read/egress lane) and contaminated
/// (#3324). The scan applies the same fail-closed
/// `lifecycle_visible_clause` allow-list every read lane uses; a visible
/// sibling seeded alongside proves the scan still ran.
#[test]
fn stale_ruling_hidden_lifecycle_rows_are_not_reported_3587() {
    let (_dir, conn) = open_db();
    let visible = seed(&conn, &["ruling"], Some("k-vis"), json!({}), 30);
    let mut hidden = Vec::new();
    for state in ["tombstoned", "quarantined", "contaminated"] {
        let id = seed(&conn, &["ruling"], Some(state), json!({}), 30);
        conn.execute(
    ...
    assert!(
        report.stale_ruling_ids_all.contains(&visible),
        "the visible ruling is stale"
    );
    for id in &hidden {
        assert!(
            !report.stale_ruling_ids_all.contains(id),
            "a lifecycle-hidden ruling must not be reported: {id}"
        );
    }
    let digest = latest_inbox_content(&conn);
    for id in &hidden {
        assert!(
            !digest.contains(id.as_str()),
            "a lifecycle-hidden id must not leave the store in the digest"
        );
    }
}

/// The dedup state row is LONG tier with no expiry, so the set-hash dedup
/// outlives the mid-tier 7 d TTL: an unchanged stale set is never re-digested
/// just because GC reaped the bookkeeping row.
#[test]
fn stale_ruling_state_row_is_long_tier_never_expires_3587() {
    let (_dir, conn) = open_db();
    seed(&conn, &["ruling"], Some("k-tier"), json!({}), 30);

    let r = run_stale_ruling_sweep(&conn, &cfg(Some(RECIPIENT), false), None).expect("sweep");
    assert_eq!(r.stale_rulings_notified, 1, "{:?}", r.errors);

    let (tier, expires): (String, Option<String>) = conn
        .query_row(
            "SELECT tier, expires_at FROM memories WHERE id = ?1",
            [ai_memory::curator::stale_ruling_state_id()],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )
        .expect("state row by deterministic id");
    assert_eq!(tier, "long", "the dedup state row must not expire");
    assert_eq!(expires, None, "a long-tier state row carries no expiry");
}

/// R3 — the sqlite reader addresses the state row by its deterministic id and
/// falls back to `(title, namespace)` only for a row an older build wrote
/// under a random id. Such a legacy row, stamped inside the floor, must still
/// suppress a changed set: the fallback may not re-open the floor.
#[test]
fn stale_ruling_reader_honours_legacy_random_id_state_row_3587() {
    let (_dir, conn) = open_db();
    ...
    let legacy = Memory {
        ...
        metadata: json!({
            "agent_id": "ai:curator",
            STALE_RULING_STATE_HASH_KEY: "legacy-set-hash",
        }),
    ...
    ai_memory::db::insert(&conn, &legacy).expect("legacy state row");
    seed(&conn, &["ruling"], Some("k-legacy"), json!({}), 30);

    let r = run_stale_ruling_sweep(&conn, &cfg(Some(RECIPIENT), false), None).expect("sweep");

    assert_eq!(
        r.stale_rulings_notified, 0,
        "a legacy state row inside the floor must suppress: {:?}",
        r.errors
    );
    assert_eq!(inbox_count(&conn), 0, "no digest inside the legacy floor");
}
