...
fn v77_migration_backfills_preexisting_rows_folded() {
    ...
    // Fresh open reaches the current tip (v79, #1942/#1941/#1945/#1834
    // crypto-core stage 2) with the v77 `folded` column present.
    let conn = db::open(&path).expect("open");
    assert_eq!(db::migrations::current_schema_version_for_tests(), 79);
    let version: i64 = conn
        ...
        .unwrap();
    assert_eq!(version, 79, "fresh open reaches the v79 tip");
    assert!(
