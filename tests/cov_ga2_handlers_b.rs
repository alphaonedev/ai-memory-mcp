...
#[tokio::test]
async fn live_notify_inbox_wire_contract_3401() {
    ...
    let _g = GA2_LOCK
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
    let (sqlite, _file) = sqlite_router();
    ...
    let state = app_state_for(db, store, StorageBackend::Postgres);
...
async fn assert_inbox_contract_3401(sqlite: &axum::Router, postgres_path: &axum::Router) {
    let request = json!({
