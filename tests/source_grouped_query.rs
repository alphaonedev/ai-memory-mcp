...
fn list_by_source_uri_returns_all_five_matches() {
    ...
    let hits = db::list_by_source_uri(&conn, shared, None, None, None, None).expect("list");
    assert_eq!(
        hits.len(),
        5,
...
fn list_by_source_uri_respects_namespace_filter() {
    ...
    let alpha = db::list_by_source_uri(&conn, uri, Some("alpha"), None, None, None).expect("alpha");
    assert_eq!(alpha.len(), 2);
    let beta = db::list_by_source_uri(&conn, uri, Some("beta"), None, None, None).expect("beta");
    assert_eq!(beta.len(), 3);
    let all = db::list_by_source_uri(&conn, uri, None, None, None, None).expect("all");
    assert_eq!(all.len(), 5);
}
...
fn search_with_source_uri_narrows_fts_results_to_matching_uri() {
    ...
    let hits = db::search_with_source_uri(
        ...
        false,
        Some(uri),
        None, // #1720 caller (as_agent None => trust-all)
    )
    .expect("search");
...
fn list_by_source_uri_orders_results_deterministically() {
    ...
    let first = db::list_by_source_uri(&conn, shared, None, None, None, None).expect("list");
    let second = db::list_by_source_uri(&conn, shared, None, None, None, None).expect("list");
    assert_eq!(
...
fn kg_query_by_source_uri_returns_all_rooted_memories() {
    ...
    let roots = db::list_by_source_uri(&conn, uri, None, None, None, None).expect("list");
...
fn empty_uri_returns_zero_rows_not_all_rows() {
    ...
    let hits =
        db::list_by_source_uri(&conn, "doc:does-not-exist", None, None, None, None).expect("list");
