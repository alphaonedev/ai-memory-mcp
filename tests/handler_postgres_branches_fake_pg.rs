...
fn seed_archived_memory_3400(file: &NamedTempFile, id: &str, namespace: &str, tags: &[&str]) {
    seed_memory_3400(
        ...
        namespace,
        tags,
        serde_json::json!({"agent_id": "ops:admin", "scope": "collective"}),
    );
...
async fn archive_list_wire_shape_matches_across_backends_3400() {
    ...
    for backend in [StorageBackend::Sqlite, StorageBackend::Postgres] {
        ...
        seed_archived_memory_3400(&file, "archive-3400", "archive-ns", &["alpha", "beta"]);
...
async fn archive_stats_wire_shape_matches_across_backends_3400() {
    ...
    for backend in [StorageBackend::Sqlite, StorageBackend::Postgres] {
        let (router, file) = build_router_for_backend(backend, vec!["ops:admin".to_string()]);
        seed_archived_memory_3400(&file, "stats-a-1", "stats-a", &[]);
        seed_archived_memory_3400(&file, "stats-a-2", "stats-a", &[]);
        seed_archived_memory_3400(&file, "stats-b-1", "stats-b", &[]);
...
async fn namespace_list_wire_shape_matches_across_backends_3400() {
    ...
    for backend in [StorageBackend::Sqlite, StorageBackend::Postgres] {
        let (router, file) = build_router_for_backend(backend, vec!["ops:admin".to_string()]);
        seed_memory_3400(&file, "ns-a-1", "ns-a", &[], json!({"scope": "collective"}));
        seed_memory_3400(&file, "ns-a-2", "ns-a", &[], json!({"scope": "collective"}));
        seed_memory_3400(&file, "ns-b-1", "ns-b", &[], json!({"scope": "collective"}));
...
async fn namespace_standard_wire_shape_matches_across_backends_3400() {
    ...
    for backend in [StorageBackend::Sqlite, StorageBackend::Postgres] {
        ...
        seed_namespace_standard_3400(&file);
