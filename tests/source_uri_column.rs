...
fn list_by_source_uri_returns_all_matching_rows() {
    ...
    let hits = db::list_by_source_uri(&conn, uri, None, None, None, None).expect("list");
...
fn list_by_source_uri_respects_limit_param() {
    ...
    let three = db::list_by_source_uri(&conn, "doc:lim", None, Some(3), None, None).expect("list");
    assert_eq!(three.len(), 3, "limit=3 caps the response");
    let unlimited = db::list_by_source_uri(&conn, "doc:lim", None, None, None, None).expect("list");
