...
///
/// # Accounting: 8 token-bearing files = 3 + 5
///
/// `git grep -il <token>` returns EIGHT files on this tree.
///
...
///
/// FIVE are this cut's declared widening of the issue's acceptance
/// grep, each listed below with its reason, and each needing the
/// Conductor's explicit ratification: two dated review ledgers, one
/// dated audit record that quotes the operator directive itself, and
/// two published-research citations.
///
...
const HISTORICAL_RECORD_CARVE_OUTS: &[(&str, &str)] = &[
    (
        ...
        "dated Conductor audit record that QUOTES the operator directive \
         verbatim as the evidence of admission; editing the quote destroys \
         the record",
    ),
    (
        "docs/reviews/branch-hygiene-3x7-2026-09-18.md",
        "dated branch-hygiene ledger naming a real git branch; the ledger \
         must keep matching git history",
    ),
    (
        "docs/reviews/branch-hygiene-3x7-2026-09-18.tsv",
        "same ledger, machine-readable twin (branch name + commit subject \
         as recorded in git)",
    ),
    (
