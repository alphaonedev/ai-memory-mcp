...
/// Postgres twin of `stale_ruling_hidden_lifecycle_rows_are_not_reported_3587`:
/// the scan applies the shared fail-closed `lifecycle_visible_clause`, so a
/// tombstoned / quarantined / contaminated ruling is never reported (and so
/// never reaches a digest) while a visible sibling still is.
#[tokio::test(flavor = "multi_thread")]
async fn stale_ruling_hidden_lifecycle_rows_are_not_reported_3587_pg() {
    let Some(store) = connect().await else {
        panic!("AI_MEMORY_TEST_POSTGRES_URL must be set for the #3587 live-pg suite");
    };
    let marker = format!("m3587-{}", uuid::Uuid::new_v4());
    ...
    let visible = raw_ruling(
        &store,
        &ns,
        &["ruling"],
        serde_json::json!({"agent_id": "ai:fable", "ruling_key": "k-vis"}),
        now - chrono::Duration::days(30),
    )
    .await;
    ...
    for state in ["tombstoned", "quarantined", "contaminated"] {
        let id = raw_ruling(
            ...
            &ns,
            &["ruling"],
            serde_json::json!({"agent_id": "ai:fable", "ruling_key": state}),
            now - chrono::Duration::days(30),
        )
        .await;
    ...
    let found = store.list_stale_rulings(&cutoff, 500).await.expect("scan");
    let ids: Vec<&str> = found.iter().map(|r| r.id.as_str()).collect();
    assert!(ids.contains(&visible.as_str()), "visible ruling is stale");
    for id in &hidden {
        assert!(
...
async fn store_backed_stale_ruling_sweep_notifies_and_dedups_3587_pg() {
    ...
    // outlives the mid-tier TTL on postgres too.
    let (tier, expires): (String, Option<chrono::DateTime<chrono::Utc>>) =
        sqlx::query_as("SELECT tier, expires_at FROM memories WHERE id = $1")
            .bind(&state_id)
            .fetch_one(store.pool())
            .await
            .expect("state row tier");
