...
fn f13_summary_and_describe_to_user_agree_on_count_full_profile() {
    ...
    // Both must report "52" for the full profile (substantive memory
    // tools, excluding the always-on `memory_capabilities` bootstrap).
    // v0.7.0 Wave-2 fix B2 (S6-M1) added `memory_reflection_origin` to
    // Family::Power, bumping the substantive total from 51 to 52.
    assert!(
...
fn f13_summary_and_describe_to_user_agree_on_count_core_profile() {
    ...
    // Total memory tools = 52 (53 - bootstrap). v0.7.0 Wave-2 fix B2
    // (S6-M1) added `memory_reflection_origin` to Family::Power,
    // bumping the substantive total from 51 to 52.
    assert!(
