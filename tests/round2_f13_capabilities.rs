...
fn f13_summary_and_describe_to_user_agree_on_count_full_profile() {
    ...
    // Both must report "56" for the full profile (substantive memory
    // tools, excluding the always-on `memory_capabilities` bootstrap).
    // v0.7.0 L1-5 added 5 memory_skill_* tools to Family::Other,
    // bumping the substantive total from 51 to 56.
    assert!(
        summary.contains("56 of 56 memory tools"),
        "summary must report 56 of 56 memory tools; got: {summary}"
    );
    assert!(
        describe.contains("all 56 memory tools"),
        "describe_to_user must report all 56 memory tools; got: {describe}"
    );
}
...
fn f13_summary_and_describe_to_user_agree_on_count_core_profile() {
    ...
    // Total memory tools = 56 (57 - bootstrap). v0.7.0 L1-5 added
    // 5 memory_skill_* tools to Family::Other, bumping the substantive
    // total from 51 to 56.
    assert!(
        summary.contains("7 of 56 memory tools"),
        "summary must report 7 of 56 memory tools; got: {summary}"
    );
    assert!(
