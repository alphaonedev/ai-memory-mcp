...
Schema migrations (sqlite v20 → v54, postgres v15 → v54 logical) run automatically on first start of a sqlite-backed daemon and are idempotent. Postgres schema bootstrap is via `ai-memory schema-init` per [`docs/migration-v0.7.0-postgres.md`](migration-v0.7.0-postgres.md). Canonical anchors: `CURRENT_SCHEMA_VERSION = 54` in both [`src/storage/migrations.rs`](../src/storage/migrations.rs) (sqlite) and [`src/store/postgres.rs`](../src/store/postgres.rs) (postgres); the highest on-disk migration file is [`migrations/sqlite/0045_v53_memories_au_trigger_columns.sql`](../migrations/sqlite/0045_v53_memories_au_trigger_columns.sql) (sqlite splits per-bump, file-name counter ≠ logical version; v54 is an in-code backfill arm with no new .sql file) and the postgres in-process `migrate_v54()` arm (postgres runs a single greenfield+upgrade pair, with v34 → v54 deltas applied through in-process `migrate_v34()` … `migrate_v54()` async functions).
