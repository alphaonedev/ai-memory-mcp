...
1. **Install the binary** (pick one):
   ...
   **From source (requires Rust + C compiler):**
   ...
   (schema v94, 104 MCP tools at `--profile full`). The supported pin
   syntax per channel:
...
Restart your AI client. With `--profile core` (default at v0.7.0) you should see 7 tools available (`memory_store`, `memory_recall`, `memory_search`, `memory_list`, `memory_get`, `memory_load_family`, `memory_smart_load`) plus the always-on `memory_capabilities` bootstrap. With `--profile full` you get all 104 advertised entries (103 callable memory tools + `memory_capabilities`). Other profiles: `graph` (19 tools — Family::Core 7 + Family::Graph 12, the latter grew to 12 with the v0.9.0 addition of `memory_lineage`, #1859), `admin` (21 tools), `power` (56 tools). The full advertised inventory is pinned by the `family_tool_names_cover_registry_all` test in `src/profile.rs` against `mcp::registry::tool_names::ALL` (104 entries) — no literal count is asserted anywhere; the canonical surface list lives in `docs/USER_GUIDE.md`.
