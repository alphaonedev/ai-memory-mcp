...
- **G12 · Durability was reported as a 503, not a subscription · LIFTED v0.8.1 W3 (commit `d1b7fdd3`) — the one carve-out exception, formerly an API-SEMANTICS BUG.** On W-of-N miss the handler **formerly returned** `503 quorum_not_met`; as of v0.8.1 W3 it returns `202 Accepted` carrying the replication state in the body (`under_replicated_response`, `src/handlers/create.rs:667/672/1018`, `memories.rs:568`) — the local row already **persisted** and is never rolled back (ADR-0001, `create.rs:941-966`), so the node is not blocked from writing and the durable write is no longer misreported as a service failure. *Residual:* replication is still not surfaced as an async subscription with a named alarm on un-receipted gaps (push-DLQ exists, `src/federation/push_dlq.rs`).
...
- ⚠️ G12 (v0.8.1 W3, commit `d1b7fdd3`) — **REPORTING LIFTED, CLAIM STILL BANNED.** The 503-misreport BUG is fixed: a durable-but-under-replicated write now returns `202 Accepted` with the replication state in the body, not a `503` (honest durability *reporting* is earned). But the unqualified ❌ **"durability guaranteed" / "quorum-durable writes"** stays BANNED — the fix made the *reporting* honest, not the write quorum-durable (the local row persists; cross-node quorum durability is still best-effort async replication).
