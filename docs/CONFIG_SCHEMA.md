...
| ai-memory postgres schema | **v96** | postgres ladder pinned in lockstep with SQLite `CURRENT_SCHEMA_VERSION = 96` (`src/storage/migrations.rs`). NOTE: the `deploy/docker-1461` / `deploy/do-1461` provisioning configs are reproducibility anchors **pinned to the v0.7.0 release** (`EXPECTED_VERSION=0.7.0`, `EXPECTED_SCHEMA=57`, golden SHA), so their `57` is correct *for that pinned release* — it is not a stale copy of the current tip. A deployment-validation anchor at the current schema would be a separate config. |
