...
# contradiction detection). ONLY `model` is accepted: `backend` /
# `base_url` / `api_key_env` / `api_key_file` were documented but never
# read by anything, and are REFUSED at parse time since v1.0.0 (#3808).
# A genuine second endpoint belongs under [decision] below.
[llm.auto_tag]
model   = "gemma3:4b"
