...
| ai-memory postgres schema | **v71** | postgres ladder pinned in lockstep with SQLite `CURRENT_SCHEMA_VERSION = 71` (`src/storage/migrations.rs`). NOTE: the `deploy/docker-1461/provision/lib.sh` `EXPECTED_SCHEMA` default still reads `57` and is stale relative to the v0.8.1 code tip (`CURRENT_SCHEMA_VERSION = 71`); the deploy-side bump is tracked separately. |
