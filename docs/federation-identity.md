...
| `AI_MEMORY_FED_PEER_FINGERPRINTS` | **v0.8.0 (#1678), unset = pinning off.** Outbound peer SERVER-cert SHA-256 pinning (`known_hosts` model); daemon quorum client fail-closed on an unpinned host. On the `ai-memory sync` CLI an unpinned host falls through to that path's own default, which since #1794 is **CA validation** (this row previously said "accept-any" — that has been false since v0.8.0). See `docs/federation.md`. |
