...
(engaged read decisions are best-effort logged; a failed append is a counted,
permanent gap — there is no DLQ behind it, #3660). Hard-crash loss for
...
| Read actions (`memory_recall` / `memory_search` / `memory_list` / `memory_get` / `memory_session_boot`) | **Governance-evaluable today.** With enabled `read_action` rules, each decision is best-effort chain-logged; the zero-rule fast path emits nothing | `event_type = "governance.check"`, canonical action is `AgentAction::Read { surface, namespace, query }` plus the decision | audit-append failure logs a warning and the read proceeds by intentional split fail-posture; a blocking rule verdict itself remains fail-closed. **The append is direct and unqueued (#3660):** a decision whose `signed_events` INSERT fails is NOT admitted to the deferred-audit spool/DLQ (that mechanism is the write pre-hook's, refusal-only, and not on this path) and is never retried — its only residence is the best-effort forensic file when that sink is enabled, and it cannot be reconstructed from `signed_events`. Every such gap is counted (`ai_memory_governance_read_audit_evidence_gap_total{residence}`, `/health` `governance.read_audit_delivery`); `AI_MEMORY_READ_AUDIT_STRICT=1` refuses the read instead | **#697** V08-PE-2 shipped; **#3660** gap measured + strict policy |
