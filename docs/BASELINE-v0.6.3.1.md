...
> **Forward note (v0.7.0):** the 43-tool baseline below is the historical v0.6.3.1 snapshot. At v0.7.0 the surface advertises **74 entries at `--profile full`** (73 callable memory tools + the always-on `memory_capabilities` bootstrap). Default `--profile core` (introduced at v0.6.4) ships 7 tools at v0.7.0 plus the always-on bootstrap. See `CHANGELOG.md` [Unreleased]/[0.7.0] for the full delta, and [`MIGRATION_v0.7.md`](MIGRATION_v0.7.html) for the upgrade path. The 43 below remain accurate as the v0.6.3.1 baseline this doc was captured at.
