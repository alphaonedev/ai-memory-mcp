...
| #1685 open | **Already fixed** (`mcp/mod.rs:2786`). #1672/#1673/#1674 also closed (v0.7.1). #1175 `source='claude'` fixed v0.7.0. | Verify-and-close; correct ROADMAP §24 doc-drift. |
| ~~#1680 "already fixed"~~ | **CORRECTION (deconfliction audit): #1680 is OPEN — NOT fixed.** Two independent literal `3`s persist: `src/models/namespace.rs:956` `unwrap_or(3)` (governance default) + `src/reranker.rs:369` `DEFAULT_REFLECTION_MAX_DEPTH_CAP=3` (advertised cap). | REMAINS-v0.8.0 (small). Earlier "already fixed" was a verify-before-claiming miss. |
| **Closed-issue ≠ feature-complete** | **#228** (E2E encryption — MVP crypto module only, encrypt-on-insert/decrypt-on-read wire-up REMAINS) · **#1095** (memory_share — send-side only, receiver accept/reject REMAINS) · **#654** (model-attest/distilled — closed as STRATEGY, only test-only `AttestedWeights` MVP with zero callers; chain REMAINS) · **#664** (compaction — closed on minimum-slice trait, 6-stage wiring + verify+rollback REMAINS) · **#224** (share/sync — design+send-side foundation, CRDT-lite merge deferred). | Do NOT treat these closed issues as shipped features. The work is in Phases 1–4. |
...
- Carried-forward hardening (ROADMAP §11.4 deferrals, also via absorbed #1695): **#1670** SqliteStore TRANSACTIONS/ATOMIC_MULTI_WRITE capability bits (parent **#302** closed; #1670 is the live carve-out) · legacy v0.6.x flat-config removal (separate from **#1175**, which is CLOSED/done — the `source='claude'` fix) · **`crate::db`** alias (`pub use storage as db`) removal · **#1095** receiver-side accept/reject share workflow (inbound half — #1095 is *closed* but only the send-side shipped) · **#1680** reflection-depth const (OPEN — two literal-3s persist; NOT fixed) · **#228** E2E content-encryption wire-up (closed as MVP module; encrypt-on-insert/decrypt-on-read remains)
