...
**Companion artefacts:** [`v0.7.0-capabilities.json`](v0.7.0-capabilities.json) inventory · [`../nsa-csi-mcp-security-mapping.md`](../nsa-csi-mcp-security-mapping.html) compliance mapping · [`../honest-limitations.md`](../honest-limitations.html) substrate boundaries.
...
| `mcp_list_tools_returns_expected_count` | `tests/mcp_integration.rs` | Tool count remains 100 at full / 7 at core (SSOT: `Profile::full()/core().expected_tool_count()`) |
...
# Expected: const CURRENT_SCHEMA_VERSION: i64 = 50; (v50 = per-namespace K8 quota dimension, #1156)
...
### 6.3 #1156 per-namespace quota dimension — SHIPPED at schema v50

Tracked at [#1156](https://github.com/alphaonedev/ai-memory-mcp/issues/1156). Delivered:
- Schema v50 migration on both adapters (`migrations/sqlite/0042_v50_per_namespace_quota.sql` + `src/store/postgres.rs::migrate_v50()`)
- `tests/postgres_schema_parity.rs::POSTGRES_CURRENT_VERSION` bumped to 50
- 14 per-namespace quota enforcement tests in `tests/per_namespace_quota.rs`
- Backwards-compat preserved via `_global` sentinel namespace backfill on pre-v50 rows
- `agent_quotas` PRIMARY KEY extended from `(agent_id)` to `(agent_id, namespace)`
- NSA CSI MCP recommendation (c) — defense-in-depth blast-radius controls

NOT a NSA CSI MCP coverage gap; additional defense-in-depth on the 7-layer DoS defense.
