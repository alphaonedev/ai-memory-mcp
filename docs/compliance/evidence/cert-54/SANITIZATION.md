...
The 2026-08-12 per-control `removal-*-{broken,restored}.out` pairs were
removed: the full-map `overall: PASS` log at the current tree is a
strictly stronger artifact recording the same per-control broken→RED /
restored→GREEN verdicts, and retaining pairs from a different tree
alongside it would invite quoting stale evidence as current.

## What was sanitized

| Pattern | Replacement | Files |
|---|---|---|
| Absolute local repo path of the operator checkout | `<repo-root>` | the `AI_MEMORY_FED_PEER_FINGERPRINTS` `actual:` lines in the two hardened/sqlcipher posture legs; cargo `Compiling ai-memory v1.0.0 (…)` banners in `removal-proof-full.log` |
