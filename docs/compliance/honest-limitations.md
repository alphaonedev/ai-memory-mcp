...
6. **Federation transport — what is required, and what is opt-in.** Stated precisely, because an earlier revision of this list asserted the inverse of the shipped control ("the sync daemon refuses to start without mTLS unless an explicit insecure flag is set"). It does not, and no procurement reviewer should size a deployment against that sentence.
   ...
   - **Federation replicates PLAINTEXT memory content.** The at-rest encryption envelope is per-node and is **not** a wire field: federation catch-up decrypts `content` and the receiving peer re-seals under its own per-node key, so a federated peer holds plaintext at apply time (`src/encryption/mod.rs`; end-to-end federation encryption is open as #1968). The wire is TLS/mTLS-protected and enrolled-peer-gated, and inbound writes are namespace-scoped per `PeerScope` — that transport protection plus peer scoping is the **entire** confidentiality story between peers. Tenant isolation in a federated deployment rests on it. There is no end-to-end encryption across federation at v1.0.0, and a peer you federate with can read what you replicate to it. **That includes `scope=private` rows, and private-by-default inbox rows:** EVERY push path replicates private rows to every configured peer without a scope filter: the write-time push (`federation::broadcast_store_quorum_with_embedding`), the bulk catch-up push (`federation::bulk_catchup_push`, a push despite its name), and the sync-daemon cycle (`daemon_runtime::sync_cycle_once`). Only the `/sync/since` PULL lane withholds them (it post-filters through `visibility::is_visible_to_caller`, #948). On the peer the owner predicate is re-applied, so no other agent there can read them, but the peer holds them in plaintext at apply time and under its own at-rest custody (plaintext at rest unless that peer runs at-rest encryption, then sealed to the peer's key, not yours). The peer's operator is therefore inside your residency boundary for private rows too; exclude a namespace from federation scope if that is not acceptable (#3928; end-to-end federation encryption is #1968).
...
### 5.1 Swarm containment is node-local at GA (#3266 item 3, R2.6)

A `memory_swarm_rewind` (and the `Contaminated` auto-stamp it builds on) contains a cascade **on the node that runs it, and only there**. At v1.0.0 there is no rewind-intent wire lane: **a federation peer does not inherit a rewind — each node rewinds its own copy.** What GA does guarantee is that a taint, once applied, is not undone or spread by replication: a local `contaminated` / `quarantined` row is never cleared by a peer's newer write (the lifecycle merge is LOCAL-wins for those two states on both backends), a peer can never taint a row on this node (a wire `contaminated` / `quarantined` lifecycle lands `open` and a wire `metadata.contamination` marker is dropped at every receive funnel), and the tainted rows themselves are never shipped (the send lanes filter hidden rows). Reversal is a deliberate, audited act: the operator release (`ai-memory quarantine release <id>` / `POST /api/v1/admin/quarantine/{id}/release`) decontaminates a `contaminated` row back to its recorded prior visible state (else `open`) and appends a signed `swarm.decontaminate` event. A lifecycle `tombstoned` is NOT node-local — it is a replicated deletion and still converges fleet-wide. Cross-node propagation of a rewind is post-GA roadmap (Boids items 4-6).
