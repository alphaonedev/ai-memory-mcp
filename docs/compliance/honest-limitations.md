...
5. **Encryption at rest — two separate controls, and the first needs no special build.** (a) `AI_MEMORY_ENCRYPT_AT_REST=1` (or `[encryption].at_rest = true`) seals every memory's `content` into a per-record ChaCha20-Poly1305 envelope keyed under the per-agent X25519 key, on the **default build** and on **both backends** (title / tags / metadata stay plaintext; keys under the key dir are minted on first use with no rotation or escrow at v1.0.0, #3717 — a lost key makes its rows unrecoverable). (b) `--features sqlcipher` adds whole-database page encryption keyed by the passphrase channel (AES-256, mode-0400 passphrase-file enforcement, #1055). An earlier revision of this line named only (b), which told a buyer on a standard build that at-rest encryption was unavailable to them; it is available, via (a) (#3720).
