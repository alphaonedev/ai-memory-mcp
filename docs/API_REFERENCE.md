...
| `federation.nonce_cache` | the federation replay-nonce cache's measured snapshot (#3662), as a signal object: `state: "available"`, `observed_at_seconds`, and `value` carrying occupancy (`peers` / `max_peers`, `fingerprints` / `per_peer_capacity`), the since-boot counters (`peer_evictions_total`, `fingerprint_evictions_total`, `replay_refusals_total`) and `persistence` — `state` (`memory_only` \| `durable` \| `degraded`), `restart_protection` (`disabled` \| `durable` \| `lost`), `reason`, `failures_by_op`, `last_success_at_seconds` (`null` until a write succeeds — never invented), `degraded_for_seconds` and `actionable` (`true` once protection has been `lost` for 300 s). No peer id or path is ever included. Does not affect the HTTP code. |
...
| `ai_memory_federation_nonce_cache_peers`, `..._fingerprints`, `..._peer_capacity`, `..._per_peer_capacity` | gauge | Replay-nonce cache occupancy against its two ceilings (#3662). Written by the cache from its own counters after every check — never estimated. |
| `ai_memory_federation_nonce_cache_peer_evictions_total`, `..._fingerprint_evictions_total`, `..._replay_refused_total` | counter | Outer-LRU peer-slot evictions, per-peer FIFO evictions and refused nonce replays since boot (#3662). One peer cycling its FIFO is the replay-window flush shape; sustained peer evictions mean churn past the ceiling. |
| `ai_memory_federation_nonce_cache_persistence_failed_total{op}` | counter | Sqlite-mirror operations that failed since boot, `op` ∈ `open` \| `insert` \| `delete_fingerprint` \| `delete_peer` \| `hydrate_prune` (closed set, pre-touched to `0` at boot). Any increment means the mirror missed a write the in-memory cache applied. |
| `ai_memory_federation_nonce_cache_persistence_state`, `..._last_persisted_at_seconds` | gauge | `0` = memory-only (never configured), `1` = durable, `2` = degraded (the last mirror op failed, or the mirror could not be opened at boot); and the unix second of the last fully successful mirror write — a series that is ABSENT until the first successful write since boot (a moment in time is never `0`; `/health` renders `null` for the same fact). **Page on `2` sustained for 300 s** — every daemon restart then re-opens the replay window for every peer. |
