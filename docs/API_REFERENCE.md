...
| `POST` | `/api/v1/admin/quarantine/{id}/release` | v1.0.0 [#2402](https://github.com/alphaonedev/ai-memory-mcp/issues/2402) — release one quarantined memory back to `lifecycle_state=open` (`handlers::release_quarantined`), the operator half of the [#1948](https://github.com/alphaonedev/ai-memory-mcp/issues/1948) route-OUT contract that shipped with no caller. **Admin-gated**; the audit actor is the principal `require_admin` RETURNS — an id it admits only when it is on the admin allowlist AND the deployment has request authentication configured (#1570), and, under the `enforce` identity-binding posture, only when it is key-attested to a per-agent api key (#2044). The handler never reads `X-Agent-Id` itself. Appends a `memory.dequarantined` signed audit row in the SAME transaction as the state change on both backends. Boids item 3 R2.5 ([#3266](https://github.com/alphaonedev/ai-memory-mcp/issues/3266)): the SAME route also releases a `contaminated` row — chosen by the state observed under the row lock, never by the caller — restoring the recorded `metadata.contamination.prior_lifecycle_state` when it is a visible state (else `open`), removing the marker, and appending a signed `swarm.decontaminate` event (payload binds the released prior / `contaminated_from` / `stamped_at`) instead of `memory.dequarantined`; the response shape is unchanged. Idempotent: an id that is neither quarantined nor contaminated answers `200 {"released": false}` and writes nothing (deliberately not `404` — that would leak the existence of rows this surface does not return). |
| `POST` | `/api/v1/memory_swarm_rewind` | v1.0.0 Boids item 3 ([#3266](https://github.com/alphaonedev/ai-memory-mcp/issues/3266), 5-agent vote `4d3ea1c5`) — **admin-only** cascade rewind, served on BOTH backends (the MCP tool is stdio/SQLite-only and the CLI refuses a Postgres store, so this is the Postgres path). Body `{to, max_depth?, freeze_routines?, dry_run?}`; `to` is a memory id (in any lifecycle state) or a checkpoint id naming a root; `max_depth` is clamped to the lineage ceiling. Contaminates the root and its bounded `derives_from` descendants, freezes the named routines, and appends ONE signed `swarm.rewind` event in one transaction; idempotent (a re-run reports `already_rewound`, no second event); `dry_run` writes nothing. `issued_by` is the server-resolved admin principal, never `X-Agent-Id` (non-admin → `403`). Record-stopped → `503 RECORD_STOPPED`. Envelope is byte-equal to the MCP tool's. Containment is NODE-LOCAL at GA: no peer fan-out. |
...
`tools/list` (MCP JSON-RPC and this HTTP mirror) ships a compacted gist per tool so the payload stays inside the C5 token budget. Full per-tool docs and schemas are not duplicated onto each label: call the always-on `memory_capabilities` bootstrap with `{ "verbose": true }` (add `family` + `include_schema` to drill one family). The compact `memory_capabilities` description on the wire is itself that pointer (`Discover runtime capabilities; full per-tool docs: verbose=true`).
...
> **Total HTTP surface at v1.0.0: 89 unique URL paths across 103
...
> `src/lib.rs` as `EXPECTED_PRODUCTION_UNIQUE_PATHS_COUNT = 89` and
> `EXPECTED_PRODUCTION_ROUTES_COUNT = 103`, asserted by
...
> ```
>
> That count is `EXPECTED_PRODUCTION_UNIQUE_PATHS_COUNT` (89): the
...
| `POST` | `/api/v1/agents/{id}/api-key` | MINT or BIND a per-agent HTTP api-key (`handlers::agent_api_key::mint_agent_api_key`, [#3474](https://github.com/alphaonedev/ai-memory-mcp/issues/3474)). **Admin-gated** through the canonical `require_admin` gate; a non-admin gets the same generic `403 {"error":"admin role required"}` whether or not the agent exists. Body `{}` MINTS a fresh 256-bit token and returns it **exactly once** as `token`; `{"token": "<operator-supplied>"}` BINDS that token's digest and echoes nothing back — a supplied token must be at least 32 bytes (`MIN_SUPPLIED_TOKEN_BYTES`, the same width the mint form draws; 16 is the floor it may never be lowered past), and a shorter one is a fixed `400 {"error":"token too short","min_token_bytes":32}` that echoes nothing of the candidate and binds nothing. Only `sha256(token)` is ever persisted (the #2044 contract) and only its 12-hex-character `key_fingerprint` is ever logged or audited. The response is `Cache-Control: no-store`, and the route is REFUSED unless the daemon's listener is loopback or TLS-terminated in-process — a minted bearer secret must not cross a cleartext off-host hop that `AI_MEMORY_ALLOW_PLAINTEXT_NONLOOPBACK` merely acknowledges. **The TARGET must be a REGISTERED agent** ([#3535](https://github.com/alphaonedev/ai-memory-mcp/issues/3535)): an id the `_agents` roster does not know is refused with `404 {"error":"agent_not_registered"}` and nothing is bound, because a key bound to an id nothing registered is a credential for a principal that does not exist — every other identity control (the governance `registered` level, the approver-eligibility gate, `agents bind-key`) consults that roster, and a typo'd `{id}` would otherwise mint a live bearer secret none of them can see. Register the target first with `POST /api/v1/agents`, then enrol its key; the refusal is reachable only after `require_admin` admits, so it is no oracle beyond what that gate already grants, and the CLI twin is deliberately NOT gated this way (it is a shell on the data tier, the bootstrap and recovery path). **A supplied token whose digest is already enrolled to a DIFFERENT agent is refused** with `409 {"error":"api_key_already_bound"}` and nothing changes (#3535): `agent_api_keys` is keyed by `sha256(token)`, so the previous upsert silently RE-POINTED a live bearer credential at another principal; the refusal names no incumbent, and re-binding the SAME `(agent, token)` pair remains an idempotent success that does not move the recorded `bound_at`. Rate-limited per admitted caller (10 mints / 60 s; the N+1th is `429 rate_limited`) — the window is FIXED, not sliding, so a caller straddling a boundary can issue up to **2N (twenty) mints across two adjacent seconds** (#3535); that is burst-shaping, not a bypass of the cap, and a sliding window is deliberately not used. **When the identity namespace's governance policy parks a MINT, the token is returned to the APPROVER, not to the requester** (#3535): the `202` carries a `pending_id` and NO token, because a token cannot be produced at queue time and delivered later without persisting it in raw form — the mint is deferred to the apply step, so the token appears exactly once, in the `200` the approving principal receives. (The queued BIND form has no such wrinkle: its payload already carries the digest of a token the operator holds.) Effective through the #3418 live registry with NO daemon restart. A malformed `{id}` is refused with a `400` on SHAPE alone BEFORE any audit row is written ([#3530](https://github.com/alphaonedev/ai-memory-mcp/issues/3530)), so a never-validated path segment cannot reach the signed chain; the `400` depends on neither target existence nor transport posture. **Behind a TLS-terminating reverse proxy this route refuses by design** — see [ADMIN_GUIDE "Minting an api-key behind a TLS-terminating reverse proxy"](ADMIN_GUIDE.md#minting-an-api-key-behind-a-tls-terminating-reverse-proxy-3474--3531) for the supported CLI / loopback / native-TLS paths; there is no trust-the-proxy header. CLI twin: `ai-memory agents bind-api-key`. |
| `POST` | `/api/v1/agents/{id}/api-key/revoke` | REVOKE every api-key bound to the agent (`handlers::agent_api_key::revoke_agent_api_key`, [#3474](https://github.com/alphaonedev/ai-memory-mcp/issues/3474)). **Admin-gated.** Revoking your OWN key is IMMEDIATE — a principal that believes its credential is compromised must never be told to find a second operator first. Revoking ANOTHER principal's key, or the LAST enrolled key on the deployment (which would make the identity gate inert in every mode, [#1985](https://github.com/alphaonedev/ai-memory-mcp/issues/1985)), returns `202` with a `pending_id`: a DIFFERENT registered approver must re-POST `{"approve_pending_id": "<id>"}` with the K10 `X-AI-Memory-Signature` / `X-AI-Memory-Timestamp` headers, signed over `<ts>.POST.<pending_id>@<approver-agent-id>.<body>` — the approver is INSIDE the signed subject (unlike `POST /api/v1/pending/{id}/approve`, whose canonical names only the row), so a captured signature cannot be presented by a different principal. Self-approval is refused, and one approval applies exactly once: a decided row is `409`, never a re-application. The last-key rule is enforced ATOMICALLY in the store — the count and the delete run in ONE transaction ([#3529](https://github.com/alphaonedev/ai-memory-mcp/issues/3529)), so two concurrent self-revokes by the last two key-holders can never both apply; the loser is parked with `reason: "last_enrolled_key"` instead of emptying the registry. Every `202` carries `empties_registry`, so an approver is told whether approving leaves the deployment with NO enrolled keys, and an approval that did NOT disclose that is re-checked at APPLY time: if the target has since become the last key the apply is refused with `409 {"error":"revoke_would_empty_registry"}`, nothing is revoked, and the refusal is audited. Unlike the mint/bind form, revoke is deliberately NOT gated on the target being a registered agent ([#3535](https://github.com/alphaonedev/ai-memory-mcp/issues/3535)): refusing a revocation is strictly worse than performing one, and revoking a key bound to an unregistered id is exactly the cleanup an operator needs for a binding that predates that rule. CLI twin: `ai-memory agents revoke-api-key`. |
