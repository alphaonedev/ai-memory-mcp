...
| `ai_memory_federation_nonce_cache_persistence_state`, `..._last_persisted_at_seconds` | gauge | `0` = memory-only (never configured), `1` = durable, `2` = degraded (the last mirror op failed, or the mirror could not be opened at boot); and the unix second of the last fully successful mirror write (`0` = none since boot). **Page on `2` sustained for 300 s** — every daemon restart then re-opens the replay window for every peer. |
