...
- `content_attestation` — **v1.0.0 ([#3548](https://github.com/alphaonedev/ai-memory-mcp/issues/3548))** — the row's OWN write attestation, read verbatim from the `metadata.attest_level` stamp the write path sets: `claimed` (unsigned write), `agent_attested` (a verified per-write agent signature), and `operator_signed` / `loader_observed` on the governance / model paths. This is the MEMORY-WRITE vocabulary (`crate::identity::verify::AttestLevel`), a DIFFERENT concept and a DIFFERENT value set from `link_attestation`'s edge vocabulary — the two are deliberately not interchangeable. Absent stamp ⇒ `claimed` (the write path's insert-if-absent default). A property of the memory itself.
