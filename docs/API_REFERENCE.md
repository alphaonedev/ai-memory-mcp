...
| `ai_memory_vector_index_insert_rejected_total{cause}`, `ai_memory_vector_index_failure_total{kind}` | counter | Vector-index inserts refused, by cause (#3665): `invalid_dim` and `empty_embedding` are caller/embedder DATA (logged at WARN; the index is healthy and unchanged), `capacity` is ACTIONABLE (logged at ERROR, hard-fail-at-cap). A sustained burst of the invalid causes (10 in 60 s) emits one ERROR naming embedder dimension drift; alert on `rate(...{cause="invalid_dim"})` rather than on the per-item lines. `failure_total{kind}` (`extension_load` \| `backend_degraded`) counts index-loss events. Closed label sets, pre-touched to `0`. |
