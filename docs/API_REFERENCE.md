...
| `governance.read_audit_delivery` | read-action audit evidence for this process (#3660) as a signal object: `state: "available"`, `observed_at_seconds`, and `value` with `scope` (`mcp_sqlite_read_gate` — HTTP reads are not gated), `policy` (`best_effort` \| `strict`), `evaluated_total`, `chain_appended_total`, `evidence_gap_by_residence` (`forensic_queued` — accepted onto the best-effort forensic sink's writer queue, not a confirmed write — / `none`), `evidence_gap_total`, `strict_refusals_total`, `last_chain_append_at_seconds` / `last_gap_at_seconds` (`null` = none since boot), `gap_open` and `actionable` (`true` once any engaged read decision has been lost from `signed_events` — there is no DLQ behind that append). Does not affect the HTTP code. |
...
| `ai_memory_governance_read_audit_evaluated_total`, `..._chain_appended_total`, `..._evidence_gap_total{residence}`, `..._strict_refusals_total`, `..._last_gap_at_seconds` | counter ×4, gauge | Read-action audit delivery (#3660, MCP sqlite read gate): engaged decisions evaluated, rows that reached `signed_events`, and rows that did NOT by residence (`forensic_queued` — a writer-queue accept, not a delivery — \| `none`; closed set, pre-touched to `0`). `..._last_gap_at_seconds` is ABSENT until the first gap since boot (a moment in time is never `0`). **Any `evidence_gap_total` increment is a permanent gap** — the read-gate append has no DLQ or retry. `strict_refusals_total` counts reads refused under `AI_MEMORY_READ_AUDIT_STRICT` (shared `1`/`true`/`yes`/`on` grammar). |
