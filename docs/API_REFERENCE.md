...
`tools/list` (MCP JSON-RPC and this HTTP mirror) ships a compacted gist per tool so the payload stays inside the C5 token budget. Full per-tool docs and schemas are not duplicated onto each label: call the always-on `memory_capabilities` bootstrap with `{ "verbose": true }` (add `family` + `include_schema` to drill one family). The compact `memory_capabilities` description on the wire is itself that pointer (`Full per-tool docs: verbose=true`).
