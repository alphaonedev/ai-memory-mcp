...
| `GET`  | `/api/v1/inbox` | Read the calling agent's inbox. MCP: `memory_inbox`; MCP requires a valid `AI_MEMORY_AGENT_ID` by default and refuses startup when the variable is empty or malformed. An isolated single-tenant process may explicitly set `[mcp] single_tenant_trust_all = true` to restore caller-unbound inbox selection; every use emits a WARN. |
