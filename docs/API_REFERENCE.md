...
- `content_attestation` — **v1.0.0 ([#3548](https://github.com/alphaonedev/ai-memory-mcp/issues/3548))** — the row's OWN write attestation, composed from `confidence_source` + the row's `metadata.attest_level` stamp: `signed_peer` > `curator_derived` > `self_signed` > `unsigned_caller`. A property of the memory itself.
- `link_attestation` — **v1.0.0 ([#3548](https://github.com/alphaonedev/ai-memory-mcp/issues/3548))** — the EDGE-derived attestation, composed from `confidence_source` + the strongest incident LINK attestation. A property of how the memory was reached, not of the memory. Same vocabulary as `content_attestation`.
