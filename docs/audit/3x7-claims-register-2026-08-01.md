...
>
> **[TRUTH-FIX 2026-08-09, Phase-2 pg-parity remediation]** The "56" here is an ARM COUNT, not a path count (see E-01), and E-01's later "73 of 94" was ALSO wrong (it counted `.route(...)` registrations, not unique paths). The AUTHORITATIVE figure, pinned by `tests/pg_supported_route_inventory_gate_2799.rs`: **59 of the 80 unique production HTTP paths are pg-supported; 21 are fully fail-closed 501.** The core finding stands — the API is a documented subset and MCP-stdio is SQLite-only — only the count is corrected.
...
- **C-35** — replace "one identical API" with the truth: **59 of the 80 unique HTTP paths** are Postgres-supported (`postgres_gate.rs`), the remaining **21 are fully fail-closed 501** (pinned by `tests/pg_supported_route_inventory_gate_2799.rs`), and **MCP stdio is sqlite-only**. This is a real limitation with a real workaround (HTTP); hiding it converts a supportable constraint into a production surprise. *(TRUTH-FIX 2026-08-09: the original "56 of 94 routes" here — and E-01's "73 of 94" correction — were both wrong units; 59/80 unique paths is the Phase-1-verified figure.)*
...
> **Erratum 2026-08-09 (#2780).** The `27 HookEvent variants` figure above was correct at this audit's HEAD `2f32dde1` (2026-08-01) and has since moved: #2637 removed the never-fired `pre_archive` gate (27 → 26) and #2758 removed `pre_recall` / `pre_search` / `pre_transcript_store` / `post_transcript_store` (26 → 22), both on 2026-08-07. `HOOK_EVENTS_COUNT` is **22** at `release/v1.0.0` HEAD. The register text is left as authored — it is a dated snapshot, not a live surface — and `docs/compliance/nsa-csi-mcp-security-mapping.md`, which IS live, was reconciled to 22 by #2780.
...
- **Every compile-anchored count and enumeration checked was exact:** `hook_events_count = 27` (exact at this audit's HEAD; **22** at `release/v1.0.0` HEAD after #2637 + #2758 — see the Erratum 2026-08-09 above), `bypass_impossibility_tests = 6`, the four `enforced_actions` wire tags (routed through the #1558 SSOT so they cannot drift in spelling), the 7 `memory_skill_*` tool names, `max_default = 3` (single-sourced post-#1680), the confidence half-life const, the three forensic CLI verbs.
...
|---|---|---|---|
| **E-01** | **C-35**: "`postgres_endpoint_supported()` is an explicit allowlist — **56** `=> true` arms out of 94 route registrations" | **56 is an ARM COUNT, not a ROUTE count.** One match arm can cover several routes and several arms cover none. ~~The real supported-route figure is **73 of 94**.~~ **[TRUTH-FIX 2026-08-09: "73 of 94" was ALSO wrong — it counted `.route(...)` registrations. The Phase-1-verified figure, pinned by `tests/pg_supported_route_inventory_gate_2799.rs`, is 59 of the 80 unique HTTP paths supported / 21 fully-501.]** The register's own framing conflated two different units | README lane (#2654) |
