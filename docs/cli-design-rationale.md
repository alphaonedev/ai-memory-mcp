...
- Verification report at `release/v0.7.0` HEAD confirmed the CLI inventory (79 subcommands in the default build / 81 under `--features sal` or `--features sal-postgres`; `Reflect` and 15 other parity variants subsequently landed via the FX-C3 batch2 work; #1598 later added `Reembed`, bringing the live counts to 80 / 82) and recommended Hold Position.
