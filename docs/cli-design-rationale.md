...
- **`ai-memory curator --reflect`** — fires a curator pass. The curator uses `models.llm` (the configured second LLM, decorrelated from the producing LLM by deployment discipline) to reflect on memories in the namespace. This is the bias-displaced path.
  ...
  - **Backend behavioral difference — SQLite vs Postgres curator (#1875).** On a **SQLite** deployment the curator runs the full conn-bound upkeep — the reflection/decorrelation sweep **plus** the `auto_tag`, contradiction-sibling, autonomy, persona, size-GC, and self-report passes. On a **Postgres**-backed curator (`--store-url postgres://…`) the sweep runs the SAL reflection/decorrelation upkeep (the curator's core job) but currently **skips** the six conn-bound extras, which are not yet ported to the SAL trait (`src/cli/curator.rs` admission comment). This is a documented, tracked parity gap ([#1875](https://github.com/alphaonedev/ai-memory-mcp/issues/1875)), deferred past the v1.0.0 freeze as a post-GA parity sprint (coupled with the `MemoryStore` decomposition, [#1802](https://github.com/alphaonedev/ai-memory-mcp/issues/1802)); it is not a correctness defect (the extras are best-effort maintenance passes, and byte-pressure size-GC is inert unless a corpus cap is configured).
...
- Verification report at `release/v0.7.0` HEAD confirmed the CLI inventory (79 subcommands in the default build / 81 under `--features sal` or `--features sal-postgres`; `Reflect` and 15 other parity variants subsequently landed via the FX-C3 batch2 work; #1598 later added `Reembed` → 80 / 82; v0.8.0 then added `VerifyAuditTrail` (#697 PE-8) and `Reown` (#1720 B2) → 82 / 84 and `UndoEdit` (#1727) → 83 / 85. The counts have since grown further to **89 / 91** (v1.0.0 #1978 added `Watch`, the L3 substrate poll-based filesystem-watcher capture daemon), the current live counts pinned by `ai_memory::EXPECTED_CLI_SUBCOMMANDS_DEFAULT` / `_SAL`) and recommended Hold Position.
...
- See also: [`docs/strategy/moonshot-synthesis.md`](strategy/moonshot-synthesis.html) §2.6 (bias-displacement through architectural separation-of-powers); [`ROADMAP.md`](../ROADMAP.md) §2.6 (the seven properties that remain load-bearing through ASI).
