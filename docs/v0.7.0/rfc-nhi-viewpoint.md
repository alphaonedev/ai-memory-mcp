...
| Gap #3 — Adoption binding constraint | [#804](https://github.com/alphaonedev/ai-memory-mcp/issues/804) | OPEN — TRACKED-FOR-V080 (distribution channels: homebrew / COPR / AUR / MCP-client defaults). Substrate-external GTM work; landing channels milestoned to v0.8.0 per the substrate's GTM lane gating. Acceptance: PKGBUILD + Homebrew formula + COPR spec + Claude/ChatGPT/Grok MCP-client defaults verified end-to-end. | v0.8.0 milestone |
| Gap #4 — Autonomous-tier latency tax | [#805](https://github.com/alphaonedev/ai-memory-mcp/issues/805) | OPEN — TRACKED-FOR-V080 (cross-refs [#654](https://github.com/alphaonedev/ai-memory-mcp/issues/654) distilled hot-path). Acceptance: median write-path latency at autonomous tier ≤ 25 ms p50 + ≤ 100 ms p99 under the Batman-active load profile from `docs/batman-active-mode.md`. Unblocks when [#654](https://github.com/alphaonedev/ai-memory-mcp/issues/654) lands the distilled hot-path. | v0.8.0 milestone |
| Gap #5 — Federation / quotas / A2A at population scale | [#806](https://github.com/alphaonedev/ai-memory-mcp/issues/806) | OPEN — TRACKED-FOR-V080 (multi-NHI integration test suite scale-out). Acceptance: N=100 agents × M=50 ops each end-to-end on the lan-parity docker harness with zero quota-bypass + zero federation-fanout failure beyond the documented retry+DLQ budget. | v0.8.0 milestone |
