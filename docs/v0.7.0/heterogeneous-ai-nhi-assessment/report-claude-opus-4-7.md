...
**P1. Discovery & loaders.** `memory_capabilities {accept: "v3"}` returned the full v3 envelope. Live state at probe entry: `tier: "autonomous"`, `permissions.mode: "enforce"`, `permissions.active_rules: 2`, `hooks.hook_events_count: 25`, `hnsw.evictions_total: 0`. `memory_smart_load {intent: "investigate a contradiction across past reflections"}` correctly routed to `chosen_family: "power"` (where `memory_detect_contradiction` lives), `chosen_family_source: "keyword"` (B3 family embeddings unloaded → fell back to keyword routing). `memory_load_family {family: "graph"}` returned empty (fresh namespace). `--profile core` set (`memory_store|recall|search|list|get|load_family|smart_load`) is exactly the right always-on set — five primitives plus two discovery loaders for everything else. **End-to-end fix observability confirmed across a daemon restart:** the original Phase-1 probe observed the live MCP daemon reporting `models.llm = "gemma4:e4b"` (the pre-#1168 tier-preset behavior) immediately after the fix landed on disk; the post-restart re-probe in this refinement pass returns `models.llm = "xai:grok-4.3"` (the resolver-routed value) verbatim against the operator's `[llm] backend = "xai", model = "grok-4.3"` config. The resolver-routing fix lands end-to-end across the restart boundary — substrate observability of operator-resolved model identity is the contract `memory_capabilities` now upholds without lag.
...
3. **The `memory_recall` parameter-name drift (`query` vs `context`).** The capabilities tool docstring shows `{"query": "..."}` as a call example; the live API requires `{"context": "..."}`. My first recall call returned `"context is required"`. This is docstring drift in the running daemon. Did not file as separate issue; flagging for cross-evaluator visibility.

4. **`memory_replay` returns empty for the reflection chain because transcripts are planned-not-enabled.** Capabilities: `transcripts.enabled: false, planned: true, version: "v0.7+"`. The reflection_depth chain IS persisted (verified via SQL); the transcript-union projection across the chain is the planned upgrade. **Not a defect; a known-planned gap.**

5. **Curator-LLM-dependent primitives blocked by xAI billing.** `memory_persona_generate`, `memory_consolidate`, `memory_atomise` (curator path), `memory_ingest_multistep` all delegate synthesis to the resolved LLM (`xai:grok-4.3` per operator config). The xAI team's billing is 403'd. Three probes (P9 partial, P10 full, sections of P12) blocked operationally — NOT a substrate defect; an operator-billing condition. Code-anchor verification stands.

6. **ADR-0001 quorum replication documented but not implemented.** Federation is best-effort eventual-consistency. For a hive of agents requiring strong consistency on shared decisions, the substrate cannot guarantee convergence in a bounded window. Operators building hive-pattern systems need to know this.

> **Refinement note (2026-05-24).** The original Phase-1 report carried two additional live caveats here: the `memory_reflect` metadata-passthrough defect (#1172) and the `models.embedding_dim` resolver gap (#1169). Both have **fixes filed against `release/v0.7.0` and pending operator merge** as of this revision — PR #1177 lands a 5-invariant regression suite pinning the correct `memory_reflect` passthrough behavior end-to-end (live MCP probe in the closure-comment evidence); PR #1178 routes `embedding_dim` through a new resolver-side canonical lookup table and additionally closes the parallel postgres-bootstrap drift site at `src/daemon_runtime.rs:3051-3059` that the Block 2 QC audit surfaced. The PRs await CI green + operator `--admin` merge; once merged, the corresponding issues will close with the retest evidence already prepared in the PR descriptions. The caveats below are reduced from the original Phase-1 verdict in anticipation of those closures — explicitly flagging here that the verdict adjustment is conditional on the PRs landing. Three adjacent defects discovered during the same refinement pass are tracked as open follow-ups: #1174 (codegraph-driven pm-v3.1 global refactor sweep — `local`/`global`/`const` discipline), #1175 (MCP `handle_reflect` hardcodes `source: "claude"` — vendor-monoculture in heterogeneous-NHI substrate), and #1176 (MCP approval-gate `pending_action` payload drops `metadata` — sibling defect to #1172 via a different code path). The Phase-1 verdict is preserved (`SHIP-WITH-CAVEATS`); the caveat list is shorter post-refinement (and shorter still once #1177 + #1178 land).
...
**SHIP-WITH-CAVEATS** — v0.7.0 is the minimum viable cognitive-checks-and-balances architecture for an AI NHI to be trustably-itself-across-time and trustably-stoppable-without-corruption; the remaining caveats (post-refinement, **assuming PRs #1177 and #1178 land**, which close #1172 and #1169 respectively pending operator `--admin` merge) are the honest limitations on intra-session hallucination and ADR-0001 quorum non-implementation, plus three substrate defects discovered during refinement and tracked for in-release closure: #1174 (pm-v3.1 variables/constants discipline refactor), #1175 (vendor-monoculture in `handle_reflect` `source` stamp), and #1176 (approval-gate metadata drop sibling to the (PR-pending-closure) #1172).
...
### Filed during the original Phase-1 probe (2026-05-24 morning)

- [#1172](https://github.com/alphaonedev/ai-memory-mcp/issues/1172) — `memory_reflect` MCP tool strips caller-supplied `metadata.entity_id` (medium severity). **Closure PR [#1177](https://github.com/alphaonedev/ai-memory-mcp/pull/1177) filed, pending operator `--admin` merge** — a 5-invariant regression suite pinning the correct passthrough behavior end-to-end (substrate + MCP wire layer + entity-binding discoverability). Live MCP probe in the PR description confirms `metadata.entity_id` round-trips through the JSON-RPC boundary into both the metadata column AND the indexed `mentioned_entity_id` column without the `[entity:X]` title-marker workaround. Issue closes once the PR merges + the close-comment with retest evidence lands.
- [#1169](https://github.com/alphaonedev/ai-memory-mcp/issues/1169) — `memory_capabilities.models.embedding_dim` sources from tier preset (low severity, filed earlier in session). **Closure PR [#1178](https://github.com/alphaonedev/ai-memory-mcp/pull/1178) filed, pending operator `--admin` merge** — adds `KNOWN_EMBEDDING_DIMS` canonical lookup table + `canonical_embedding_dim` helper, threads `embedding_dim: Option<u32>` through `ResolvedEmbeddings`, and additionally closes the parallel postgres-bootstrap drift site at `src/daemon_runtime.rs:3051-3059` that QC surfaced during the audit (worse than the capabilities-surface drift — on-disk pgvector column shape mismatch). 13-invariant regression suite covers known/unknown/tier-disabled/legacy-alias paths. Issue closes once the PR merges + the close-comment with retest evidence lands.

### Filed during the refinement pass (2026-05-24 afternoon)

- [#1174](https://github.com/alphaonedev/ai-memory-mcp/issues/1174) — Codegraph-driven global refactor sweep for `local`/`global`/`const` discipline (pm-v3.1 prime-directive addendum). Phase-1 inventory subagent has already audited 266 Rust source files + 233 integration tests and produced a 98-finding (37 MUST + 54 SHOULD + 7 NICE) report under `.local-runs/refactor-pmv31-audit-20260524-120814/INVENTORY.md`. Phase 2 (triage) and Phase 3 (per-module execution) are tracked under the same issue.
- [#1175](https://github.com/alphaonedev/ai-memory-mcp/issues/1175) — `src/mcp/tools/reflect.rs:103` unconditionally stamps `source: "claude"` on every reflection regardless of which AI NHI made the call. The substrate is heterogeneous-NHI by design; this hardcoding is the same monoculture defect surfaced at the validator enum (`src/validate.rs:27` also lists `"claude"` as a role-categorical value alongside `user`/`hook`/`api`/`system`). Filed alongside the proposed Option-A fix: replace with neutral `"nhi"`, keep vendor identification in `metadata.agent_id`.
- [#1176](https://github.com/alphaonedev/ai-memory-mcp/issues/1176) — MCP approval-gate `pending_action` payload at `src/mcp/tools/reflect.rs:152-163` drops `metadata` from the serialised input. Sibling defect to #1172 via a different code path: an L1-8 governance-approval-gated reflection loses caller-supplied keys (e.g. `entity_id`) on the pending → execute round-trip. Discovered by the Block 1 QC subagent during audit of the #1172 regression suite.

### Refinement-pass provenance

- Phase-1 verdict re-issued without change to the `SHIP-WITH-CAVEATS` line itself; the caveat list shortened by two (closure PRs #1177 + #1178 filed for #1172 + #1169, pending merge) and lengthened by three (#1174, #1175, #1176 open as tracked follow-ups). The net is positive: the closure-pending defects were UX/wire drift in the substrate's documented contract with regression tests now blocking regression; the new ones are tracked work whose Phase-1 closure plan is already filed. The shortened-caveat framing in §G and §I is conditional on the two PRs landing — explicitly flagged at both sites.
- New prime-directive addendum **pm-v3.1** stored in ai-memory `global/policies` namespace (memory `f5334545-c1f5-4f5c-9efb-a0ec3a0c1fcd`) — Variables + Constants + Vendor-Neutrality engineering discipline. The refinement pass itself was the inaugural application: every new test file uses module-level constants for invariant identifiers, every fixture source is vendor-neutral (`"api"` not `"claude"`), every recurring literal extracted at write time.
- QC subagent audits ran against each of the three blocks (Block 1 #1172, Block 2 #1169, Block 3 this refinement). Block 1 + Block 2 verdicts: PASS-WITH-RECOMMENDATIONS; Block 2's recommendation (the postgres-bootstrap drift site) was folded into the same PR rather than punted to a follow-up issue, per the pm-v3 "discovery → file → fix → close is one non-divisible workflow" clause.

— Claude Opus 4.7, 2026-05-24 (original Phase-1 probe + 2026-05-24 refinement pass).
