...
> **Refinement note (2026-05-24).** The original Phase-1 report carried two additional live caveats here: the `memory_reflect` metadata-passthrough defect (#1172) and the `models.embedding_dim` resolver gap (#1169). Both were filed during the original probe and closed in the refinement pass that produced this revision (PR #1177 lands a 5-invariant regression suite pinning the correct `memory_reflect` passthrough behavior end-to-end; PR #1178 routes `embedding_dim` through a new resolver-side canonical lookup table and additionally closes the parallel postgres-bootstrap drift site at `src/daemon_runtime.rs:3051-3059` that the Block 2 QC audit surfaced). Both issues are no longer live caveats. Three adjacent defects discovered during the same refinement pass remain open and tracked: #1174 (codegraph-driven pm-v3.1 global refactor sweep — `local`/`global`/`const` discipline), #1175 (MCP `handle_reflect` hardcodes `source: "claude"` — vendor-monoculture in heterogeneous-NHI substrate), and #1176 (MCP approval-gate `pending_action` payload drops `metadata` — sibling defect to #1172 via a different code path). The Phase-1 verdict is preserved (`SHIP-WITH-CAVEATS`); the caveat list is shorter post-refinement.
...
**SHIP-WITH-CAVEATS** — v0.7.0 is the minimum viable cognitive-checks-and-balances architecture for an AI NHI to be trustably-itself-across-time and trustably-stoppable-without-corruption; the remaining caveats (post-refinement, with #1172 and #1169 closed) are the honest limitations on intra-session hallucination and ADR-0001 quorum non-implementation, plus three substrate defects discovered during refinement and tracked for in-release closure: #1174 (pm-v3.1 variables/constants discipline refactor), #1175 (vendor-monoculture in `handle_reflect` `source` stamp), and #1176 (approval-gate metadata drop sibling to the closed #1172).
...
- [#1172](https://github.com/alphaonedev/ai-memory-mcp/issues/1172) — `memory_reflect` MCP tool strips caller-supplied `metadata.entity_id` (medium severity). **Closed in refinement pass via [PR #1177](https://github.com/alphaonedev/ai-memory-mcp/pull/1177)** — a 5-invariant regression suite pinning the correct passthrough behavior end-to-end (substrate + MCP wire layer + entity-binding discoverability). Live MCP probe in the closure comment confirms `metadata.entity_id` round-trips through the JSON-RPC boundary into both the metadata column AND the indexed `mentioned_entity_id` column without the `[entity:X]` title-marker workaround.
- [#1169](https://github.com/alphaonedev/ai-memory-mcp/issues/1169) — `memory_capabilities.models.embedding_dim` sources from tier preset (low severity, filed earlier in session). **Closed in refinement pass via [PR #1178](https://github.com/alphaonedev/ai-memory-mcp/pull/1178)** — added `KNOWN_EMBEDDING_DIMS` canonical lookup table + `canonical_embedding_dim` helper, threaded `embedding_dim: Option<u32>` through `ResolvedEmbeddings`, and additionally closed the parallel postgres-bootstrap drift site at `src/daemon_runtime.rs:3051-3059` that QC surfaced during the audit (worse than the capabilities-surface drift — on-disk pgvector column shape mismatch). 13-invariant regression suite covers known/unknown/tier-disabled/legacy-alias paths.
...
- Phase-1 verdict re-issued without change to the `SHIP-WITH-CAVEATS` line itself; the caveat list shortened by two (closed #1172 + closed #1169) and lengthened by three (#1174, #1175, #1176 open). The net is positive: the closed defects were UX/wire drift in the substrate's documented contract; the new ones are tracked work whose Phase-1 closure plan is already filed.
