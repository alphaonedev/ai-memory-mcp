...
| 12 | `memory_notify` | — | **Added in fix/arch3-mcp-cli-parity-batch2** (`notify`) | Cross-agent inbox primitive (writes into `_inbox/<target>/`). Distinct from `share` (which copies into `_shared/<from>→<to>/`); notify is one-shot message-style, share is durable knowledge-copy. The substrate handler enforces target-agent + title + payload validators and writes the row with the per-tier expiry. |
| 13 | `memory_inbox` | — | **Added in fix/arch3-mcp-cli-parity-batch2** (`inbox`) | Read `_inbox/<agent_id>/`. `--unread-only` and `--limit` mirror the MCP tool. |
