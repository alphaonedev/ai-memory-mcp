...
> **Status — v1.0.0:** ai-memory's iOS xcframework (`ai-memory-ios.xcframework.tar.gz`)
> and Android tar.gz (`ai-memory-android.tar.gz`) ship from the release
> pipeline **unsigned** (no producer code-signing certificate). Code
> signing is the **consumer's responsibility** at app-integration time
> — see §"Why consumer-signs-at-integration" below for the rationale.
> SLSA build-provenance attestation now DOES ship for both mobile
> artifacts (`actions/attest-build-provenance`, #2487 — verify with
> `gh attestation verify`); the remaining producer-side *codesign*
> follow-up (an AlphaOne-owned distribution certificate) is still
...
1. ai-memory publishes **unsigned** mobile artifacts (SLSA build
   provenance IS attested per #2487; a producer code-signing
...
3. Producer-side *codesign* (an AlphaOne-owned certificate that
   pre-signs the artifact at release time) is still on the roadmap. It
   is NOT load-bearing — the consumer-side codesign below is the
   contract end-users actually trust.
...
curl -L -o ai-memory-ios.xcframework.tar.gz \
  "https://github.com/alphaonedev/ai-memory-mcp/releases/latest/download/ai-memory-ios.xcframework.tar.gz"
tar -xzf ai-memory-ios.xcframework.tar.gz   # → ./AiMemory.xcframework
...
codesign -dvv --verbose=4 \
  /path/to/YourApp.app/Frameworks/AiMemory.xcframework/...
```
...
curl -L -o ai-memory-android.tar.gz \
  "https://github.com/alphaonedev/ai-memory-mcp/releases/latest/download/ai-memory-android.tar.gz"
...
Producer-side supply-chain hardening sits **above** the consumer
...
pipeline and was not tampered with on a mirror.

**Shipped today (#2487):** SLSA build-provenance attestation is
generated by the release workflow (`actions/attest-build-provenance`)
for BOTH mobile artifacts, so a downstream operator can mechanically
verify the artifact came from `release.yml`:

```bash
gh attestation verify ai-memory-ios.xcframework.tar.gz \
  --repo alphaonedev/ai-memory-mcp
gh attestation verify ai-memory-android.tar.gz \
  --repo alphaonedev/ai-memory-mcp
```

**Still on the roadmap (#1068 Layer 2):** producer-cert *codesign*:
...
## Verifying the unsigned artifact

The producer-cert codesign is still a roadmap item, but integrators
can verify artifact integrity + provenance TODAY using:

1. **SLSA build provenance (#2487)** — the strongest check. Confirm
   the artifact was built by `release.yml` (not tampered on a mirror):
...
2. **Per-asset SHA-256 checksum** — the `release.yml` workflow
   publishes a `<artifact>.sha256` file alongside each artifact.
   Verify with (run from the directory holding the downloaded tarball):

   ```bash
   curl -L -o ai-memory-ios.xcframework.tar.gz.sha256 \
     "https://github.com/alphaonedev/ai-memory-mcp/releases/latest/download/ai-memory-ios.xcframework.tar.gz.sha256"
   shasum -a 256 -c ai-memory-ios.xcframework.tar.gz.sha256
   ```

3. **GitHub release-tarball signature** — GitHub itself signs
   release tarballs with its own infrastructure key. The release
   page UI confirms the signing chain.

4. **Build-from-source** — the entire pipeline is open. If you
   have any doubt about the published binaries, build the
   xcframework / android tarball yourself from the release tag:

   ```bash
   git clone https://github.com/alphaonedev/ai-memory-mcp.git
   cd ai-memory-mcp
   git checkout <release-tag>   # the tag you downloaded the artifact from
