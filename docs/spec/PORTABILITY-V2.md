...
**[#3405, v1.0.0] Referential integrity (normative).** An exporter MUST NOT emit an edge whose `source_id` or `target_id` is not carried in the SAME artifact's `memories[]`. `memories[]` and `links[]` are computed from two independent reads — the memories read applies the fail-closed lifecycle allow-list (a `tombstoned`/`quarantined` row never leaves) and then the confidentiality screen (forbidden-class rows are DROPPED), while the links read filtered only on expiry — so a naive exporter emits edges naming rows the artifact does not contain. That is not cosmetic: `memory_links` carries `REFERENCES memories(id)` and importers run with foreign keys ON, so such an edge can NEVER be materialised, and `export | import` therefore exited `0` on the producing side and `3` on the consuming side forever, with no operator disposition. The withheld edges MUST be reported, not silently dropped: the default `export` carries the COUNT in-band under `withheld.dangling_links_withheld` and the rendered `"<source>-><target>"` labels on the operator stderr channel ONLY (an endpoint named there is by construction a withheld id, so publishing it in-band would leak the same index the #2490 `withheld_ids` boundary keeps out of the artifact). Dropping the edge does NOT by itself make an export partial — the endpoint omission that caused it is already accounted for. An importer that receives a pre-#3405 (dangling) bundle MUST skip the unresolvable edge with a per-edge report, count it separately from a destination-side refusal, and exit non-zero unless the operator explicitly acknowledges it with `import --allow-dangling`.
