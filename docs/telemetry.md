...
**Prometheus `/metrics` (HTTP daemon, pull-based).** The `serve` daemon additionally exposes a Prometheus scrape surface at the bare `/metrics` path (alias `/api/v1/metrics`; `src/metrics.rs`). It is pull-only — nothing is pushed anywhere. The registered series at v0.7.0 include the core counters/gauges `ai_memory_store_total`, `ai_memory_recall_total`, `ai_memory_recall_latency_seconds`, `ai_memory_memories`, the HNSW gauges (`ai_memory_hnsw_size`, `ai_memory_hnsw_evictions_total`, `ai_memory_hnsw_last_eviction_at_nanos`), the webhook/subscription series (`ai_memory_webhook_dispatched_total`, `ai_memory_webhook_failed_total`, `ai_memory_subscriptions_active`, `ai_memory_subscription_dlq_overflow_total`, `ai_memory_subscription_dispatch_truncated_total` — v1.0.0 #2592; non-zero means the subscriber scan hit its 1000-row ceiling and subscribers past it received NO event — and, v1.0.0 #3659, the delivery-audit persistence twins `ai_memory_webhook_audit_status_persisted_total`, `ai_memory_webhook_audit_update_failed_total{stage}` (closed set) and `ai_memory_webhook_audit_last_failure_at_seconds`), the curator series (`ai_memory_curator_cycles_total`, `ai_memory_curator_cycle_duration_seconds`, `ai_memory_curator_operations_total`), the autonomy/quality series (`ai_memory_autonomy_hook_total`, `ai_memory_contradiction_detected_total`, `ai_memory_corrupt_provenance_rows_total`, `ai_memory_auto_export_spawn_failed_total`), and the federation series (`ai_memory_federation_push_dlq_depth`, `ai_memory_federation_push_dlq_quarantined_total`, `ai_memory_federation_fanout_retry_total`, `ai_memory_federation_fanout_dropped_total`, `ai_memory_federation_partial_quorum_total`, the #3654 per-peer freshness series (`ai_memory_federation_peer_configured`, `ai_memory_federation_peer_last_attempt_timestamp_seconds`, `ai_memory_federation_peer_last_success_timestamp_seconds`, `ai_memory_federation_peer_consecutive_failures`, `ai_memory_federation_peer_failures_total`, `ai_memory_federation_peer_clock_skew_seconds`, `ai_memory_federation_peer_push_dlq_depth`, `ai_memory_federation_peer_push_dlq_oldest_failed_timestamp_seconds`, `ai_memory_federation_catchup_interval_seconds`; their `peer` label is the minted `peer-h1…` id, or a hashed label for an id of unknown shape, never a URL), `ai_memory_federation_cred_verify_total`, `ai_memory_federation_inbound_cred_total`, `ai_memory_federation_cred_max_age_seconds`, `ai_memory_federation_renewal_lag_seconds`), and — v1.0.0 #3806 — the three `[decision]` seam series `ai_memory_decision_latency_seconds{seam}`, `ai_memory_decision_outcome_total{seam,outcome}` and `ai_memory_decision_abstain_total{seam,reason}`. `seam` is the preregistered closed set `classify_kind|detect_contradiction|synthesis_verdict|consolidation_merge` (all four are wired: the two W2 seams and the two DESTRUCTIVE seams, W3's synthesis delete and W4's consolidation merge). Since the vote's R5 the labels are DERIVED from a typed `DecisionOutcome` (`decided|fallback|abstained(reason)`) through `metrics::record_decision(seam, outcome, secs)` rather than passed as strings, so their cardinality is bounded by the type: `outcome` is the closed set `decided|fallback|timeout|egress_refused|abstained` and a `reason` label (closed set `no_provider|timeout|egress_refused|unavailable|unusable|unsupported`, the `AbstainReason` wire tokens) can exist ONLY on an abstain. The last two series answer DIFFERENT questions and are both needed: `outcome` answers "did the seam get a decision?", where an ABSTAIN is a first-class outcome so "no opinion" is never indistinguishable from a decided verdict; `reason` answers "why not?". **Alert on `reason="unavailable"` and not on `reason="unusable"`**: the first means the endpoint could not be reached or did not answer with a decision response (an outage), the second means the model ANSWERED and declined, which is the feature working as designed. The R9 per-surface circuit breaker (3 consecutive unavailability abstains, 30 s cooldown, one probe) reports its short-circuit as that same `unavailable` abstain — NO new series and no new label value — and on the destructive seams a `yes` the seam cannot use (no confidence, or below the 0.80 floor) is `outcome="abstained"`, `reason="unusable"`, never a decided verdict the seam did not take. All three are created LAZILY by their first observation, so a deployment with no `[decision]` section exposes none of them and its exposition is unchanged. Their worst case — all four seams by all five outcomes and all six reasons — is MEASURED deterministically by `tests/decision_metrics_budget_3806.rs` against an unchanged 8 KiB inner budget (the series add 8114 bytes to a 18322-byte baseline, 78 of the 8192-byte budget spare; total 26436 of 65536). Neither that inner budget nor the 64 KiB read cap (#3654) was raised for this feature. Metric values are operation counts and durations only — never memory content.
