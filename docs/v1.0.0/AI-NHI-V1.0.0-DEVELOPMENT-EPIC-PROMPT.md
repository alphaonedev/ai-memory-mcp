...
2. **Multi-agent code review — AI NHI, codegraph-anchored** (operator correction 2026-07-09): the AI NHI performs it within the epic — multiple review-lens agents (correctness, reuse/simplification, efficiency, altitude) working against the current codegraph index (re-index first), adversarially verified before filing, every finding an issue. Not an external service, not a one-shot tool call — a multi-agent pass the orchestrator drives.
