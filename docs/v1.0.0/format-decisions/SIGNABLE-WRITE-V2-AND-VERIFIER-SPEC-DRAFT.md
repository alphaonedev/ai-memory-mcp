...
## Appendix A — Golden-vector codec pins + generation methodology

> These are the concrete codec decisions that were previously `TBD-impl`. They are **frozen design pins**, not hand-authored bytes. **The actual `input → exact-hex` golden vectors are GENERATED by the pinned in-house encoder and frozen as the CI gate — never hand-written** (a hand-authored hex blob risks a subtle CBOR shortest-form / length-prefix / ordering error baked permanently into a reference vector; the encoder is the sole authority for the bytes, the vectors pin its output).

### A.1 CBOR codec pins (per §1)
- **Top-level:** CBOR **array**, major type **4**, definite-length, element count fixed per record type (e.g. 11 for the SignableWrite v2 array in §2.2). Header byte for an 11-element array = `0x8b` (`0x80 | 11`); ≥24 elements would use the 1-byte-length form `0x98 <count>` — but every frozen record type has <24 elements, so all use the single-byte `0x8_` form.
- **Text (mt3):** definite-length UTF-8; length in shortest form (`0x60|len` for len<24, else `0x78 <len>` …). Element 0 (`_dst`) and every text field.
- **Unsigned int (mt0) / negative (mt1):** shortest form only (values 0–23 inline in the type byte; 24–255 as `0x18 <byte>`; etc.). No non-canonical wide encodings.
- **Byte-string (mt2):** definite-length; used for `content_digest`, `instance_key_id`, `model_version_ref`.
- **Forbidden across all frozen types:** floats/simple (mt7), tags (mt6), indefinite-length, `null`, map (mt5 is v1-only), duplicate/non-shortest encodings.

### A.2 Multihash content-digest codec (per §2.2 element [6])
Self-describing `<codec-varint><length-varint><digest-bytes>` (unsigned-LEB128 varints):
- `0x12` = sha2-256 (32-byte digest) — the default for `content_sha256`.
- `0x1e` = blake3 (32-byte digest) — the cid outer-address hash family.
- A future PQ/new hash binds by allocating a new codec byte; the digest length follows from the codec, so the verifier never guesses length.

### A.3 Presence-encoding for optional array elements (per §2.2 [9] `session_id`)
...
[ "ai-memory/write/v2",                     # [0] _dst
  ...
  [0],                                        # [9] session_id ABSENT (presence-encoded)
  0 ]                                         # [10] suite_tag = 0 (ed25519+sha256)
```
Ed25519 is computed over the pinned canonical encoding of this array. **The exact hex is produced by the encoder + pinned as `tests/golden/signable_write_v2/*.hex` (the #1837 corpus + the CI golden-vector gate); it is deliberately not reproduced here.**

### A.5 Domain-tag registry (frozen strings)
`ai-memory/write/v2` · `ai-memory/subkey-cert/v1` · `ai-memory/peer-head-attestation-v1` · `ai-memory/equivocation-proof/v1` · `ingestion-v1` · (v1x, deferred) `ai-memory/recall-attestation/v1`. Each is committed inside its record's signed bytes; distinct tags never cross-verify (§1). New record types allocate a new tag; a version bump on an existing type increments the trailing integer.

---

*Draft skeleton — authored 2026-07-09 in the Fable window; Appendix A added same day. Freeze decisions are final (memories cited). The codec pins in Appendix A are frozen; the byte-exact golden vectors are encoder-generated + CI-pinned (#1837), never hand-authored. This document is the R24 conformance target. Ship-law escalation of any spec-axis amendment routes through the #1171 heterogeneous panel (#1967).*
