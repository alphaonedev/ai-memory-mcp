...
| ↳ **SUPERSEDED (v0.9 scope)** | **The "port 6 extras to SAL, SAME epoch+decorrelation path" reading above is superseded for v0.9** by the `2x5-wave vote (fb2aa557-88be-4c16-a360-f4a600457351)` ACCEPT-gate decision (see the EPIC §25.3-P0 scope record + #1872). v0.9 slice = decorrelation-probe cycle parity only; full `run_once`↔SAL unification → **RQ-PARITY-02 (v1.0)**; epoch parity → RQ-10. | — | — | — | — | PARTIAL recorded as PARTIAL, never DONE | EPIC §25.3-P0 / #1872 |
