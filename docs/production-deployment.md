...
SQLite deployments use `ai-memory backup` (a `VACUUM INTO` wrapper that emits a defragmented snapshot plus a manifest signed with the operator key). It is SQLite-only and **refuses** a store it cannot capture — see the Postgres note below ([#2444](https://github.com/alphaonedev/ai-memory-mcp/issues/2444)):
...
ai-memory restore --from /var/backups/ai-memory --latest                     # or: the newest SIGNED backup
```

The manifest pins the snapshot's file name, sha256, byte size, source-DB path, creation time, backend, schema version and the binary version that produced it, and since v1.0.0 it is **signed with the operator key** ([#3199](https://github.com/alphaonedev/ai-memory-mcp/issues/3199)): `restore` verifies the signature against the operator public key it resolves itself (`AI_MEMORY_OPERATOR_PUBKEY`, else the key directory), never a key the manifest carries, and only then checks the sha256 the signature covers. So the backup host needs the operator signing key (`ai-memory rules keygen`), and the restore host needs its public key. A pre-v1.0.0 (unsigned) manifest is refused unless you pass `--allow-unsigned-manifest`; take a fresh backup right after upgrading. `--keep` rotates oldest-first by the SIGNED creation time and never deletes a backup it cannot verify ([#3604](https://github.com/alphaonedev/ai-memory-mcp/issues/3604)). Pass `--skip-verify` only for a manifest-less file you have already verified out-of-band: it WARNs, records an audit event, and is refused under `asi-hard`, which restores only signed backups. The details, and the procedure for a legacy file under `asi-hard`, are in [`CLI_REFERENCE.md` §"Signed manifests"](CLI_REFERENCE.html#signed-manifests-v100-3199).
...
**Post-restore verification.** Three verifiers ship: `ai-memory verify-audit-trail` (the canonical PE-8 / #697 whole-of-audit verifier — walks the cross-row `signed_events` hash chain plus the tamper-evidence anchors: tail-truncation via the off-table watermark high-water, head-hash, witness dual-head, and the schema-v73 cause-binding; exits non-zero on any dirty verdict), `ai-memory verify-reflection-chain <memory_id>` (L1-3 — walks `reflects_on` edges backward to depth 0 and verifies each Ed25519 signature; exit 0 on a fully-verified chain), and `ai-memory verify-signed-events-chain --format json` (V-4 — the narrower cross-row `signed_events` hash-chain walk; `chain_holds: true` is the pass signal). Run `verify-audit-trail` against the restored database before promoting it, plus `verify-reflection-chain` for any reflection subtree you depend on.
...
**The mirrored refusal at the low end** ([#2564](https://github.com/alphaonedev/ai-memory-mcp/issues/2564)). A database whose recorded `schema_version` is zero, negative, or deleted outright, but which provably holds durable rows, is refused as `SCHEMA_STAMP_INVALID` rather than migrated. The corroboration is structural, not a heuristic: rows are present AND `memories.confidence` is present — a column only a post-v2 schema has — so the stamp is provably wrong about the database it describes. Operating it would replay the entire v1 → ladder-tip migration over live data — and before #2564 it did so with the pre-migration safety snapshot SUPPRESSED, because that snapshot was gated on a non-zero stamp. #2564 closes both halves: the snapshot gate now also fires on a durable-row probe (so the one database that still legitimately replays the whole ladder over live data, a genuine pre-v2 upgrade, is snapshotted first), and a stamp that is provably wrong is refused outright rather than replayed. **The operator action is the opposite of the downgrade refusal's:** do not install a different binary — restore the correct `schema_version` row, or restore a snapshot taken before the stamp was lost. As with the downgrade refusal, egress survives: `ai-memory backup` still works against such a database (it re-opens through the unmigrated funnel), and `ai-memory boot` / `ai-memory doctor` name the refusal explicitly. On the HTTP surface (postgres backend) it maps to `503`, not `500`, so an orchestrator parks the node instead of crash-looping it.
...
- **Tracing spans on stderr.** Every MCP tool call, every governance decision, every federation event emits a `tracing::info!` span. The default is `info` for every target (#3650), so no `RUST_LOG` is needed; `RUST_LOG` directives are layered on top of it, e.g. `RUST_LOG=ai_memory=debug` for deep traces. Note (post-#1562, 2026-06-09): the postgres SAL adapter emits under the literal targets `store::postgres` / `store::postgres::kg`, which an `ai_memory=...` directive does not match. The default `info` covers them; to raise them add e.g. `store::postgres=debug`. Before #3650 the default was `ai_memory=info`, which dropped those events entirely.
...
Hooks (`pre_store`, `pre_delete`, `pre_governance_decision`, etc. — 22 lifecycle events, see [`hook-pipeline.md`](hook-pipeline.html)) are the supported extension surface for routing events to a SIEM, paging an operator, or short-circuiting writes. **Before you wire a SIEM to one, check the firing-status table in [`hook-pipeline.md`](hook-pipeline.html):** at v1.0.0 the decision-class `pre_*` events all fire, but 11 advertised notify events (`post_store`, `post_recall`, `post_search`, `post_delete`, `post_promote`, `post_link`, `post_consolidate`, `post_governance_decision`, `post_reflect`, `on_index_eviction`, `on_compaction_rollback`) have no production fire site — a hook on one of those will never execute. See [`docs/integrations/`](integrations/) and [`telemetry.md`](telemetry.html).
...
**W-of-N federation.** Three or more peers, each holding its own SQLite, mesh-federating writes with a quorum commit requiring the local write plus W−1 peer acknowledgements within `--quorum-timeout-ms` before the write returns OK (per [`ADR-0001`](ADR-0001-quorum-replication.html); per-message Ed25519 signing rides on `AI_MEMORY_FED_REQUIRE_SIG`). Default 2000 ms assumes same-DC peers; cross-region (WAN) meshes need 5000-10000 ms — the do-1461 reference deployment uses 8000 (#1565). Resolves the "any single operator can rewrite history" problem. Consistency is CRDT-based (last-writer-wins on the durable text, eventual convergence); there is no strict-consistency / MVCC federation mode at v1.0.0, and federation is not end-to-end encrypted ([#1968](https://github.com/alphaonedev/ai-memory-mcp/issues/1968) open) — peer traffic is protected by mTLS in transit, not E2E. The certified operating envelope is **500-1000 agents across ≤50 peers**, not internet-scale; the sizing table below is design guidance within that envelope.
