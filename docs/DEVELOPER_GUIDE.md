...
3. **HTTP daemon** -- an Axum web server exposing the same operations as a REST API with 93 route registrations / 79 unique URL paths (completely AI-agnostic)
...
```
main.rs            -- Thin CLI shim (W6 refactor); top-level Command enum now lives in daemon_runtime.rs (91 subcommands with --features sal, 89 in the default build)
...
storage/           -- sqlite SQL primitives; CRUD, FTS5, recall scoring, GC, migration (CURRENT_SCHEMA_VERSION = 84)
...
Start the HTTP daemon (93 route registrations / 79 unique URL paths).
