...
3. **HTTP daemon** -- an Axum web server exposing the same operations as a REST API with 89 route registrations / 76 unique URL paths at v0.7.0 (completely AI-agnostic)
...
storage/           -- sqlite SQL primitives; CRUD, FTS5, recall scoring, GC, migration (CURRENT_SCHEMA_VERSION = 69)
...
Start the HTTP daemon (89 route registrations / 76 unique URL paths at v0.7.0).
