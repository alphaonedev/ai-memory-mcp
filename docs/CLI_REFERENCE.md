...
(the `Command` enum in `src/daemon_runtime.rs`; 97 top-level
subcommands in the default build, 99 under `--features sal` — SSOT:
...
### `tls` — listener certificate material (v1.0.0, #3709 item 2)

The daemon serves TLS only (#3705). Its material lives under
`<key_dir>/tls/` and these verbs read and write that directory through the
same module the listener uses, so the verdict `status` prints is the verdict
`serve` reaches.

| Subcommand | Notes |
|---|---|
| `ai-memory tls init [--host <host>]` | Mint the local CA and server certificate now (what first boot does). **Singleton shape only**: any other declared `[deployment] shape` is refused and told to `import` enterprise PKI. Idempotent — valid material is reused; a leaf that does not cover `--host` is re-issued from the local CA. Refuses when operator-supplied material is installed (it mints nothing over it). |
| `ai-memory tls import --cert <fullchain.pem> --key <key.pem> [--ca <ca.pem>] [--host <host>]` | Verify an operator pair the way the listener will — PEM parse, private key matches the leaf, not expired, `--host` covered by a subject alternative name, `--ca` is the issuer — then install it as `server.pem` / `server.key` (0644 / 0600). **Nothing is written unless every check passes.** Any shape. `--ca` installs `operator-ca.pem`, the root the bundled clients (`doctor --remote`, the MCP forward) trust. The daemon never renews operator material; re-run `import` with fresh files. |
| `ai-memory tls status` | Source (`absent` / `local-ca` / `operator-supplied`), issuer, subject alternative names, expiry and days remaining, the client root, the declared shape, and the next-boot verdict: `encrypted as required for shape <s>` or `REFUSES — <why>. Fix: <the command>`. Exit 0 either way; `--json` carries `boot_would_refuse`. |

Global flag: `--key-dir <PATH>` overrides the key directory (default:
`AI_MEMORY_KEY_DIR`, then the platform key directory). Every refusal
prints the command that resolves it (#3709 item 5). Not in v1.0.0:
`tls acme`, automatic renewal of imported material, `db check-tls`.
