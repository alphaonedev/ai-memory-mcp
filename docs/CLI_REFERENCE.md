...
| `AI_MEMORY_REQUIRE_AGENT_ATTESTATION` | #1751 — store-path write-attestation posture. **On by default as of v0.9.0**: an unsigned direct CLI `store` / MCP `memory_store` / HTTP write is rejected (`403 ATTESTATION_FAILED`). Set to `0`/`false` to restore the pre-v0.9 permissive (`claimed`) behavior. |
