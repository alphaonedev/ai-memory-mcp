...
(the `Command` enum in `src/daemon_runtime.rs`; 83 top-level
subcommands in the default build, 85 under `--features sal` — SSOT:
...
Run as an MCP tool server over stdio (JSON-RPC 2.0). At v0.8.0,
`--profile full` advertises 100 entries (99 callable memory tools + the
