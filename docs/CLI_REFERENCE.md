...
| `--ruling-key` | string | — | #3587 — keyed store. Always inserts under a fresh id (a same-title row in the namespace is a conflict, never a merge) and archives the newest live row with the same key in the exact namespace as `superseded`, when `AI_MEMORY_AGENT_ID` owns it and the new row is strictly newer. `--agent-id` is a claim and does not authorize. Otherwise the row is stored, the old one kept, and `--json` output carries `supersede_skipped`. The key is write-once: `ai-memory update --metadata` cannot change it. SQLite only; a `postgres://` store is refused. |
| `--as-admin` | bool | `false` | #3587 — with `--ruling-key`, supersede a row owned by another agent. Requires `AI_MEMORY_AGENT_ID` in the `[admin].agent_ids` allowlist of `config.toml` (or `AI_MEMORY_ADMIN_AGENT_IDS`); otherwise refused and audited. |
