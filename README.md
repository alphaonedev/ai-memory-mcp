...
```bash
ai-memory serve
# 96 REST route registrations (82 unique URL paths) at http://127.0.0.1:9077/api/v1/
```
...
### Interfaces
- **97 HTTP route registrations (83 unique URL paths)** -- full REST API on 127.0.0.1:9077 (works with any AI or tool). 65 of the 83 unique paths are served on the PostgreSQL backend; the remaining 17 are fully fail-closed 501, and MCP-stdio is SQLite-only — see [Backend parity](#backend-parity).
- **94 CLI subcommands under `--features sal` OR `--features sal-postgres`** (92 in the default build) -- complete CLI with identical capabilities, including `ai-memory features` (#2676)
