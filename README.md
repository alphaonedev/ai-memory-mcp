...
v0.9.0 is primarily a **security-hardening and code-review release** — 49 fixes from a 5-lane adversarial review ([#1885](https://github.com/alphaonedev/ai-memory-mcp/issues/1885)–[#1935](https://github.com/alphaonedev/ai-memory-mcp/issues/1935)) — plus a smaller set of additive features layered on the v0.8.0 coordination substrate. Full changelog: [`CHANGELOG.md`](CHANGELOG.md) §"[Unreleased] — v0.9.0".

### Secure-by-default hardening

- **Agent attestation required by default on writes** ([#1751](https://github.com/alphaonedev/ai-memory-mcp/issues/1751)). `AI_MEMORY_REQUIRE_AGENT_ATTESTATION`'s compiled default flips `false → true`: an unsigned direct-store write (MCP `memory_store`, HTTP `POST /api/v1/memories`, CLI `store`) is now rejected (`403 ATTESTATION_FAILED`) instead of landing `attest_level="claimed"`. Sign writes (`ai-memory store --sign` with a keypair bound via `ai-memory agents bind-key`) or set the explicit opt-out `AI_MEMORY_REQUIRE_AGENT_ATTESTATION=0` to restore the pre-v0.9 permissive posture. This is the flip promised by the v0.8.0 one-cycle deprecation WARN ([#1464](https://github.com/alphaonedev/ai-memory-mcp/issues/1464)).
