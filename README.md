...
### Interfaces
- **96 HTTP route registrations (82 unique URL paths)** -- full REST API on 127.0.0.1:9077 (works with any AI or tool). 61 of the 82 unique paths are served on the PostgreSQL backend; the remaining 21 are fully fail-closed 501, and MCP-stdio is SQLite-only — see [Backend parity](#backend-parity).
