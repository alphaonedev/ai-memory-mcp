...
- **93 CLI subcommands under `--features sal` OR `--features sal-postgres`** (91 in the default build) -- complete CLI with identical capabilities, including `ai-memory features` (#2676)
...
93 top-level subcommands under `--features sal` OR `--features sal-postgres` (91 in the default build; the 2-variant gap is `Migrate` + `SchemaInit`, both gated `#[cfg(feature = "sal")]` per `src/daemon_runtime.rs::Command::{Migrate,SchemaInit}`; was 40 at v0.6.4). Run `ai-memory <command> --help` for details on any command, or `ai-memory --help` for the full list.
