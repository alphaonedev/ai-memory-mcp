...
### Interfaces
- **98 HTTP route registrations (84 unique URL paths)** -- full REST API on 127.0.0.1:9077 (works with any AI or tool). 67 of the 84 unique paths are served on the PostgreSQL backend; the remaining 17 are fully fail-closed 501, and MCP-stdio is SQLite-only — see [Backend parity](#backend-parity).
...
Both statements are mechanically checkable and pinned against regression: `postgres_endpoint_supported` is the allowlist, `EXPECTED_PRODUCTION_UNIQUE_PATHS_COUNT` (`src/lib.rs`) is the denominator (82), and `tests/pg_supported_route_inventory_gate_2799.rs` freezes the exact 65-supported / 17-fully-501 inventory so a silent drift between the router and the allowlist fails CI.
