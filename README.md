...
- **Federation hardening.** Peer key enrollment, per-message signatures and nonce freshness are required by default. Namespace authorization is separate: the default-on `AI_MEMORY_FED_REQUIRE_PUSH_NAMESPACE_SCOPE` refuses inbound writes without a peer allowlist. Configure explicit peer scopes in `AI_MEMORY_FED_PEER_ATTESTATION`; Standard permits the existing explicit namespace opt-out (`=0`), while `asi-hard` requires a valid, nonempty allowlist whenever peers are configured and refuses boot without it. Standard warns. Ordinary doctor reports the posture; `/capabilities` v2/v3 carries the daemon’s boot evaluation. See [`docs/federation.md`](docs/federation.md).
...
2. **HTTP / mTLS daemon** -- 100 REST route registrations (86 unique URL paths) on `127.0.0.1:9077`, TLS + optional mTLS allowlist + API-key auth, background GC loop. `ai-memory serve`
...
### Interfaces
- **100 HTTP route registrations (86 unique URL paths)** -- full REST API on 127.0.0.1:9077 (works with any AI or tool). 73 of the 86 unique paths are served on the PostgreSQL backend; the remaining 13 are fully fail-closed 501, and MCP-stdio is SQLite-only — see [Backend parity](#backend-parity).
- **97 CLI subcommands under `--features sal` OR `--features sal-postgres`** (95 in the default build) -- complete CLI with identical capabilities, including `ai-memory features` (#2676)
...
100 route registrations / 86 unique URL paths on `127.0.0.1:9077`. Start with `ai-memory serve`. The table below shows the most commonly used REST endpoints; see [`docs/API_REFERENCE.md`](docs/API_REFERENCE.md) for the full surface (governance, federation, subscriptions, knowledge-graph, quotas, approvals SSE).
