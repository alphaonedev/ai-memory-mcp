...
if [[ -n "$EXISTING" && $RESET -eq 0 ]]; then
    ...
    if [[ $DRY_RUN -eq 1 ]]; then
        ...
        # #1751 — the v0.9 store-path default REQUIRES attestation; this
        # standard-seed memory is an unsigned installer write, so use the
        # documented explicit opt-out SCOPED to this one invocation (the
        # operator's global posture is untouched).
        STORE_OUT=$(AI_MEMORY_REQUIRE_AGENT_ATTESTATION=0 ai-memory --db "$DB" store \
            --namespace "$NAMESPACE" \
