...
OUT="$ROOT/docs/compliance/evidence/cert-f32c18dad"
FIX="$ROOT/.local-runs/cert-f32c18dad-fixtures"
...
fix = Path(os.environ.get("FIX") or Path.cwd() / ".local-runs/cert-f32c18dad-fixtures")
...
sanitize() {
  ...
  sed -e "s|$ROOT|<repo-root>|g" \
      -e "s|$FIX|<repo-root>/.local-runs/cert-f32c18dad-fixtures|g" \
      -e "s|$HOME|<home>|g"
}
...
{
  echo "# posture-legs transcript — recorded exit codes (2026-09-21 f32c18dad re-issue recapture (22 checks) @ $(git -C "$ROOT" rev-parse --short HEAD))"
