...
#
# This is `scripts/check-docs-vs-ssot.sh` exactly as it stood at
# `03bbd556` (the parent of the #2492 widening), preserved verbatim
# below the shebang. It is the R-203 regression leg for the widened
# pattern set: `scripts/check-docs-vs-ssot.sh --self-test` plants the
# five stale README shapes the 3x7 claims audit found
# (`docs/audit/3x7-claims-register-2026-08-01.md` 3.3.1) and asserts
# that THIS script ACCEPTS them while the live one REJECTS them. A
# self-test that only proves the new gate works is tautological -- the
# whole finding was that the old gate greened a page carrying five
# stale SSOT values.
#
# Its own `--self-test` path is never invoked (it would `mktemp -d`,
# which the house rule forbids); the live gate drives it only in
# normal mode with `AI_MEMORY_DOCS_GATE_ROOT` pointed at a fixture
# tree under `.local-runs/`.
#
# Delete this file only when no rule in the live gate depends on it.
