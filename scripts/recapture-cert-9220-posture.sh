#!/usr/bin/env bash
# #3607 re-issue: recapture §2 four-leg doctor --posture evidence at the published tip (22 checks: +#3553 synchronous, +#3199 backup signing) — derived from
# the current release tip (20 checks). Docs/evidence only.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT="$ROOT/docs/compliance/evidence/cert-92209ad91"
FIX="$ROOT/.local-runs/cert-92209ad91-fixtures"
...
fix = Path(os.environ.get("FIX") or Path.cwd() / ".local-runs/cert-92209ad91-fixtures")
...
PY
FIX="$ROOT/.local-runs/cert-92209ad91-fixtures"
# #3199 check #22 (backup-manifest signing): the certified configuration carries the operator key pair in the key dir.
if [[ ! -f "$FIX/keys/operator.key" ]]; then
  env -i PATH="/usr/bin:/bin:$HOME/.cargo/bin" HOME="$HOME" TMPDIR="$TMPDIR" AI_MEMORY_NO_CONFIG=1 AI_MEMORY_KEY_DIR="$FIX/keys" \
    "$DEFAULT_BIN" rules keygen >"$FIX/keygen.out" 2>&1 || { echo "operator keygen failed: $(cat "$FIX/keygen.out")" >&2; exit 1; }
fi
if [[ -f "$FIX/keys/operator.key.pub" ]]; then OPUB="$(tr -d '\n' < "$FIX/keys/operator.key.pub")"; else OPUB="$(tr -d '\n' < "$FIX/operator.pub.b64")"; fi
ATTEST="$(cat "$FIX/peer-attestation.json")"

sanitize() {
  # collapse absolute repo + fixture paths
  sed -e "s|$ROOT|<repo-root>|g" \
      -e "s|$FIX|<repo-root>/.local-runs/cert-92209ad91-fixtures|g" \
      -e "s|$HOME|<home>|g"
}
...
if [[ ! -x "$SQLCIPHER_BIN" ]]; then
  ...
  # #3553 check #21 observes the LIVE pragma on the store the sqlcipher build can open only with a passphrase.
...
{
  echo "# posture-legs transcript — recorded exit codes (2026-09-21 92209ad91 re-issue recapture (22 checks) @ $(git -C "$ROOT" rev-parse --short HEAD))"
