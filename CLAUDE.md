...
| 48 | `AI_MEMORY_REQUIRE_AGENT_ATTESTATION` | bool (`0`/`false` opt-out; `1`/`true` opt-in) | `true` (REQUIRED — v0.9.0 store-path default flip, #1751) | CLI/daemon/MCP (every store write path) | config | **[#626 Layer-3 C7, v0.7.0; default flipped false→true by #1751, v0.9.0]** Fail-CLOSED on unsigned direct-store writes BY DEFAULT since v0.9.0 (the flip promised by the v0.8.0 one-cycle deprecation WARN from the #1464 5-agent vote): every store surface (MCP `memory_store`, HTTP `POST /api/v1/memories`, CLI `store`) that receives a write WITHOUT a caller-presented Ed25519 `signature` is rejected with `403 ATTESTATION_FAILED` instead of landing `attest_level = "claimed"`. When a `signature` IS presented it is verified against the agent's bound public key regardless of this flag (valid → `agent_attested`; forged → `403`); this flag only governs the unsigned-write disposition. The explicit `0`/`false` (case-insensitive) opt-OUT restores the pre-v0.9 permissive posture where unsigned writes land claimed; `1`/`true` is the now-redundant opt-in spelling; any other value falls through to the required default (a typo fails closed). **Migration (v0.8 → v0.9):** operators with unsigned CLI `store` / MCP `memory_store` / HTTP `POST /api/v1/memories` workflows must either sign writes (`ai-memory store --sign` with a keypair bound via `ai-memory agents bind-key`) or set `AI_MEMORY_REQUIRE_AGENT_ATTESTATION=0`. **Scope:** the STORE/direct-write path ONLY — the federation receive path attests via the peer-authorship allowlist (`resolve_inbound_attribution`, #1464), NOT this flag, so the flip does not change the network boundary; curator/autonomy self-writes go through the SAL `store()` surface (`CallerContext::for_admin`) and are exempt. Source: `src/identity/attest.rs::require_agent_attestation_enabled` (`AI_MEMORY_REQUIRE_AGENT_ATTESTATION`). |
...
full`, 91 production HTTP route registrations / 78 unique URL paths, 85 CLI subcommands (83 in the default build) at v0.8.0 (post FX-12/ARCH-3 + FX-C3 batch2 + #1389 L2 `RecoverPreviousSession` + #1443 `Expand` + #1598 `Reembed` + #1727 `UndoEdit`), tens of
