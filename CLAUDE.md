...
3. **CLI** (`src/main.rs` thin shim + `src/daemon_runtime.rs::Command`) — clap-based, **63 top-level subcommands** at v0.7.0 release (was 40 at v0.6.4; 63 verified via `awk '/^pub enum Command/,/^}/' src/daemon_runtime.rs | grep -E '^    [A-Z]' | wc -l` — count grew from 57 at v0.7.0 dev tip via #1146 adding the `Config` subcommand for `ai-memory config migrate`, then to 58 with the #1095 `Share` subcommand, then to 63 via FX-12/ARCH-3 adding `KgQuery` / `FindPaths` / `RecallObservations` / `CheckDuplicate` / `Replay` for MCP/CLI parity build-out) with optional `--json` output (count: `--features sal-postgres` includes the postgres-only `schema-init` subcommand for a total of 65; the default build ships 63 — the 2-variant gap is `Migrate` + `SchemaInit`, both sal-gated)
...
| `daemon_runtime.rs` | clap top-level `Command` enum (65 subcommands at v0.7.0 with `--features sal-postgres`; 63 in the default build — the 2-variant gap is `Migrate` + `SchemaInit`, both sal-gated; FX-12/ARCH-3 added `KgQuery` / `FindPaths` / `RecallObservations` / `CheckDuplicate` / `Replay`), HTTP daemon `serve` bootstrap, MCP `mcp` dispatch |
...
full`, 87 production HTTP route registrations / 73 unique URL paths, 63 CLI subcommands at v0.7.0 (post FX-12/ARCH-3), tens of
