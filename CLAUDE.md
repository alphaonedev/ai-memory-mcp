...
| `models/` | Core data structures: `Memory` (27 fields at v0.8.0 incl. v0.7.0 recursive-learning + Batman vocabulary + Form-4 provenance + Form-5 confidence-calibration columns + the v45 `version` BIGINT for Gap-1 optimistic concurrency + the v0.8.0 v64 `lifecycle_state`), `MemoryLink`, request/response types |
...
- **MemoryLink**: Typed directional relationships. **Nine variants at v0.8.0** (six at v0.7.0, was four at v0.6.x): `related_to`, `supersedes`, `contradicts`, `derived_from`, `reflects_on` (recursive-learning Task 1/8), `derives_from` (WT-1-A atomisation — atom row → parent memory), and the three v0.8.0 typed-cognition relations added when the `memory_links.relation` CHECK was extended 6→9 at schema v63: `decomposes_into`, `depends_on`, `advances` (Goal/Plan/Step DAG). Canonical enum in `src/models/link.rs::MemoryLinkRelation`. Each link row also carries the v0.7 temporal-validity columns (`valid_from`, `valid_until`, `observed_by`) and attestation columns (`signature`, `attest_level`, `signed_at`).
...
full`, 91 production HTTP route registrations / 77 unique URL paths, 85 CLI subcommands (83 in the default build) at v0.8.0 (post FX-12/ARCH-3 + FX-C3 batch2 + #1389 L2 `RecoverPreviousSession` + #1443 `Expand` + #1598 `Reembed` + #1727 `UndoEdit`), tens of
