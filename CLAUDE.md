...
| 37 | `AI_MEMORY_ENCRYPT_AT_REST` | bool (`1`/`true`) | `false` (plaintext at rest) | CLI/daemon/MCP (ANY build) | config | **[#228; TRUTH-FIX #3720, v1.0.0]** Toggles APP-LEVEL at-rest CONTENT encryption, and it WORKS ON A DEFAULT (non-sqlcipher) build — the pre-#3720 row said "sqlcipher build only", which UNDERSTATED a real security control (an operator reading the old row could ship plaintext believing encryption needed a binary they did not have, when this one env var would have encrypted it — a doc understating a control talks a customer out of protection they already hold). MEASURED on a stock `ai-memory` (no `--features sqlcipher`): with `AI_MEMORY_ENCRYPT_AT_REST=1` (or `[encryption].at_rest = true`) each memory's `content` is sealed and the plaintext `content` column is left EMPTY (the plaintext does not appear anywhere in the DB file); the ciphertext lands in the `memories.encrypted_envelope` BLOB (schema v44). **Cipher:** ChaCha20-Poly1305 AEAD (`crate::encryption`); per-record envelope, the `0x03` per-record-DEK variant (DEK wrapped under the per-agent KEK) is crypto-erasable, the `0x02` direct-ECDH variant is not. **Key source:** a per-agent X25519 ECDH keypair minted and cached under the resolved key directory (`get_or_create_keypair`, beside the Ed25519 signing keys, `mode & 0o077`-guarded) — the ECDH shared secret is run through HKDF-SHA256 (domain-separated) to derive the symmetric key, with the envelope version + ephemeral pubkey bound into the AEAD AAD; this is NOT a passphrase. **Scope:** the memory `content` column ONLY — `title` / `tags` / `metadata` stay plaintext — and it is NOT end-to-end across federation (the receiving peer re-seals under its own per-node key, #1809). **Key-loss = silent data loss:** losing the per-agent X25519 secret makes every `encrypted_envelope` permanently undecryptable, so back up the key directory. **What `--features sqlcipher` ADDS (a SEPARATE control):** whole-DATABASE (whole-file) encryption — under a sqlcipher build, `AI_MEMORY_ENCRYPT_AT_REST=1` requires an `AI_MEMORY_DB_PASSPHRASE` (or `--db-passphrase-file`) and refuses plain-sqlite opens, and switching the flag on against an existing plain DB does NOT retroactively encrypt it (operators `ai-memory export → encrypted-init → import`, or the sqlcipher `ATTACH … KEY` recipe). **`doctor` agrees with `serve`:** `at_rest = ENCRYPT_AT_REST` (healthy, Info) on a default build, `at_rest = sqlcipher` under a sqlcipher build, plaintext WARN when neither is engaged. Source: `src/encryption/mod.rs::{ENV_ENCRYPT_AT_REST,encryption_enabled,seal_content,get_or_create_keypair}` + the seal gate in `src/storage/mod.rs` + the at-rest disposition in `src/cli/doctor.rs`. |
