...
| 183 | `AI_MEMORY_TEST_3152_CHILD_ROLE` | enum `sqlite-sal` / `pg-trait` / `pg-if-match` / `mcp-update`, or the name of an `mcp::update::update_3152_tests` test re-run in a clean-environment child | unset (not a child) | tests (`store::update_atomicity_3152_tests` + `mcp::update::update_3152_tests`, re-exec'd test binary; helpers in `recover::in_tx_fault`) | **test-only** | **[#3152, v1.0.0]** Marks a re-exec of the test binary as a single-commit-update crash child: it arms the test-only `recover::in_tx_fault` point for one row and `abort()`s between the content patch and the lifecycle transition, so the parent can prove the row reads back fully unchanged on that backend/funnel. Never read outside `#[cfg(test)]`; the fault point does not exist in a shipped binary.
| 184 | `AI_MEMORY_TEST_3152_CHILD_DB` | path | unset | tests (`update_atomicity_3152` sqlite child) | **test-only** | **[#3152]** The sqlite database the `sqlite-sal` / `mcp-update` child updates (the pg children use `AI_MEMORY_TEST_POSTGRES_URL`, #27).
| 185 | `AI_MEMORY_TEST_3152_CHILD_ID` | memory id | unset | tests (`update_atomicity_3152` child) | **test-only** | **[#3152]** The row the child updates; the fault point is armed for this id only.
| 186 | `AI_MEMORY_TEST_3152_CHILD_MARKER` | path | unset | tests (`update_atomicity_3152` child) | **test-only** | **[#3152]** File the crash child writes (with the row id) immediately before aborting at the fault point; a clean-environment child writes it (with its test name) only after its test body passes, so a filter that ran nothing cannot pass.
