...
| `wake_hub/` | v1.0.0 #3467 (EPIC #3466) — `ai-memory wake-hub`: the same-host, CONTENT-FREE agent wake plane. A 0600 Unix-domain-socket switch (owner-only 0700 parent dir; peer credentials checked via `SO_PEERCRED` on Linux / `LOCAL_PEERPID`+`getpeereid` on macOS, with pid availability ASSERTED at start-up) that routes a bounded wake HINT — `{inbox_row_id, namespace, sender, digest, seq_high_watermark}`, <=256 B — and NEVER a message body: the v1 protocol has no `request`/`reply`/`notify` kinds and refuses their wire numbers by name. Holds no durable truth and OPENS NO DATABASE on either backend: the ai-memory inbox row is the record, the wake is a hint, and the <=60 s backstop poll is the guarantee, so hub loss degrades wake LATENCY only. Everything is bounded in BYTES (`limits.rs` is the one place to audit the memory envelope): per-recipient queue byte cap + hub-wide egress budget + a 500/s burst-2000 token bucket that charges topic fan-out amplification to the SENDER, `error/507` to the sender on overflow, offline state a coalesced set (count + bounded id set + `lagged`) never a payload ring, `RLIMIT_NOFILE`-derived connection ceiling. Sharded routing tables with NO `.await` under any lock. Identity is a trait boundary (`identity::HelloVerifier`) whose SHIPPED default refuses every hello until the scoped `a2a-hub/join/v1` delegation lands in #3468 — and there is deliberately no CLI flag that substitutes a permissive one. |
| `wake_sink/` | v1.0.0 #3469 (EPIC #3466) — the BRIDGE from the `agent_notified` wake bus (`inbox_wake`, #3465) to the wake hub (`wake_hub/`, #3467), in the two deployment shapes the EPIC allows. `uds` is the shape `serve` wires today: set `[wake_hub].sink_socket = "<hub socket>"` (the ONE key in that block `serve` reads; UNSET is the fail-closed default and starts nothing, no socket, no identity load) and `boot::install_from_config` starts a forwarder that is an ORDINARY hub client — same handshake, same length-delimited codec, same frame ceiling, no privileged side channel. `in_process` hands the frame straight to `Router::deliver` for a CO-HOSTED hub; it is a library call exercised by tests, because nothing in `serve` hosts a hub in this build. ONE IDENTITY ROOT: `producer_identity` issues the session under the daemon's OWN enrolled `daemon` key, minting a short-lived `a2a-hub/join/v1` delegation for the reserved `wake-hub-producer` principal with a FRESH in-memory session key per connection — no second enrolled root, no second private key, nothing written to disk; the operator's single revocable grant is the allowlist row binding that name to the daemon public key, published by the explicit `identity hub-cache --daemon-producer` switch (off by default; reads only `daemon.pub`; `bind_authority: daemon_key_dir`, which the hub accepts for that ONE reserved principal and no other, and which an older hub maps to "unrecognised" and refuses; audited on the `signed_events` spine like every allow/revoke, so dropping the switch revokes within the hub's one-second revalidation). Naming the reserved principal via `--include-agent` (#3508 renamed the selector off `--agent-id`, which is the global CALLER flag) is REFUSED, because it has no v97 key history and the store loop would silently omit it. A wake carries `{inbox_row_id, namespace, sender, digest, seq_high_watermark}` and NEVER a body or title, is addressed DIRECTLY to the recipient and never to a topic (own-inbox only per #3468 until #3505), and is stamped with the reserved producer id rather than the notifying agent's. Fire-and-forget with bounded buffering throughout: a slow, full or absent hub costs a HINT and a counter, never a committed notify, and every drop cause has its own `SinkMetrics` line. Reconnects are jittered, exponentially backed off, capped at the backstop interval and reset only after a session that actually lasted. A lost wake self-heals via `seq_high_watermark` (a gap = ONE catch-up inbox read) on top of the NORMATIVE `<=60 s` backstop poll (`wake_sink::BACKSTOP_POLL_MAX`, `docs/wake-hub.md`). A configured-but-unstartable forwarder is an ERROR log and no socket — never a silent skip, and never a reason to take `serve` down, because the durable inbox row is the record. |
| `wake_client/` | v1.0.0 #3470 (EPIC #3466) — the wake-hub CLIENT half, behind `ai-memory wake-listen` and `ai-memory inbox --wait`. Loads the scoped `a2a-hub/join/v1` delegation bundle `ai-memory identity delegate --scope a2a-hub` wrote into the key directory (`<key-dir>/<agent-id>.a2a-hub.json`, 0600) — a DELEGATED private key, NEVER the enrolled one, so there is no second identity root and no second enrolment ceremony — checks the hub socket the way the hub hardened it (owner-only 0700 directory, owner-only 0600 socket, both owned by the caller), performs the hello handshake through the same codec and frame ceiling every peer uses, and VERIFIES the welcome. `bundle` is fail-closed with no flag that opens it: a bundle that is not 0600 / not caller-owned / symlinked / an unknown version / minted for another hub / minted for another agent / whose private key is not the one its certificate authorises / whose certificate does not verify under the agent's ENROLLED public key in the same key directory / outside its window, is refused BEFORE the socket is dialled. `session` is an ordinary hub client (NO topics — own-inbox only per #3468). `mod` is the state machine: welcome, `lagged`, `wake`, and a `seq_high_watermark` gap each cost EXACTLY ONE catch-up inbox read through the existing `memory_inbox` funnel, never a read per queued hint; signals cross to the consumer through a BOUNDED channel with `try_send`, so a full channel coalesces reads (the queued read sees every row a dropped signal named) and is counted rather than silently lost. The `<=60 s` backstop poll (`wake_sink::BACKSTOP_POLL_MAX`) is ALWAYS armed, hub or no hub, and its clock resets on every catch-up read — so a hub that is down, refusing, or never deployed costs wake LATENCY and nothing else, and `--poll-secs` is REFUSED above 60 rather than clamped. Reconnects are jittered, exponentially backed off, capped at the same bound, and reset only after a session that actually lasted. |
...
| `llm.rs` | Provider-agnostic LLM client (#1067): query expansion, auto-tagging, contradiction detection. Two wire shapes — Ollama-native (`/api/chat` + `/api/embed`, no auth) and OpenAI-compatible (`/v1/chat/completions` + `/v1/embeddings`, Bearer auth). Backend selected by `AI_MEMORY_LLM_BACKEND` env var with 14 vendor aliases (xai, openai, anthropic, gemini, kimi, qwen, mistral, groq, together, cerebras, openrouter, fireworks, lmstudio, plus the generic `openai-compatible` escape hatch). The struct name `OllamaClient` is preserved post-#1066 for call-site backward compat; rename to `LlmClient` is non-breaking and tracked separately. |
...
| `kg/` | Knowledge-graph traversal (recursive-CTE; AGE Cypher for `kg_query` / `kg_timeline` / `lineage`; `find_paths` is CTE-only on both backends) |
...
- **Tiers**: short (6h TTL), mid (7d TTL), long (permanent). Tier transitions: **there is no automatic tier promotion at v1.0.0** — Boids item 1 (5-agent vote `4d3ea1c5`) removed the recall-driven mid→long auto-promote (fold/touch no longer escalate tier), so recall popularity can never rewrite a row's tier. `memory_promote` is the SOLE tier-raising verb; it jumps to long in a single call by default (short→long or mid→long, NOT short→mid→long stepwise). The MCP tool now accepts an optional `target_tier` parameter (`"mid"` or `"long"`) for callers that want to stop at an intermediate tier; omitting it preserves the historical highest-reachable-tier behavior. Downgrades (e.g. mid→short) are never honored — `db::update` enforces tier monotonicity.
...
- **Feature tiers**: keyword (FTS5 only) → semantic (embeddings) → smart (LLM-backed expansion/auto-tag/contradiction) → autonomous (cross-encoder reranking). **Post-#1067 (v0.7.0): tier no longer dictates LLM vendor.** Any tier can speak to any provider — local Ollama, xAI Grok, OpenAI, Anthropic, Google Gemini, Kimi, Qwen, Mistral, Groq, Together, Cerebras, OpenRouter, Fireworks, LMStudio, vLLM, or llama.cpp server — via the `AI_MEMORY_LLM_BACKEND` env var. The previous "smart/autonomous require local Ollama" framing is gone. **Post-#1598 (v0.7.x): the same is true of the embedder.** The `[embeddings]` section / `AI_MEMORY_EMBED_*` env vars select any #1067 vendor alias, `openai-compatible` (self-hosted TEI / vLLM / llama.cpp server), or `ollama` (default) — embeddings are no longer Ollama/local-only. Embedder construction is fail-closed: on failure, semantic recall degrades loudly to keyword mode (#1593); the chat LLM client is NEVER reused for embeddings.

### Recall Pipeline

Recall is multi-stage and **never read-only** — every recall mutates the database:

1. **FTS5 keyword search** — fuzzy OR query, scored by `(fts.rank * -1) + priority*0.5 + MIN(access_count, 10)*0.1 + (CASE WHEN confidence_source='default' OR confidence_source IS NULL THEN 0.5 ELSE confidence END)*2.0 + tier_bonus + recency_factor` (Boids item 1, vote `4d3ea1c5`: the access-count term is capped at `models::ACCESS_SCORE_CAP` = 10 so popularity contributes at most +1.0 — a tiebreak, not a driver; the confidence term reads PROVENANCE so an UNASSESSED row — caller omitted confidence ⇒ `confidence_source='default'`, or a genuinely-unattested NULL — scores as neutral 0.5 instead of the compiled 1.0 fallback, while an explicitly-asserted confidence keeps full weight; tier bonus is long=3.0 / mid=1.0 / short=0)
...
4. **Touch operations** (atomic) — increment `access_count`, **raise `expires_at` to `MAX(current expires_at, now + per-tier window)`** (1h short / 1d mid; extension FLOOR per #1596 — an access can extend a memory's life but can never move its expiry earlier, so the create-time 6h short / 7d mid backstop is preserved when it is later than the per-access window; supersedes the pre-#1596 sliding-window REPLACEMENT contract from #830). **v1.0.0 Boids item 1 (vote `4d3ea1c5`) removed the recall-driven escalation from the maintenance verbs (fold/touch/touch_many, both backends): no more mid→long auto-promote, no `updated_at` rewrite on promotion, no priority decade ladder.** The fold still applies the access_count bump (cap 1M), `last_accessed_at` and the per-tier TTL floor-extend. `memory_promote` jumps a memory to the highest reachable tier (long) in a single call by default; the optional `target_tier` parameter (`"mid"` | `"long"`, #831) stops at an intermediate tier.
...
world-readable via `/proc/<pid>/cmdline`, so the leakier channel must
...
| 8 | `AI_MEMORY_AUTONOMOUS_HOOKS` | bool (`1`/`0`) | `false` | CLI/daemon/MCP | config | Truthy = fire `auto_tag`+`detect_contradiction` after every `memory_store`. **[TRUTH-FIX #2587, v1.0.0]** Pre-#2587, `auto_tag` on the HTTP `create_memory` path did NOT check this flag at all (its sibling `maybe_detect_conflicts` did) — an untagged write fired the LLM `auto_tag` call UNCONDITIONALLY whenever an LLM was configured, measured at 4.9-11.1s per write in production. `auto_tag_eligible` (`src/handlers/http.rs`) now gates on this flag, matching the doc as originally written. Additionally (5-agent vote `4d3ea1c5`, decision memory `4e51e315`, 2026-08-11): even when this flag is truthy, `auto_tag` no longer runs "synchronously" on the HTTP write path — the durable insert completes first (tags are derived/regenerable, not durable truth) and the LLM call is deferred to a bounded background worker (`crate::background::auto_tag_worker`, row #164). `detect_contradiction`'s sibling hook (`maybe_detect_conflicts`) remains `#[allow(dead_code)]`, never wired into `create_memory` — this flag has no live effect on it today. |
...
| 10 | `AI_MEMORY_PERMISSIONS_MODE` | enum (`enforce`/`advisory`/`off`) | `enforce` (v0.7.0 secure default) | CLI/daemon/MCP | config | K3/K9 governance gate. Overrides `[permissions].mode`. Unparseable values warn + fall through. PINNED by `asi-hard` (#130 / #3168) to `enforce` — `off`/`advisory` refuse boot. |
...
| 12 | `AI_MEMORY_OPERATOR_PUBKEY` | base64 ed25519 | falls back to on-disk `operator.key.pub` | CLI/daemon/MCP/governance | **operator-advisory** (override authority) | Treated as override-authority — anyone who sets it controls rule signing. Lock down host. The VALUE is a PUBLIC key (no credential), so `secret` would be wrong; `operator-advisory` is the declared class for an authority-bearing knob. (This cell read `secret-adjacent` until v1.0.0 — a token outside the four-member class vocabulary declared above the table.) |
| 13 | `AI_MEMORY_KEY_DIR` | path | platform config dir + `/ai-memory/keys` | CLI/daemon/MCP | config | Override for ed25519 key storage location. Used by H4 `memory_verify` tests. **[#3709 item 1]** The zero-config listener TLS material lives under it: `<key_dir>/tls/` (`local-ca.pem`, `local-ca.key` 0600, `server.pem`, `server.key` 0600, dir 0700; `src/tls_bootstrap.rs::{TLS_SUBDIR,LOCAL_CA_CERT_FILE,SERVER_CERT_FILE}`), generated on first `serve` without `--tls-cert`/`--tls-key` and renewed automatically; the bundled clients trust that CA for THIS installation only, never for a federation peer. |
...
| 22 | `AI_MEMORY_CONFIDENCE_SHADOW_SAMPLE_RATE` | float 0.0-1.0 | `1.0` (i.e. EVERY touch, once shadow is on) | CLI/daemon/MCP | config | Fraction of touches that pay the shadow calibration cost, clamped to `0.0..=1.0`. **[TRUTH-FIX, v1.0.0]** this row read `0.0`, which inverts the operational meaning: `ShadowConfig::from_env` ends `.unwrap_or(1.0)`, so with `AI_MEMORY_CONFIDENCE_SHADOW=1` and this var unset, **100%** of touches pay the shadow cost, not 0%. The var only ever matters when #21 is on (`enabled` and `sample_rate` are read together into one `OnceLock` snapshot). Source: `src/confidence/shadow.rs::ShadowConfig::from_env`. |
| 23 | `AI_MEMORY_FED_PEER_ATTESTATION` | JSON object (peer ID to authorship/namespace scope) | unset | federation | config | Separate from peer key enrollment, which is default-required. Default-required namespace checks refuse inbound writes with no allowlist (#3582); Standard retains an explicit require-scope `0` opt-out (row 147). A configured empty/invalid map fails closed even at `=0`. With explicit outbound peers or inbound bindings configured, Standard warns and `asi-hard` refuses boot without a valid map. Valid `{}` permits boot and denies all peers; malformed input refuses hardened boot. Shared identity key enrollment or its read errors only warn in both postures. Ordinary doctor remains runnable; local outbound/listener argv is unobservable, while capabilities v2/v3 and remote doctor carry the daemon boot snapshot. Source: `src/federation/peer_attestation.rs::PeerAttestationConfig`, `src/federation/peer_posture.rs`. |
...
| 27 | `AI_MEMORY_TEST_POSTGRES_URL` | conn-string | unset | tests (CLI `schema-init`, postgres store) | **test-only** + **secret** | Points the Postgres backend tests at a live instance. The value is a full libpq conn-string with an inline password (the repo's own example is `postgres://ai_memory:dev_password@localhost:5433/ai_memory`), so it is credential-bearing and the Class column says so — a consumer keying off the column, rather than off this prose, must still scrub it. |
| 28 | `AI_MEMORY_TEST_AGE_URL` | conn-string | unset | tests (Apache AGE store) | **test-only** + **secret** | Same password-bearing conn-string shape as `AI_MEMORY_TEST_POSTGRES_URL` (#27), for the AGE-backed graph tests; same secret classification for the same reason. |
...
| 37 | `AI_MEMORY_ENCRYPT_AT_REST` | bool (`1`/`true`/`yes`/`on`, case-insensitive) | `false` (memory `content` persisted in PLAINTEXT) | CLI/daemon/MCP (every write that maps a `memories` row, both backends; **any** build — NOT sqlcipher-gated) | config | **[#3720 truth-fix]** App-level at-rest **content** encryption, and **NOT** a SQLCipher request: `encryption_enabled` (`src/encryption/mod.rs:1024`) reads this env (or the `[encryption].at_rest = true` config seed) with **no** `sqlcipher` feature gate, so the knob is live on the default build. When truthy, every subsequent memory WRITE seals `content` under a fresh per-record ChaCha20-Poly1305 DEK wrapped to the per-agent X25519 KEK and stores the `0x03` envelope in `memories.encrypted_envelope`, leaving an empty-string placeholder in `content` (`seal_content`, `src/encryption/mod.rs:1076`). **Content only** — `title` / `tags` / `metadata` stay plaintext. Both backends: sqlite `src/storage/mod.rs:1514`, postgres `src/store/postgres.rs:17448` (column added by schema v68, `src/store/postgres.rs:6126`). Reads are gated on envelope PRESENCE, never on this flag (`open_content`, `src/encryption/mod.rs:1183`), so rows sealed while it was on stay readable after it is turned off; conversely rows written BEFORE it was on stay plaintext — there is no backfill or re-seal sweep, so flipping this on does not encrypt an existing corpus. **Key custody:** the KEK is `<key-dir>/<agent_id>.x25519.priv` (mode 0600, key dir honours `AI_MEMORY_KEY_DIR` — `src/encryption/mod.rs:44`); lose it and every sealed `content` is unrecoverable — no escrow, no rotation path at v1.0.0 (#3717), and a READ never mints a replacement (#3718). Setting this on a **non-sqlcipher** build is supported and boots normally: the boot gate refuses a *passphrase* on such a build, not this knob (`refuse_at_rest_requested_without_sqlcipher`, `src/storage/connection.rs:521`). Whole-DB SQLCipher is a **separate** control engaged by the passphrase channel (rows #2 / #40 plus `--features sqlcipher`; `apply_sqlcipher_key`, `src/storage/connection.rs:1222`) — the pre-#3720 `export → encrypted-init → import` note here was a SQLCipher recipe and does not describe this knob. NB: certified-posture control #15 on a sqlite backend still requires `sqlcipher_build && this env` (`src/enterprise_federation_posture.rs:611`), so on a default build this knob encrypts content but does not by itself satisfy control #15; reconciling that predicate is the open half of #3720. |
...
| 125 | `AI_MEMORY_FED_REQUIRE_CHECKPOINT_SIG` | bool (`1`/`0`) | `1` (fail-CLOSED, v1.0.0 secure default) | federation (`/sync/push` receive) | config | **[#1936 FED-RQ-01, v1.0.0]** Gates the INNER per-resolution signature requirement on inbound federated commit-checkpoint RESOLUTIONS (the `checkpoints` `/sync/push` subcollection). A resolved commit-checkpoint is an **authority-granting** write — the separation-of-duties freeze anchor (who resolved this coordination gate, to what verdict, when) that the epoch-apply verify-only consumer (#1878) later trusts — so it shares #87's authority-lane fail-closed posture, NOT the permissive data-lane default of #94/#96. When truthy (default) an inbound resolution is applied ONLY when its Ed25519 resolution signature verifies against the resolver's (`resolved_by`) locally-**enrolled** key (binds `resolved_by → enrolled key`; the wire `resolver_pubkey` is NOT trusted for the authoritative check — the #1718/#87 discipline). Unsigned / non-enrolled resolutions are refused (per-item skip; the batch survives). A **forged** signature (present but invalid against the enrolled key) is rejected UNCONDITIONALLY regardless of this knob. Set falsy (`0`/`false`/`no`/`off`) for a heterogeneous-rollout window (mirrors #87's escape-hatch shape). The receiver NEVER re-signs (v0.8.0 local-substrate rule); the sender's attestation is persisted verbatim. Application is idempotent under **first-resolution-wins**: a checkpoint already resolved locally with a *different* resolution is a per-item conflict (local kept, counted `checkpoints_conflicted`), never a batch drop. The `EpochAdvance` epoch-freeze checkpoint rides this transport (ROADMAP §25.2); this does NOT flip any epoch-enforcement default. **[#3075, v1.0.0 — this binding now holds on a postgres receiver]** this clause read "On a postgres-backed receiver the checkpoints table is not yet MemoryStore-trait-covered for a federated verbatim-resolution write, so the postgres funnel reports inbound checkpoints as `unsupported_on_postgres` (honest count, never a silent drop) — the sqlite / MCP-native path applies them fully." That was TRUE and describes a CLOSED gap. Lane L-PGP added `MemoryStore::apply_remote_checkpoint_resolution` (a method DISTINCT from `checkpoint_resolve`, which re-signs — a receiver stamping its own attestation onto another node's verdict is exactly what the freeze anchor must never allow), so a pg receiver now APPLIES federated resolutions and THIS KNOB IS LOAD-BEARING THERE. Consequence for the separation-of-duties story: the `EpochAdvance` freeze anchor finally replicates to a pg peer instead of stopping at it, so two nodes in one federation can no longer disagree about whether an epoch is frozen. The verdict chain is UNCHANGED and SHARED with the sqlite funnel — the resolved-only filter, the #2708 claimed-AND-stored namespace confinement (fail-closed on an unresolvable by-id probe), `receive_auth::authorize_remote_checkpoint_resolution` against the resolver's locally-ENROLLED key, the #3049 secret screen AFTER that verification, the L5 substrate-reserved-anchor refusal on BOTH the claimed and the stored kind, and first-resolution-wins (`checkpoints::classify_against_local`, now `pub(crate)` so the pg twin renders the identical disposition rather than re-deriving "what counts as an idempotent replay"). The receiver still NEVER re-signs on either backend: the CAS and the first-landing INSERT both write the sender's `signature` / `resolver_pubkey` verbatim, asserted byte-for-byte on pg by `tests/fed_checkpoint_lane_3075_pg.rs`. `checkpoints_applied` / `checkpoints_conflicted` join the postgres envelope and the lane leaves `unsupported_on_postgres`. Pinned on pg by six cells: enrolled+signed applies verbatim, forged signature refused even under the `=0` hatch, unenrolled refused under the default AND accepted under the hatch (so the knob is proven load-bearing, not merely present), L5 reserved kind refused, #2708 out-of-scope namespace refused with the in-scope half applied, and first-resolution-wins keeping the local verdict. Direct-read knob (not clap-bound / not a `[section]` field), so no `config_precedence` entry (mirrors #84/#85/#87/#88/#94/#96/#123). Format spine votes: `4d3ea1c5` (authority-write fail-closed) + #1947 decision `00d599ec` (head-entanglement rides checkpoint resolutions). Source: `src/federation/receive_auth.rs::{REQUIRE_CHECKPOINT_SIG_ENV,require_checkpoint_sig_enabled,authorize_remote_checkpoint_resolution}` + `src/handlers/federation_receive.rs` (checkpoints receive loop) + `src/checkpoints/mod.rs::apply_inbound_resolution`. |
...
| 130 | `AI_MEMORY_SECURITY_PROFILE` | enum (`standard`/`asi-hard`, case-insensitive) | `standard` (`SecurityPosture::Standard`) | CLI/daemon/MCP (boot posture enforcement) | config | **[#1961 R23/R7, v1.0.0]** Named security posture selector. `standard` (default / unset) keeps every security knob at its own current default. `asi-hard` engages the hardened, NO-DISABLE posture: at boot it PINS the fail-closed security knobs ON (unset → pinned to the hard value so every downstream read site honours the hard posture) and REFUSES to boot if an operator set any pinned knob BELOW its hard floor (the "no-disable" contract — fail-closed). Pinned set (SSOT `src/security_profile.rs::KNOBS`): `AI_MEMORY_SECRET_SCREEN_MODE`=refuse, `AI_MEMORY_REQUIRE_AGENT_ATTESTATION`/`AI_MEMORY_FED_REQUIRE_WRITE_SIG`/`AI_MEMORY_FED_REQUIRE_SIGNAL_SIG`/`AI_MEMORY_FED_REQUIRE_TRANSITION_SIG`/`AI_MEMORY_FED_REQUIRE_CHECKPOINT_SIG`/`AI_MEMORY_FED_REQUIRE_SIG`/`AI_MEMORY_FED_REQUIRE_NONCE`/`AI_MEMORY_FED_REQUIRE_PEER_ENROLLMENT`/`AI_MEMORY_FED_REQUIRE_PUSH_NAMESPACE_SCOPE`/`AI_MEMORY_FED_QUARANTINE_UNATTRIBUTED`/`AI_MEMORY_CID_ENFORCE`/`AI_MEMORY_REQUIRE_ROLLBACK_CHECK`/`AI_MEMORY_REQUIRE_WITNESS`/`AI_MEMORY_REQUIRE_CAUSE_BINDING`/`AI_MEMORY_REQUIRE_ROLE_SEPARATION`/`AI_MEMORY_REQUIRE_IDENTITY_LINEAGE`/`AI_MEMORY_FED_REQUIRE_SERVER_VERIFY`=1 (the four `FED_REQUIRE_SIG`/`_NONCE`/`_PEER_ENROLLMENT`/`_PUSH_NAMESPACE_SCOPE` gates added #3033 — the OUTER federation-TRANSPORT access controls: per-message Ed25519 sig + nonce freshness, enrolled-peer identity, inbound-write namespace confinement — all default-ON, so pinning them only removes the ability to DISABLE them; their `meets_floor` delegates to the live default-ON reader grammar, NOT `is_truthy`, per the NB1 false-red lesson) (that one being the FIRST network access-control knob in the pinned set, #2448 — before it the hardened PROCUREMENT profile pinned only crypto/attestation/durability knobs, so `docs/deploy/asi-hard.env` verbatim still permitted `ai-memory sync-daemon --insecure-skip-server-verify`), `AI_MEMORY_DB_SYNCHRONOUS`=FULL (#128), `AI_MEMORY_MIGRATION_REQUIRE_CORE_TABLES`=1 (#168, added #3113 — the FIRST schema-integrity pin: a certified deployment REFUSES to stamp a schema version whose core relations were lost rather than merely warning; safe to pin because refusal additionally requires a positively observed POPULATED corpus, so a fresh hardened node is never bricked) — PLUS the two PERMISSIVE-shaped pins whose hard floor is the INVERSE (the knob must be ABSENT, and setting it REFUSES boot rather than being honoured): `AI_MEMORY_ALLOW_SCHEMA_AHEAD` (#149, must be unset) and `AI_MEMORY_FED_ALLOW_PLAINTEXT_PEERS` (#154, must be non-truthy) — PLUS the three #3168 residual #3033 knobs: `AI_MEMORY_PERMISSIONS_MODE`=enforce (K3/K9 gate ON), `AI_MEMORY_GOVERNANCE_FAIL_OPEN_ON_ERROR` must be non-arming (PERMISSIVE-shaped; live grammar exact `1`/case-insensitive `true`), `AI_MEMORY_FED_REQUIRE_POLICY_CURRENT`=1 (#132, live `FED_`-prefixed name — the unprefixed `REQUIRE_POLICY_CURRENT` does not exist) — PLUS the two #3201 federation escape hatches: `AI_MEMORY_FED_ALLOW_UNENROLLED_PEERS` must be non-truthy (#44, hatch of the already-pinned `REQUIRE_PEER_ENROLLMENT`) and `AI_MEMORY_FED_CERT_PEER_BINDING`=enforce (#135; documented `standard` unset default stays `warn`). — PLUS the #3124 `AI_MEMORY_UNSTAMPED_MUTATION`=refuse (#183; a caller-scoped mutation of a row with no provable owner is refused on every funnel of both backends; the documented `standard` default stays `warn`) — PLUS the two #3813 secret-FILE lax-perms hatches (PERMISSIVE-shaped, must be UNSET/non-truthy): `AI_MEMORY_STORE_URL_FILE_ALLOW_LAX_PERMS` (#1927 store-url file channel) and `AI_MEMORY_AGENT_API_KEY_FILE_ALLOW_LAX_PERMS` (#3781 per-agent api-key token file) — a truthy value on either would accept a group/world-readable secret file, so it refuses boot. **[TRUTH-FIX, v1.0.0 lane-1 sweep; count updated #3033]** `KNOBS` now holds **30** entries (17 pre-#3033 + the four outer-transport gates + the #3113 schema-integrity gate + the three #3168 governance knobs + the two #3201 federation escape hatches + the #3124 unstamped-row mutation posture + the two #3813 secret-file lax-perms hatches). An earlier form of this enumeration listed only 15 of the then-17, omitting the two PERMISSIVE-shaped pins — while rows #149 and #154 each already declared themselves "PINNED by `asi-hard` (#130)", so the table contradicted itself and under-reported the hardened posture by one schema control and one network control. `docs/deploy/asi-hard.env` names all 30 correctly, and that claim is now ENFORCED knob-by-knob by `tests/deploy_templates.rs::asi_hard_env_names_every_pinned_knob` (#3113 — until then the template's only link to the SSOT was `!pinned_knobs().is_empty()`, true of any non-empty set, so a knob added to `KNOBS` could leave the operator-facing enumeration silently short). The count SSOT is the DERIVED `PINNED_KNOB_COUNT` (30) = `KNOBS.len()` in `src/security_profile.rs` — not a hand-bumped literal, which is what drifted: the module `## Pinned knobs` doc table sat TWO rows behind `KNOBS` for a full release. That table is pinned to `KNOBS` by SET equality (both directions) in `src/security_profile.rs::pinned_knobs_doc_table_matches_the_knobs_ssot_exactly`, and every PROSE citation of the count — this row included — is pinned by the `ASI_HARD_PINNED_KNOB_COUNT` rule in `scripts/check-docs-vs-ssot.sh`. PLUS the config-backed `[governance].require_operator_pubkey`=true (bridged at the governance boot check via `is_asi_hard`). An unrecognised token fails LOUD (boot error), not silent-standard. A garbage/loosening value aborts boot. See `PERFORMANCE.md` §"Hardened asi-hard security posture". Direct-read knob, no `config_precedence` entry. Source: `src/security_profile.rs::{ENV_SECURITY_PROFILE,SecurityPosture,enforce_at_boot,is_asi_hard}`. **[#3582 conditional federation check]** In addition to the unchanged fixed pin set, `src/federation/peer_posture.rs` requires valid `AI_MEMORY_FED_PEER_ATTESTATION` with explicit outbound peers or inbound bindings; malformed input and unreadable bindings refuse `asi-hard` boot. Valid `{}` permits boot while denying all peers; shared identity key enrollment or its read errors only warn in both postures. Standard warns. Ordinary doctor remains runnable, with local argv unobservable and the daemon snapshot reported remotely through capabilities v2/v3. **[#3700 shape DETECTOR]** The posture no longer has to be guessed from signals — the DECLARED `[deployment] shape` (#3714, `ai-memory config show`) pins it as a floor for `production`/`federated`/`hive`. #3700 adds the DETECTOR that keeps the declaration honest: at boot (`src/config/shape/detector.rs::{observe,assess_with,assess_pre_runtime,enforce_pre_open,assess_post_open}`) the node observes content-free shape signals — federation class: `outbound_peers` argv, `inbound_bindings` env, `listener_mtls` argv, `peer_allowlist` env (`AI_MEMORY_FED_PEER_ATTESTATION` set at all), `mcp_federation_forward_url` config; multi-agent class: `[wake_hub]` config, an `agent_registry` of 2+ agents read from the real store after it opens — and derives the least demanding shape they allow (`singleton` / `team` / `federated`). When that observed floor EXCEEDS the declared shape the boot WARNS once and RECORDS it (forensic audit kind `deployment_shape.undeclared_signals`; the `deployment_shape` field of capabilities v2/v3), naming the exact line to declare (`[deployment] shape = "<observed>"`) — promotion is an OPERATOR act, detection never re-postures and never pins. A HARDENED declared shape with any pinned knob set below its floor REFUSES to boot naming EVERY such knob (`hardened_shape_refusal`). A singleton with no signals boots byte-identically. `ai-memory doctor` never refuses: its DEFAULT report carries "Deployment shape detector (#3700)" right after the declared shape (declared vs observed floor, signals, registry, the promotion line, posture and origin, protections off, boot verdict) — the pre-upgrade detector. Signals a process cannot see (argv from doctor, the store before open) are `unobservable`, never `absent`. |
| 131 | `AI_MEMORY_INFERENCE_EGRESS` | enum (`allow`/`loopback-only`/`internal-only`/`deny`, case-insensitive) | `allow` (`InferenceEgressMode::Allow`, byte-identical legacy) | CLI/daemon/MCP (LLM + API-embedder construction) | config | **[#1963 R68/D14, v1.0.0]** Inference-plane egress-class gate. `allow` (default) permits any resolved LLM/embedding target; `loopback-only` permits only loopback/localhost targets (local Ollama / self-hosted TEI on `127.0.0.1`) and REFUSES external-vendor egress; `deny` refuses ALL inference egress (keyword-only posture — no memory content leaves the host for inference). `internal-only` (#3822, synonyms `internal_only`/`internal`) permits only targets whose EVERY resolved address is an internal address (loopback / RFC1918 / RFC4193 ULA / RFC6598 CGNAT, none link-local/multicast/broadcast/unspecified, none a known cloud-metadata literal such as `169.254.169.254` / `fd00:ec2::254` / `100.100.100.200` — after `normalize_ip`); it RESOLVES the target and PINS the boot-resolved addresses into the reqwest client (`resolve_to_addrs`) with redirects disabled and `.no_proxy()` (a pinned address a proxy never connects to would defeat the pin; a proxied internal endpoint is v1.x), and fails CLOSED on a DNS failure or any public / DNS-rebind address (`AI_MEMORY_SSRF_GUARD_ALLOW_DNS_FAIL` does NOT apply to this lane). A DNS change after boot requires a client swap (the hot-swap path re-admits + re-pins). ENFORCED at the boot chokepoints (`build_llm_client` / `build_embedder` + the MCP stdio init): on refuse the outbound client is NOT constructed (`None`), so no memory content can be POSTed to the refused vendor — enforcement is the absence of the egress path. A best-effort SIGNED refusal row (`signed_events.event_type = egress.inference_refused`, substrate-emitted / daemon-signed-when-enrolled) records the egress class + non-secret target when a db conn is reachable at the boot site. Only NETWORK inference is gated: the local in-process candle/MiniLM embedder never egresses and is never gated; the API-embed arm is gated on `is_api_embed_backend`. **The `[decision]` provider (#3806) is a THIRD gated lane:** `build_decision_provider` (`src/decision_boot.rs`) evaluates the SAME posture under its own egress class `inference_decision` — `internal-only` resolves-and-pins the decision endpoint exactly as the `[llm]` lane (#3822), `deny` constructs no provider and appends the signed refusal row, `local-nli` (in-process, no socket) is skipped rather than waived — and no new env var was added for it; `docs/CONFIG_SCHEMA.md` §`[decision]` carries the posture ladder. The `asi-hard` posture (#130) does NOT force this — the `asi-hard` config TEMPLATE (`docs/deploy/asi-hard.env`, #1962) sets `loopback-only` explicitly. UNSET → byte-identical-legacy `allow`; a SET-but-UNRECOGNISED value (a typo of a restriction intent) → WARN + fails CLOSED to `deny` (FBL-14, v1.0.0 pre-ship — an unrecognised token must NEVER silently widen egress; degrade-not-crash keeps the boot manageable). Direct-read knob, no `config_precedence` entry. Source: `src/egress.rs::{ENV_INFERENCE_EGRESS,InferenceEgressMode,EgressClass,evaluate_inference_egress,evaluate_inference_egress_resolved,admit_inference_target,resolve_inference_egress_mode}` (the #3822 internal classifier reuses the `src/subscriptions.rs` SSRF sub-predicates `is_rfc1918_or_ula`/`is_cgnat`/`is_link_local_or_special`/`is_metadata_literal`). |
| 132 | `AI_MEMORY_FED_REQUIRE_POLICY_CURRENT` | bool (`1`/`0`/true/false) | `1` (fail-closed for a DETECTED-stale value, v1.0.0 secure default) | federation (`/sync/push` receive) | config | **[#1947 FED-RQ-03, v1.0.0]** Cross-node governance `policy_version` REFUSE-STALE gate. When truthy (default) the receive path refuses an inbound federated push whose advertised `sender_policy_seq` is STRICTLY BEHIND the local committed governance policy (`crate::governance::policy_version::current_policy_version().seq`) with a typed `409 stale_policy_version` — a receive-path refusal (reject-before-apply) that touches NO `MemoryStore` checkpoint-apply path (postgres-clean, independent of #1990) and fails closed IDENTICALLY on sqlite + postgres. **Fail-closed means DETECTED-stale only:** an ABSENT / undeterminable epoch (a peer that does not advertise `sender_policy_seq`, or a transient local policy-read fault) is fail-OPEN (accepted — staleness cannot be determined), so existing federation is NOT hard-refused by this new gate (there is no prior v0.10.0 WARN carrier). Reserve refusal for a strictly-lower advertised seq. Set a falsy token (`0`/`false`/`no`/`off`) to accept stale-policy pushes during a deliberate heterogeneous-governance-policy rollout window. Mirrors the fail-closed escape-hatch shape of `AI_MEMORY_FED_REQUIRE_CHECKPOINT_SIG` (#125) / `AI_MEMORY_FED_REQUIRE_TRANSITION_SIG` (#87) via the shared `env_flag_default_on` grammar. The authoritative ATTESTED epoch (the signed `SignableEpochManifest` `(policy_seq, policy_digest_hex)`) rides the DEFERRED epoch-manifest-doc federation (ADR-002); this gate uses a minimal ADDITIVE unsigned `sender_policy_seq` wire field to refuse an HONEST stale peer that advertises. Direct-read knob (not clap-bound / not a `[section]` field), so no `config_precedence` entry (mirrors #84/#85/#87/#88/#94/#96/#123/#125). Vote `wd8wtmg0n` (A_MINIMAL). PINNED ON by `asi-hard` (#130 / #3168). Source: `src/federation/receive_auth.rs::{REQUIRE_POLICY_CURRENT_ENV,require_policy_current_enabled,evaluate_inbound_policy_freshness}` + `src/handlers/federation_receive.rs::refuse_if_stale_policy`. |
| 133 | `AI_MEMORY_ALLOW_PLAINTEXT_NONLOOPBACK` | bool (`1`/`true`, case-insensitive) | `false` | daemon (bind guard) | **operator-advisory** (config) | **[#2032 M2, v1.0.0; WIRED at tranche 3, 5-agent vote `4d3ea1c5`]** "TLS terminated upstream" acknowledgement escape hatch: when truthy the operator asserts a reverse proxy / service-mesh sidecar terminates TLS in front of the daemon, so a non-loopback bind without in-process TLS is an accepted posture. **Tranche 3 WIRES the consuming `tls_bind_guard`:** a plaintext non-loopback bind (no `--tls-cert`/`--tls-key`) now emits a HARD boot WARN naming the cleartext-off-host exposure + the escape hatches; setting this env truthy SILENCES that WARN (accepts the plaintext bind). Loopback binds are exempt. Binding is NOT refused this release — the REFUSE flip for unacknowledged plaintext non-loopback binds lands v1.1.0. Default `false`. Mirrors the truthy grammar + escape-hatch shape of `AI_MEMORY_REQUIRE_API_KEY` (#56). Direct-read knob (not clap-bound / not a `[section]` field), so no `config_precedence` `[limits]`-style entry — the precedence test lives alongside the resolver. Source: `src/daemon_runtime.rs::{ENV_ALLOW_PLAINTEXT_NONLOOPBACK,allow_plaintext_nonloopback_enabled,tls_bind_guard}`. **[#3705 REMOVED downgrade path, v1.0.0]** Under the operator mandate "only encrypted data in transit" (loopback included) this acknowledgement can never open a plaintext bind again: `allow_plaintext_nonloopback_enabled()` always answers `false`, `tls_bind_guard` refuses EVERY bind without in-process TLS (loopback too), and a TRUTHY value here REFUSES boot (`src/transit_encryption.rs::{REMOVED_DOWNGRADE_ENVS,enforce_no_downgrade_paths}`) — a reachable downgrade path is the defect, not its default. Keep it unset; `ai-memory doctor` ("Transit encryption (#3705)") reports it armed before the upgrade refuses. |
| 134 | `AI_MEMORY_REQUIRE_TLS` | bool (`1`/`true`, case-insensitive) | `false` | daemon (bind guard) | config | **[#2032 M2, v1.0.0; WIRED at tranche 3, 5-agent vote `4d3ea1c5`]** Fail-closed-now TLS opt-in: when truthy the operator demands in-process TLS on every bind and a plaintext bind is REFUSED. **Tranche 3 WIRES the consuming `tls_bind_guard`:** when this is truthy and no `--tls-cert`/`--tls-key` pair is configured, `bootstrap_serve` refuses to start (fail-closed-now) on ANY host, including loopback. Default `false`. Mirrors the truthy grammar of `AI_MEMORY_REQUIRE_API_KEY` (#56). Direct-read knob; precedence test lives alongside the resolver. Source: `src/daemon_runtime.rs::{ENV_REQUIRE_TLS,require_tls_enabled,tls_bind_guard}`. **[#3705 FLOOR, v1.0.0]** Since the "only encrypted data in transit" mandate this is a FLOOR, not an opt-in: unset and every canonical truthy token (`1`/`true`/`yes`/`on` via `security_profile::is_truthy` — the ONE grammar; the pre-#3705 `1`/`true`-only reader silently left TLS optional on `yes`/`on`) mean in-process TLS is REQUIRED on every bind, loopback included; `require_tls_enabled()` always answers `true`. A falsy token (`0`/`false`/`no`/`off`) is a downgrade request and REFUSES boot; any other token is unrecognised and REFUSES boot rather than proceeding in cleartext (FBL-14). Source: `src/transit_encryption.rs::{ENV_REQUIRE_TLS,require_tls_token,enforce_require_tls_token}`; the refusal names the plaintext path (`plaintext_listener_refusal`). |
| 135 | `AI_MEMORY_FED_CERT_PEER_BINDING` | enum (`off`/`warn`/`enforce`, case-insensitive) | `warn` (`CertPeerBindingMode::Warn`) | federation (inbound mTLS `/sync/*` receive) | config | **[#2045 L6, v1.0.0]** Enforcement posture of the mTLS client-cert ↔ `X-Peer-Id` cross-check — the compensating control for the `AI_MEMORY_FED_REQUIRE_SIG=0` (#29) window, **independent of** it. `enforce` refuses a cert↔peer-id mismatch with `401 peer_id_cert_mismatch`; `warn` (default, one release then flip to `enforce`) logs and proceeds; `off` restores the pre-#2045 verbatim-`X-Peer-Id` behaviour. Inert unless `AI_MEMORY_FED_CERT_PEER_BINDING_MAP` (#136) is configured (no map ⇒ the peer-binding acceptor is not installed ⇒ no `ClientCertPeerId` extension ⇒ the check is skipped — the control is a no-op without a map even when the mode is `enforce`; `enforce` does brick an unbound client cert *when a map is configured*). Read per request (direct-read knob; no `config_precedence` entry, mirrors #87/#94). Composes with the #1056 TOFU allowlist (strictly stronger). Unrecognised / empty value is a parse error (fail-loud #3289; empty assignment does **not** silently become `enforce`). UNSET still defaults to `warn` — the documented `standard` posture default is unchanged; only `asi-hard` (#130 / #3201) pins to `enforce`. config.toml advisory is silently overridden by the env pin, not refused. The enforce pin can partition an existing mTLS mesh that has no binding map. Source: `src/tls.rs::{FED_CERT_PEER_BINDING_ENV,CertPeerBindingMode,cert_peer_binding_mode}` + `src/handlers/federation_receive.rs::enforce_cert_peer_binding`. |
...
| 147 | `AI_MEMORY_FED_REQUIRE_PUSH_NAMESPACE_SCOPE` | bool (`1`/`0`/true/false) | `1` (fail-closed without an allowlist or for an enrolled peer with no scope) | federation (`/sync/push` receive) | config | **[#2447, v1.0.0]** Layer 2 of the inbound-WRITE namespace confinement (CWE-284). The federated write lane was the ONE lane with no namespace scoping: the pull lane (`/sync/since`) filters its projection through the peer's `allowed_namespaces` and the delete lane resolves each target row's namespace (#1934 — but see the #2488/#2491 truth-fix below: that delete gate was itself broken in OPPOSITE directions on the two backends), but the `memories[]` loop consulted NEITHER — so a peer attested with `allowed_namespaces: ["public/*"]` could push a memory whose `namespace` is `secure/ops` and have it persisted, then served to that tenant's agents as substrate truth. **Layer 1 (always-on, NOT gated by this knob)** enforces a peer's DECLARED non-empty `allowed_namespaces` against BOTH the inbound CLAIMED namespace and the STORED namespace of any local row with the same `id` — the second check is load-bearing, not belt-and-braces: `merge_memory` resolves `namespace` by LWW and `merge_inbound` field-merges any same-`id` inbound row, so a claimed-only gate would let a `public/*` peer RELOCATE + content-clobber a `secure/ops` row by pushing its id under an in-scope namespace. **[#3582] This knob also gates an ABSENT allowlist:** default-on refuses inbound namespace mutations when no `AI_MEMORY_FED_PEER_ATTESTATION` exists. Under a configured map it governs Layer 2 for an enrolled peer whose `allowed_namespaces` is empty/absent (`#[serde(default)]`). Enabled unless an explicit falsy token, it refuses that peer's writes too. Key enrollment alone grants no namespace authority. **[TRUTH-FIX #2488/#2491, v1.0.0]** this row previously also claimed the delete lane already meant "empty = may delete nothing" and that the flip made "the three lanes agree" — BOTH were false at the time of writing. On sqlite the delete lane hard-coded deny-on-empty via a verbatim unconditional `namespace_allowed` call, so it was not governed by this knob at all AND it refused every ZERO-CONFIG deletion as well (a silent delete-replication outage, #2491 — no DLQ, no retry, permanent replica divergence behind an HTTP 200). On postgres the delete gate was wrapped in the read-ELISION predicate `inbound_write_needs_existing_namespace`, which returns `false` for exactly the enrolled-unscoped peer Layer 2 exists to refuse, so the gate was structurally UNREACHABLE for that shape and an enrolled peer refused every write could still hard-delete anything by id (CWE-284, #2488). Since #2488/#2491 both delete funnels use the shared `receive_auth::inbound_by_id_namespace_authorized`. #3582 removes the caller-side absent-allowlist bypass too: default denial now covers absent authorization as well as enrolled-unscoped peers on BOTH backends. Fable 5 1x7 vote (`4d3ea1c5`), 7/7. **Upgrade behavior (#3582):** absent-map namespace acceptance now requires the explicit Standard opt-out `AI_MEMORY_FED_REQUIRE_PUSH_NAMESPACE_SCOPE=0`. Leaving the map absent with the default requirement refuses writes; removing a map alone is no longer a recovery. With configured peers, Standard still warns about the missing map, and `asi-hard` requires a valid map at boot (`{}` declares no peers) and forbids the opt-out. Two recoveries: declare the peer's real scope, or `["**"]` for a deliberate PER-PEER allow-all (`glob_match` treats `**` as unconditional match-all) — preferred, since it exempts ONE peer; or set this knob falsy (`0`/`false`/`no`/`off`) for a fleet-wide rollout window. Note `AI_MEMORY_FED_SYNC_TRUST_PEER=1` (#25) does NOT rescue this case and never did: `namespace_allowed` consults that bypass only in the `scope_for(peer) == None` arm, so an enrolled peer's declared allowlist always wins over the legacy override. Shares the `env_flag_default_on` grammar with #87/#94/#96/#125/#132. Direct-read knob (not clap-bound / not a `[section]` field), so no `config_precedence` entry (mirrors #84/#85/#87/#94/#96/#123/#125/#132) — the resolver test is co-located in `src/federation/receive_auth.rs`. 3x3 adversarial vote (`4d3ea1c5` protocol): the permissive-default variant was ranked LAST by both voting lenses. **[#2497 KNOB-CONTRACT + AVAILABILITY, amended #3582]** within a configured allowlist this knob governs an ENROLLED peer whose `allowed_namespaces` is empty; #3582 additionally governs the absent-allowlist shape. The other two shapes that reach Layer 2 under an enrolled posture (**header-absent**, including a whitespace-only header that `extract_peer_id` collapses to `None`; and **peer not in a non-empty allowlist**) are refused UNCONDITIONALLY by `receive_auth::peer_enrolled_in_allowlist` BEFORE this knob is read. The first #2488 revision routed all three through the knob, so `=0` briefly turned a per-shape rollout hatch into an ANONYMOUS-delete grant. Because that refusal lives in the SHARED Layer 2 it is not delete-only: for `{allowlist configured} × {header-less legacy peer} × {=0}`, `memories[]`/`archives[]`/`restores[]` entries that were APPLIED before are REFUSED now — correct direction, but an operator on exactly that posture loses WRITES as well as deletes on upgrade, silently, inside an HTTP 200 with no DLQ enqueue (#2498). So **`=0` is NOT a general rollout hatch.** Remedy: make the peer identifiable (enrol it; `["**"]` for a deliberate per-peer allow-all) or, under Standard only, deliberately select the absent-map posture AND explicit require-scope `0` opt-out — for a header-LESS push "enrol the peer" is not actionable, since a per-peer scope cannot be applied to a peer that cannot be identified. Note also that `inbound_write_needs_existing_namespace` is an ELISION predicate ("may I skip the probe?"), NEVER a gate predicate ("must I check this peer?"); conflating the two IS the #2488 defect, and the probe elision is preserved only where the stored namespace provably cannot change the verdict, so the fix costs ZERO extra reads. The namespace probe on BOTH backends is now the SCALAR projection (`db::namespace_by_id` / `MemoryStore::namespace_by_id`), never a full-row `get`: the full-row mappers are pinned to `DecryptFailurePolicy::FailClosed`, so a gate built on `get` made a row with an unopenable at-rest envelope PERMANENTLY un-erasable by federation, with no operator escape hatch (`AI_MEMORY_STRICT_DECRYPT_READS` #146 only hardens, never relaxes). An unresolvable-row refusal now carries the distinguishable cause token `namespace_probe_unresolvable` so an un-erasable row is not an indistinguishable `skipped` in a 200. **[#2478 GOVERNANCE LANES, v1.0.0]** the knob now also governs the `/sync/push` GOVERNANCE lanes `pendings[]` + `pending_decisions[]`, which reached `db::execute_pending_action` — an arbitrary-namespace `insert` / `delete` / `promote_to_namespace` / `reflect` — with NO namespace check whatsoever (`pending_author_authorized` inspects only `requested_by` + the payload's `metadata.agent_id`). The subject there is the UNION of every namespace the execution would touch, NOT the pending row's declared `namespace`, because `execute_pending_action` never reads that field: the payload's `namespace` (store/reflect) + `to_namespace` (promote CLONES, so the destination is a write) as CLAIMED, and the stored namespace of `memory_id` (delete/promote) + of every `source_ids[i]` (reflect edges) as BY-ID. An unknown `action_type` is default-deny. The original #2478 implementation excluded REJECT; #2532 added its scope gate. #3582 pins default/explicit-require denial and scoped/Standard-opt-out allowance for both APPROVE and REJECT, including local-decisions-only payloads, with persisted-state assertions on both backends. **[#3075, v1.0.0 — the pg funnel now APPLIES these two lanes]** this clause read "Postgres is unaffected: `sync_push_via_store` buckets both subcollections into `unsupported_on_postgres` (a sender-side non-ack) and never reaches a pending-execution path". That was TRUE and describes a CLOSED gap. `sync_push_via_store` now runs both loops: `pendings[]` upserts through the new `MemoryStore::apply_remote_pending_action` and `pending_decisions[]` reaches `approve_with_approver_type` -> `execute_pending_action` (APPROVE) or `pending_decide(false)` (REJECT), so federated governance finally converges on a pg peer instead of stopping at it. EVERY gate the sqlite loop runs runs there too, in the same order: the #2529 terminal-status refusal on the wire row AND the locally-decided-row clobber refusal (belt-and-braces on pg, where the upsert SQL additionally hard-codes `status = 'pending'` and its `ON CONFLICT` arm fires only `WHERE pending_actions.status = 'pending' AND requested_by = EXCLUDED.requested_by`), the #1920 authorship gate, the #2478 effect-namespace gate, the #3278 payload secret screen, the hardened approve (self-approval refusal + registered approver + approver-type policy — on pg that is `governance_approve_with_consensus`, which the trait's `approve_with_approver_type` forwards to and which shares the #3448 eligibility predicate with the sqlite `ApproveSurface::Http` funnel), and on REJECT the #2532 scope gate with the #2720 decider rebinding. The #2478 VERDICT is now literally one function: `pending_namespaces_authorized` was split so `pending_namespaces_authorized_resolved` renders the decision from PRE-RESOLVED by-id probes, and each backend supplies only the probe (`db::namespace_by_id` vs `MemoryStore::namespace_by_id`). A by-id target with no probe entry is treated as UNRESOLVABLE and fails CLOSED, so a caller that forgets to probe refuses rather than passes, and the #2488 elision is preserved (Layer 1 unarmed ⇒ no probes ⇒ zero extra reads). `pendings_applied` / `pending_decisions_applied` join the postgres envelope. Pinned by `tests/federation_pending_ns_scope_2478_pg.rs`, whose two exploit cells are now LIVE (they run against a lane that really applies) plus three added cells: in-scope pending upserts AND its approval EXECUTES the payload row, a terminal-status injection is refused, and a replay cannot clobber a locally-decided row. Still a claim about the FEDERATED lanes only, since the LOCAL approve surfaces reach the same paths on pg under local authz. **[#2479 GOVERNANCE-STANDARD LANES, v1.0.0]** the knob ALSO governs the `/sync/push` `namespace_meta[]` + `namespace_meta_clears[]` lanes, which had NO scope gate at all — they validated `validate_namespace` / `validate_id` and called `db::set_namespace_standard` / `db::clear_namespace_standard` directly, so a peer could rebind or re-parent ANY namespace's governance standard (an authz-CONFIG takeover: the row selects the standard memory whose `metadata.governance` `resolve_governance_policy` returns, rewriting the rules every subsequent LOCAL write to that namespace is judged against) and the clears lane is the same reach destructively. The subject is the UNION of the row's `namespace` AND its declared `parent_namespace` — the parent is persisted into the chain `build_namespace_chain` walks, so it changes which OTHER namespace's policy governs BY REFERENCE. No probe is needed and the verdict (`inbound_namespace_meta_authorized`) is BACKEND-BLIND, because `namespace_meta`'s PRIMARY KEY is the namespace itself: there is no id under which a row could be relocated, so the #2447 stored-vs-claimed split has no subject here. **Amendment E (5/5 unanimous) is NOT governed by this knob under a configured allowlist:** a `namespace_meta` row on the literal global standard `*` is refused unless the peer declared `**` (checked LITERALLY via `peer_scope_is_allow_all`, before the layered verdict). `validate_namespace("*")` passes and `build_namespace_chain` prepends `*` to EVERY namespace's chain, while `glob_match("*", "*")` is `true` — so a scope of `["*"]`, documented as "any TOP-LEVEL namespace", would otherwise confer governance authority over every DEEP namespace the peer may not write, re-opening the #1902 whole-tree widening by another door; a per-peer rollout hatch must never be the lever that hands over the substrate-wide governance default. Refusing on the STORED parent was considered and rejected (it would BRICK the row permanently un-updatable, and entries apply in wire order so it is launderable in-batch); instead a verdict-INDEPENDENT WARN reports a severed out-of-scope inheritance link, read through `db::get_namespace_meta_entry` (note BOTH namespace_meta accessors collapse a read error into "no row" today — `get_namespace_parent` via a bare `.ok()`, `get_namespace_meta_entry` via `#[allow(clippy::unnecessary_wraps)]` — so that WARN's `Err` arm is unreachable and a storage fault silently costs the log line; the former is preferred only because it returns the whole row and its signature can already carry an error). Refusals increment the additive `namespace_meta_refused` response counter rather than folding into `skipped`, because this funnel has no DLQ (#2498) and the sender is the only party that can retry. NOT closed: a peer scoped to an ANCESTOR still supplies the DEFAULT policy of out-of-scope DESCENDANTS (leaf-first-wins resolution; closing it needs pattern-vs-pattern subsumption `glob_match` cannot express), and `standard_id` may still bind a memory in any namespace (the documented shared-policy feature). **[#3075, v1.0.0 — the pg funnel now APPLIES these two lanes]** this clause read "Postgres is unaffected: `sync_push_via_store` buckets both subcollections into `unsupported_on_postgres` and never calls the two trait methods", which was TRUE and is now the description of a CLOSED coverage gap, not of the shipped funnel. `sync_push_via_store` runs both loops and writes through `MemoryStore::{set,clear}_namespace_standard` on BOTH adapters, so a federated governance-standard rebind finally replicates to a postgres peer instead of leaving two nodes answering `get_standard` with divergent policy. The verdict is UNCHANGED and SHARED: `inbound_namespace_meta_authorized` is backend-BLIND (it takes no connection and performs no probe, because `namespace_meta`'s PRIMARY KEY is the namespace itself), so the pg funnel calls it VERBATIM — Amendment E's unconditional global-`*` refusal, the own-namespace + declared-parent union, and the #2536 descendant probe all hold identically, and there is no second implementation for the two backends to drift apart on (the #2488 lesson). The pg apply runs on `federation_apply_ctx` (a `for_admin` bypass, C8-allowlisted): the sqlite twin calls the RAW `db::*` functions which consult no caller identity, so routing the federated apply through a tenant-scoped context would make the pg receiver refuse rows sqlite applies — the peer-scope gate is the authorization on this lane, not the local #1777/#2545 owner gate, which governs the TENANT surfaces only. Refusals are sender-visible in the additive `namespace_meta_refused` counter (alongside `namespace_meta_applied` / `namespace_meta_cleared`) and the lane no longer inflates `unsupported_on_postgres`. Pinned on BOTH backends by `tests/federation_ns_meta_scope_2479.rs` + `tests/federation_ns_meta_scope_2479_pg.rs` (each cell asserts the refused AND the applied half — asserting only the refusal would pass on the pre-#3075 funnel that applied nothing at all). Still a claim about the FEDERATED lanes only: `hook_subscribers` + the MCP namespace tools reach the same trait methods on pg under LOCAL authz. Source: `src/federation/receive_auth.rs::{REQUIRE_PUSH_NAMESPACE_SCOPE_ENV,require_push_namespace_scope_enabled,inbound_write_namespace_authorized,inbound_by_id_namespace_authorized,inbound_namespace_meta_authorized,peer_declares_namespace_scope,peer_enrolled_in_allowlist,peer_scope_is_allow_all,lane_is_destructive,CAUSE_NAMESPACE_PROBE_UNRESOLVABLE,GLOBAL_STANDARD_NAMESPACE,ALLOW_ALL_NAMESPACE_PATTERN,LANE_DELETIONS,LANE_PENDINGS,LANE_PENDING_DECISIONS,LANE_PENDING_DECISION_DELETE,LANE_NAMESPACE_META,LANE_NAMESPACE_META_CLEARS}` + `src/handlers/federation_receive.rs` (sqlite memories/deletions/archives/restores loops + the #2478 `pendings`/`pending_decisions` loops via `pending_action_effect` / `pending_namespaces_authorized` + the #2479 `namespace_meta`/`namespace_meta_clears` loops via `warn_on_severed_out_of_scope_parent`) + `src/handlers/federation_signing_check.rs` (postgres twin, including the #3075 governance and governance-standard apply loops) + `src/store/mod.rs::MemoryStore::namespace_by_id`. |
...
| 154 | `AI_MEMORY_FED_ALLOW_PLAINTEXT_PEERS` | bool (`1`/`true`/`yes`/`on`, case-insensitive) | unset (= the plaintext-peer refusal is in force) | federation (`serve --quorum-peers` + `sync-daemon --peers`) | **operator-advisory** (config) | **[#2477, v1.0.0; 3x3 adversarial vote, option D 6/9, citing `4d3ea1c5`]** Staged-rollout escape hatch for the plaintext-federation-peer refusal. **The defect it closes:** `FederationConfig::build` formatted the raw operator `--quorum-peers` string straight into `PeerEndpoint::sync_push_url` with NO scheme validation, and `cli::sync::build_sync_client` (`sync-daemon --peers`) — a SECOND, fully independent peer-URL door — did the same. Federation replicates memory content that is NOT end-to-end encrypted (`src/encryption/mod.rs`, #1968 open), so `http://peer:9077` shipped tenant memory in the clear to anyone on the path. That bypassed with ZERO ceremony the four-condition opt-in #2448 built for the strictly WEAKER accept-any-server-cert case, and `docs/encryption.html` asserted unqualified that peer traffic "travels over mutual TLS ... TLS 1.3 only (no fallback)". **The guard** (`tls::validate_peer_url_scheme`, ONE validator called from BOTH doors — a fix scoped to `federation/peer.rs` alone would have been theatre): `https://` always accepted; `http://` to a LITERAL loopback host (`127.0.0.1`/`::1`/`localhost`/`[::1]`/`0:0:0:0:0:0:0:1`, the SSOT `tls::host_is_loopback` the inbound `tls_bind_guard` now shares) always accepted with no hatch, because the bytes never leave the kernel and forcing a hatch-flip on every dev mesh and CI fixture would train reflexive hatch use; `http://` to anything else REFUSED unless this knob is truthy; any other scheme, and a scheme-LESS `peer.example:9077` (previously accepted, then failing opaquely at request time), REFUSED. A container-bridge hostname such as `http://alice:9077` is NOT loopback — it crosses an interceptable virtual NIC, and treating "feels local" as "is local" is the category error the control exists to prevent (this repo's own `infra/plan-c` + `infra/lan-parity-test` fleets use that shape and therefore carry an explicit hatch line). **Refusal is WHOLE-BOOT, never per-peer skip-and-continue:** `n = 1 + peer_urls.len()` feeds `QuorumPolicy::new`, so dropping a peer would change the quorum guarantee without saying so, and `PeerEndpoint.id` is a positional index (#2442) so a shrunken list also re-keys every DLQ row above the gap. PERMISSIVE-knob grammar (default OFF): opened ONLY by an explicit truthy token — unset, empty, `0`, or an unrecognised word all KEEP the refusal (the #131/FBL-14 rule: an unrecognised token must never silently widen a security control). The refusal message names the SECURE remedies (`https://`, `--quorum-ca-cert`, `AI_MEMORY_FED_PEER_FINGERPRINTS`) BEFORE the hatch, pinned by an ordering assertion, so an operator is steered to fix the posture rather than disable the control. **PINNED by `asi-hard`** (#130) as the SECOND permissive knob in that SSOT (after `AI_MEMORY_ALLOW_SCHEMA_AHEAD`) and the SECOND network access-control pin (after `AI_MEMORY_FED_REQUIRE_SERVER_VERIFY`, #148): its hard floor is "not truthy", so a hardened deployment refuses to boot with the hatch on. Direct-read knob (not clap-bound / not a `[section]` field), so no `config_precedence` entry (mirrors #85/#148/#149) — the resolver tests are co-located in `src/tls.rs`. Source: `src/tls.rs::{FED_ALLOW_PLAINTEXT_PEERS_ENV,plaintext_peers_allowed,host_is_loopback,validate_peer_url_scheme}` + `src/federation/mod.rs::FederationConfig` (`impl` in `src/federation/peer.rs`)` + `src/cli/sync.rs::build_sync_client` + `src/security_profile.rs::KNOBS`. **[#3705 REMOVED downgrade path, v1.0.0]** Under the "only encrypted data in transit" mandate `validate_peer_url_scheme` refuses EVERY `http://` peer — the literal-loopback exemption is gone (loopback is shared by every local process; "peer is loopback" is not "peer is trusted", the #2502 ruling) — and this hatch can never reopen it: `plaintext_peers_allowed()` always answers `false` and a TRUTHY value here REFUSES boot in every posture (`src/transit_encryption.rs::enforce_no_downgrade_paths`), not only under `asi-hard`. Keep it unset. |
| 155 | `AI_MEMORY_RERANK_BUDGET_MS` | u64 ms (tri-state: unset = default; explicit `0` = DISABLED) | `2000` (`RERANK_BUDGET_MS_DEFAULT`) | CLI/daemon/MCP (autonomous-tier recall rerank) | config | **[#2608, v1.0.0; 5-agent vote `4d3ea1c5`]** Wall-clock budget for the cross-encoder RERANK stage — the DATA-lane sibling of the #2577 recall-embed budget (`AI_MEMORY_RECALL_EMBED_BUDGET_MS`, #152). **The defect it closes:** on the reference corpus the rerank forward is the LARGER read-path term (~1,063 ms of a ~1,290 ms autonomous p50; a long-content corpus pushed warm recall to ~4 s, #1588), and left unbounded that is an AVAILABILITY defect exactly as #2577 documents for the query embed (MCP stdio's single-threaded JSON-RPC loop stalls EVERY subsequent tool call; the HTTP daemon holds an admission permit per recall, #2032 M3, so sustained rerank latency sheds HEALTHY traffic incl. durable-truth writes with 503s). **Enforced PRE-FLIGHT, not mid-flight:** a candle BERT forward is a synchronous CPU call with NO cancellation point and the pool cap (`RERANK_POOL_MAX`) makes each rerank ONE forward over ≤ 20 candidates (no mid-flight checkpoint), so a wall-clock timeout could only abandon the waiting thread while the forward keeps burning a core — strictly worse at fleet scale (latency → CPU-exhaustion, firing hardest when the fleet is busiest). Instead the forward cost is ESTIMATED before it starts (candidate count × padded token length, both known up front; token length proxied byte/`RERANK_EST_BYTES_PER_TOKEN` so the estimate is a pure, model-free, deterministic function; coefficient `RERANK_FORWARD_NANOS_PER_PAIR_TOKEN` grounded in the ~1,063 ms reference, a NAMED calibratable const, documented as an ESTIMATE not a measured guarantee), and when the estimate exceeds the budget the neural stage is SKIPPED and the recall ships the pre-rerank HYBRID ordering — deterministic (no load-dependent ordering), never starting an over-budget forward. The budget GRAMMAR is copied verbatim from #2577; the ENFORCEMENT (pre-flight admission vs #2577's mid-flight future-drop) differs by necessity and was resolved by the 5-agent vote (`4d3ea1c5`), 4-1 for pre-flight admission. On a degrade the recall stays hybrid (fewer, FTS/semantic-ranked results, NOT cross-encoded) and the operator's `AI_MEMORY_RERANK_SCORE_FLOOR` (#77) is STILL applied — the floor lives in the outer `BatchedReranker::rerank`, so a skip cannot silently return rows the operator configured to be dropped (note the floor was calibrated on the BLENDED post-rerank score; on the hybrid degrade scale `RerankerScoreFloor::RelativeToTop` is scale-invariant while `Absolute` is scale-sensitive). That is a DEGRADE, never a wrong result — the durable memory text is untouched and recall is pure (#1869/#1953). Tri-state (mirrors `AI_MEMORY_MAX_INFLIGHT_REQUESTS` #79 / #2577): unset ⇒ 2000; explicit `0` ⇒ DISABLED (pre-#2608 unbounded); unparseable ⇒ default + WARN (never silently WIDEN the failure window, the #131/FBL-14 rule). The seam that makes this CI-provable is the pluggable `PairScorer` trait (impl'd by `CrossEncoder`, mirroring the shipped `crate::embeddings::Embed`/`Embedder` pattern) — a CI host always degrades `CrossEncoder::new_neural()` to lexical, so a test double is the only way to drive `neural_score_pairs` + the budget branch without a BERT model. Observability: `ai_memory_rerank_budget_degraded_total` counter + the `rerank.budget.degraded` WARN (the only channel on MCP stdio, which serves no `/metrics`). Direct-read cached knob (the `strict_dim_enabled` / `recall_embed_budget` shape), deliberately NOT boot-seeded (the #2233 defaults-lie class); no `config_precedence` `[limits]`-style entry — the resolver tests are co-located (mirrors #92/#146/#148/#149/#152). Source: `src/reranker.rs::{ENV_RERANK_BUDGET_MS,RERANK_BUDGET_MS_DEFAULT,rerank_budget,resolve_rerank_budget_ms,estimate_rerank_forward_ms,PairScorer,degrade_to_hybrid_ordering}` + `src/reranker.rs::BatchedReranker::rerank_unfloored`. |
| 156 | `AI_MEMORY_CAPABILITIES` | enum (`on`/`off`, case-insensitive; also `1`/`0`/`true`/`false`/`yes`/`no`) | unset (config value stands; `[capabilities].enabled` compiled default `true` at v1.0.0, R9 #1960 — was `false` through v0.9.0) | CLI/daemon/MCP | config | **[#1827 G10.1, v0.9.0; #1960 R9 default-on + zero-config owner, v1.0.0]** Operator override for `[capabilities].enabled` — the master on/off for the stateless macaroon capability-token grant layer (`src/governance/capability.rs`). Mirrors the enum-resolve shape of `AI_MEMORY_SECRET_SCREEN_MODE` (#95): a recognised `on`/`off` (or `1`/`0`/`true`/`false`/`yes`/`no`) token wins; any other string falls through to the config value, which itself now defaults to `true` (`capability::DEFAULT_CAPABILITIES_ENABLED`). **R9 default-on is ADDITIVE-ONLY — it changes NOTHING for a capability-LESS caller** (the gate hook short-circuits `token.is_none() || base==Allow` BEFORE it consults `enabled`, so a token-less request is byte-identical to the legacy inert posture and adds ZERO new denials; only a caller that actively presents a token pays any extra work). When on, a valid capability token whose caveat chain + issuer ceiling cover an in-flight `(action, namespace)` flips an otherwise-`Deny`/`Ask`(pending) coarse-gate decision to `Allow` (attenuation-only widening; the closed `[capabilities.issuers]` allowlist is the sole issuer source — NEVER the `db::agent_pubkey` registry — PLUS the R9 reserved zero-config `owner` issuer, auto-enrolled from on-disk custody `owner.priv`/`.pub` + `owner.caproot` with an Admin ceiling once `ai-memory capability init` has run). The issue's "deny-semantics" reading (refusing a capability-LESS caller by default) is a gate-TIGHTENING flip and is deliberately NOT enabled here — it rides the v0.10.0 WARN cycle (a one-shot boot note in `AppConfig::load_capability_config` documents this). Revocation is short `ExpiresAt` + `root_secret` rotation (no online-revocation store). **Security boundary:** the macaroon caveat-chain non-escalation (a `verify` accept ⟹ request ⊆ every ancestor grant) is ATTESTABLE (HMAC + Ed25519); the default-on posture, owner custody, and issuer enrollment are OPERATIONAL. Set `AI_MEMORY_CAPABILITIES=off` / `[capabilities].enabled = false` to restore the fully-inert posture. Source: `crate::governance::capability::{ENV_CAPABILITIES,DEFAULT_CAPABILITIES_ENABLED,OWNER_ISSUER,capabilities_env_override}`. |
| 157 | `AI_MEMORY_STORE_URL` | conn-string (`postgres://…` / `sqlite://…`) | unset (the local `--db` sqlite file is the store) | CLI/daemon (`serve` + `curator`, postgres store) | **secret** | **[#1927 (CWE-214), v0.9.0]** Non-argv channel for the store URL, so a Postgres DSN password need NOT ride on `--store-url` — argv is world-readable via `/proc/<pid>/cmdline` and `ps auxww`, whereas `/proc/<pid>/environ` is owner-only. **Precedence here is DELIBERATELY INVERTED relative to the universal ladder** stated above this table: `AI_MEMORY_STORE_URL_FILE` (#158) > this env > the `--store-url` CLI flag. `--store-url` is declared `#[arg(long, value_name = "URL")]` with NO `env =`, so clap does not merge the two into one slot and the env genuinely beats the flag — the inversion is the point of the control (the safer channel must not be overridable by the leakier one), and passing a password-bearing `--store-url` while an env channel is set logs a WARN naming both. A `postgres://` value on a binary built without `sal-postgres` is refused fail-closed with the password redacted (#2679). Source: `src/store_url.rs::{STORE_URL_ENV,resolve_store_url,refuse_postgres_store_url_without_feature}`. **[#3705 sslmode FLOOR, v1.0.0]** A `postgres://` DSN from ANY channel must pin `sslmode=verify-full` (the LAST `sslmode` in the query string decides; `sslrootcert=<ca.crt>` alongside): the ONE connect funnel `PostgresStore::connect_with_dim_and_timeout` refuses before opening a socket (`src/transit_encryption.rs::{dsn_pins_sslmode_verify_full,pg_sslmode_refusal}`), for every deployment — no longer only the enterprise posture's check. The DSN is never echoed. `ai-memory doctor` ("Transit encryption (#3705)", fact `store_url_sslmode`) reports an unpinned DSN before the upgrade refuses. |
| 158 | `AI_MEMORY_STORE_URL_FILE` | path | unset | CLI/daemon (`serve` + `curator`, postgres store) | **secret** | **[#1927 (CWE-214), v0.9.0]** Path to a file whose sole contents are the store URL — the MOST restrictive channel (never in argv, never in the process environment block, never in shell history) and the HIGHEST-precedence layer of the store-url ladder (see #157 for the deliberate CLI-flag inversion). Owner-only permissions are enforced fail-closed: any bit set in `mode & 0o077` is refused with `store-url file <path> has lax permissions`, so `chmod 0600` before first start. The escape hatch is `AI_MEMORY_STORE_URL_FILE_ALLOW_LAX_PERMS` (#122) — which was documented in this table for a full release while the two credential channels it guards were not. An empty file is a hard error. Source: `src/store_url.rs::{STORE_URL_FILE_ENV,store_url_from_file,resolve_store_url}`. **[#3705]** The file's DSN is subject to the same `sslmode=verify-full` floor as #157. |
| 159 | `AI_MEMORY_REQUIRE_WHY_TRACE` | bool (`1`/`true`/`yes`/`on`) | `false` (permissive/advisory) | CLI/daemon/MCP (store write funnel) | config | **[#2059, v1.0.0]** TRACT covenant clause 1 (`docs/spec/TRACT-L1-CLAIM-CONTRACT.md` §8.1) — WRITE-path gate requiring a caller-authored provenance rationale on the memory's `metadata.why_trace` key (`META_KEY_WHY_TRACE`). Only PRESENCE is checked (non-empty after trim), never content. Deliberately distinct from `signed_events.cause_hash` (#1822 G5a): `cause_hash` binds an identity-derived pre-image into the append-only audit chain for specific privileged actions at the point of AUDIT, while `why_trace` is a caller rationale carried on the memory itself and checked at the point of AUTHORSHIP. Default `false` keeps every write path byte-identical to pre-#2059. Shares the truthy grammar of the K2 `AI_MEMORY_REQUIRE_*` knobs via the one shared parser `governance::audit::env_flag_enabled`. Source: `src/storage/mod.rs::{REQUIRE_WHY_TRACE_ENV,require_why_trace_enabled,META_KEY_WHY_TRACE}`. |
| 160 | `AI_MEMORY_REQUIRE_IMMUTABLE_AUTHORSHIP` | bool (`1`/`true`/`yes`/`on`) | `false` (permissive/advisory) | CLI/daemon/MCP (update + merge write funnel) | config | **[#2060, v1.0.0]** TRACT covenant clause 2 — turns an authorship-REWRITE attempt into a REFUSAL. Consulted at the update/merge funnel BEFORE the SQL UPDATE runs: a caller-supplied `metadata.agent_id` that DIFFERS from the row's existing authorship is refused. Defense-in-depth and deliberately INDEPENDENT of the SILENT preserve helpers (`identity::preserve_agent_id` / `preserve_provenance_keys`), which run upstream at the MCP/CLI/HTTP handler layer and quietly fix a compliant caller's value up before it ever reaches this funnel — this gate exists to catch a caller that BYPASSED them (or a direct SAL / postgres caller) and to give operators a REFUSE posture instead of only a silent fix-up. Default `false` keeps every update path byte-identical to pre-#2060. Source: `src/storage/mod.rs::{REQUIRE_IMMUTABLE_AUTHORSHIP_ENV,require_immutable_authorship_enabled}`. |
| 161 | `AI_MEMORY_SKILLS_IMPORT_ROOT` | path | unset (= the canonical folder is its own root — "self-jail") | MCP (`memory_skill_register` `folder_path` import) | config | **[#1923, v1.0.0]** Operator-configured import JAIL root for skill registration from a local folder. When set, a `folder_path` register MUST canonically resolve to a path INSIDE this root or the read is refused (fail-closed). Unset keeps the legitimate "register any local skill folder" workflow while the symlink-escape guard still prevents reads outside the imported tree. Independent per-import resource ceilings (`MAX_IMPORT_FILES` = 4096, `MAX_IMPORT_BYTES` = 64 MiB) bound a hostile tree from INSIDE the jail and are not operator-tunable. Source: `src/mcp/tools/skill_register.rs::SKILLS_IMPORT_ROOT_ENV`. |
| 162 | `AI_MEMORY_RECOVERY_GUARDIAN_PUBKEYS` | comma-separated base64 Ed25519 pubkeys | unset (no guardians enrolled ⇒ no recovery can be minted) | CLI (identity-lineage M-of-N key recovery) | config | **[#1831 G17, v1.0.0]** The enrolled recovery-GUARDIAN key set for M-of-N threshold key recovery on the `agent_lineage` succession chain — the enrollment shape mirrors the #1957 approver set (`AI_MEMORY_APPROVER_PUBKEYS`, #126). A recovery record commits, INSIDE its signed CBOR body, both the `guardian_set_id` (SHA-256 over the sorted enrolled guardian pubkeys) and the `recovery_threshold` it was minted against (schema v81), so a persisted recovery is re-verified against its MINT-TIME trust bar and never against the verifier's later environment — this env is consulted when MINTING, not when verifying. Duplicate signers collapse to one distinct signature; an unenrolled signer is skipped, not counted. Source: `src/identity/lineage.rs::RECOVERY_GUARDIAN_PUBKEYS_ENV`. |
| 163 | `AI_MEMORY_RECOVERY_THRESHOLD` | usize (`M`, >= 1) | **no default — a mint with guardians enrolled but this UNSET is REFUSED** | CLI (identity-lineage M-of-N key recovery) | config | **[#1831 G17, v1.0.0]** The M of the M-of-N recovery threshold: the minimum count of DISTINCT valid enrolled guardian signatures (#162) a recovery record must carry. Unlike every other numeric knob in this table it has **NO compiled fall-through** (ratification delta C) — a key-TAKEOVER primitive must never silently default to `M = 1`, so an unset value refuses the mint rather than picking one. Consulted ONLY when MINTING; verification uses the threshold COMMITTED in the record. Source: `src/identity/lineage.rs::RECOVERY_THRESHOLD_ENV`. |
| 164 | `AI_MEMORY_AUTOTAG_QUEUE_CAPACITY` | usize (`>0`; `0` or unparseable falls through) | `256` (`AUTOTAG_QUEUE_CAPACITY_DEFAULT`) | daemon (HTTP `create_memory`, both sqlite and postgres) | config | **[#2587, v1.0.0; 5-agent vote `4d3ea1c5`, decision memory `4e51e315`]** Bounded-channel capacity of the async `auto_tag` worker (`crate::background::auto_tag_worker`). **The defect it closes:** pre-#2587 `maybe_auto_tag` ran the LLM `auto_tag` chat-completion call SYNCHRONOUSLY, inline on `POST /api/v1/memories`, whenever the caller supplied no tags and an LLM was configured — measured 4.9-11.1s per write in production (row #8's truth-fix has the full defect writeup). That is an AVAILABILITY defect: on MCP stdio a stall of that shape blocks every subsequent tool call (single-threaded JSON-RPC loop, the #965 audit); on the HTTP daemon the call held an admission-control permit (#2032 M3) for its whole duration, so sustained latency could saturate the in-flight cap and shed HEALTHY traffic — including durable-truth WRITES — with 503s. **The fix, per the 5-agent vote:** unlike the `AI_MEMORY_RECALL_EMBED_BUDGET_MS`/`AI_MEMORY_RERANK_BUDGET_MS` precedent (#152/#155) — both tail-cutting BUDGETS calibrated at ~4x a HEALTHY p99 — a budget shape does not fit here: auto_tag's 4.9-11.1s is the FLOOR of a normal LLM generation call, not a rare tail, so any budget tight enough to matter would degenerate into "always degrade" while still holding the request open for the budget duration. Instead the durable INSERT completes FIRST (tags are derived, regenerable data — NOT durable truth, the North Star) and `try_enqueue_auto_tag` (`src/handlers/http.rs`) does a non-blocking `try_send` onto this bounded channel AFTER the row is durable; a dedicated background worker (spawned unconditionally by `bootstrap_serve`, one consumer — so at most one LLM `auto_tag` call is in flight per daemon, no vendor-burst hazard under a concurrent write storm) drains the channel and applies tags via a MERGE (never a blind overwrite): sqlite uses `storage::update_with_expected_version` (a real optimistic-concurrency CAS, one retry on a version conflict); postgres routes through the CONCRETE `PostgresStore`'s inherent `update_with_expected_version` (the SAME If-Match CAS primitive `PUT /memories/{id}` uses, #1628) via the established `app.store.as_any().downcast_ref::<PostgresStore>()` hatch (`src/handlers/memories.rs` precedent) — so the postgres tag-merge is CAS-guarded exactly like sqlite, never a lost-update race against a concurrent caller edit. The apply runs under `CallerContext::for_agent(job.agent_id)` (the ORIGINAL WRITER's own identity, threaded through `AutoTagJob`), never `for_admin` — a targeted, by-id, single-row continuation of the caller's own write needs no privacy-bypass authority. This knob is a plain positive-usize resolver (the `AI_MEMORY_VECTOR_INDEX_CAPACITY` #115 shape), deliberately NOT the tri-state budget grammar — disabling `auto_tag` entirely is already governed by `AI_MEMORY_AUTONOMOUS_HOOKS` (#8), so a second disable lever here would be redundant. `0` or an unparseable value falls through to the compiled default with a WARN (the #131/FBL-14 rule: an unrecognised token must never silently widen a resource bound to unbounded). A full queue DROPS the job (never blocks the write) and increments `ai_memory_autotag_dropped_total` + a `autotag.queue.dropped` WARN — a DEGRADE (no tags scheduled for that write), never a write failure; the response carries `"auto_tagging":"skipped_queue_full"` (honest, never silently omitted) instead of the pre-#2587 literal `"auto_tags":[...]` array — that field is now ABSENT when auto_tag was never eligible (byte-identical to the pre-#2587 no-LLM-ran contract) and `"auto_tagging":"queued"` when a job was accepted (tags may land later, or may not, on a soft LLM failure). Every LLM call the worker makes is still bounded by the EXISTING `app.llm_call_timeout` (env #34-adjacent `[llm]` config, no new budget knob needed). Direct-read knob (not clap-bound / not a `[section]` field), so no `config_precedence` `[limits]`-style entry — the resolver test is co-located (mirrors #150/#151/#152/#153). Observability: `ai_memory_autotag_{enqueued,dropped,applied,degraded}_total` counters (the only channel on MCP stdio, which serves no `/metrics` — though this knob's write path is HTTP-only). Source: `src/background/auto_tag_worker.rs::{ENV_AUTOTAG_QUEUE_CAPACITY,AUTOTAG_QUEUE_CAPACITY_DEFAULT,resolve_queue_capacity,spawn,AutoTagJob,apply_tags_postgres,apply_tags_sqlite}` + `src/handlers/http.rs::{auto_tag_eligible,try_enqueue_auto_tag,AutoTagOutcome}` + `src/handlers/create.rs` + `src/store/postgres.rs::PostgresStore::update_with_expected_version`. |
| 165 | `AI_MEMORY_AUDIT_PUBKEY` | base64 (url-safe, no pad; 32 raw bytes) | unset (pin unenrolled — signature coverage INFORMATIONAL) | CLI/daemon/MCP (`verify-audit-trail`, both backends) | config | **[L4 / PR-3, v1.0.0]** OUT-OF-BAND enrolled AUDIT-daemon PUBLIC key — the K1-style pin for the per-row audit-SIGNATURE coverage lane. When enrolled (this env, or the higher-precedence `--audit-pubkey` flag), `verify_audit_trail` verifies every walked `signed_events` daemon row against THIS key (falling back to the in-process daemon key only when the pin is absent) and folds `unverified = rows_checked − rows_positively_verified` into `is_clean` via the shared `crate::signed_events::compute_signature_verdict` (`SignatureCheck::Unverified` ⇒ dirty, exit 1) — so a stripped / DOWNGRADED / forged / skip-class row (a `lineage_signed` or `recorder_signed`-unenrolled relabel that the tri-state `classify_row_signature` now COUNTS instead of silently discarding) can no longer escape the verdict. When UNSET the coverage is `SignatureCheck::Unenforced` — informational only, byte-identical legacy so rotated / restored / federated nodes do not regress. DELIBERATELY env/flag-only with NO on-disk fallback (a `.pub` beside the key an attacker who owns the host already controls is not a trust anchor — the witness custody mistake this control refuses to repeat). Resolved ONCE in the binary's SYNCHRONOUS pre-runtime phase and threaded as an explicit `Option<&VerifyingKey>` parameter through `verify_chain` / `verify_audit_trail` + the postgres twin (K3 parity) — never re-published to the environment via `set_var` (the #2905 env-leak class). Direct-read knob (not clap-bound as an env, resolved alongside the flag), so no `config_precedence` entry (mirrors #101/#104's out-of-band pin shape). **Limitation (#3354 / #3479):** a chain that ran UNSIGNED before its key was provisioned keeps that unsigned prefix — enrolling this pin then convicts those rows, and provisioning the key does not clear the past. In v1.0.0 there is no supported in-product remedy (the epoch-seal / re-anchor of an unsigned prefix is #3479, v1.0.1); the pin is for chains signed from their first row, which #3354 makes every fresh store. `verify-audit-trail` prints the same sentence (`audit::UNSIGNED_PREFIX_LIMITATION`) beside the coverage FAIL. Source: `src/governance/audit.rs::{AUDIT_PUBKEY_ENV,UNSIGNED_PREFIX_LIMITATION,resolve_audit_pubkey}` + `src/signed_events.rs::{classify_row_signature,compute_signature_verdict,SignatureCheck}`. |
| 166 | `AI_MEMORY_ATOMISE_QUEUE_CAPACITY` | usize (`>0`; `0` or unparseable falls through) | `256` (`ATOMISE_QUEUE_CAPACITY_DEFAULT`) | daemon (HTTP `create_memory`, sqlite) + MCP stdio (deferred `memory_store`) | config | **[#2986, v1.0.0; 5-agent vote `4d3ea1c5`, verdict on #2983]** Bounded-channel capacity of the single-consumer auto-atomise worker (`crate::background::atomise_worker`). **The defect it closes:** the deferred Batman auto-atomise path did a raw `std::thread::spawn` PER over-threshold store — each spawned thread opened its OWN rusqlite connection and drove an LLM curator round trip with the 3-retry ~3.1 s backoff ladder. Unbounded, unpaced, unresumable: a write burst converted directly into thread / connection / vendor-QPS exhaustion. That is the bare-spawn shape the #2587 vote already rejected for `auto_tag`; it was latent only because #2983 made the whole path inert (the process-global `AUTO_ATOMISE_DISPATCH` slot had ZERO production callers), and it would have gone live the moment the wiring landed. **The fix:** the durable INSERT completes first (atoms are DERIVED data; the memory TEXT is the source of truth) and the enqueue is a non-blocking `try_send` onto a BOUNDED `std::sync::mpsc::sync_channel` with exactly ONE consumer, so at most one curator round trip is in flight per daemon. A plain OS thread rather than a tokio task because the two enqueueing surfaces have different runtime shapes — HTTP `serve` is multi-threaded tokio, MCP stdio is a synchronous single-threaded JSON-RPC loop with no runtime at all (the #965 audit) — and `try_send` is safe from both. The worker resolves the atomiser at DRAIN time through a provider closure, never a boot-pinned `Arc<Atomiser>`: a pinned client would keep egressing to a REVOKED vendor after an `[llm]` / egress reload AND would sign `atomisation_complete` payloads whose `curator_model` names a model that never ran (the #2172 boot-capture, laundered into the #1870 attestation lane). A plain positive-usize resolver (the `AI_MEMORY_AUTOTAG_QUEUE_CAPACITY` #164 shape), deliberately NOT the tri-state budget grammar — disabling atomisation is already governed by the namespace standard's `auto_atomise` knob, so a second disable lever here would be redundant. `0` or an unparseable value falls through to the compiled default with a WARN (an unrecognised token must never silently widen a resource bound to unbounded — the #131/FBL-14 rule). A full queue DROPS the job (never blocks the caller) and increments `ai_memory_atomise_dropped_total` + an `atomise.queue.dropped` WARN, with the honest envelope token `atomise_outcome: "skipped_queue_full"` — a DEGRADE (no atoms for that write; `memory_atomise` recovers it), never a write failure. **Backend confinement:** no postgres job is ever enqueued — the enqueue SITES branch on the storage backend and report `skipped_backend_unsupported`, because the atomiser is `rusqlite::Connection`-bound and landing atoms in a different store than their source would be mixed-state corruption. Observability: `ai_memory_atomise_{enqueued,dropped,applied,degraded,no_curator}_total` (the WARN is the only channel on MCP stdio, which serves no `/metrics`). Direct-read knob (not clap-bound / not a `[section]` field), so no `config_precedence` entry — the resolver test is co-located (mirrors #150/#151/#152/#153/#164). Source: `src/background/atomise_worker.rs::{ENV_ATOMISE_QUEUE_CAPACITY,ATOMISE_QUEUE_CAPACITY_DEFAULT,resolve_queue_capacity,spawn,AtomiseJob,AtomiseQueue,AtomiserProvider}` + `src/hooks/pre_store/auto_atomise.rs::run_auto_atomise` + `src/handlers/http.rs::try_enqueue_auto_atomise`. |
| 167 | `AI_MEMORY_PG_AT_REST_ATTESTED` | bool (`1`/`true`/`yes`/`on`) | unset (⇒ compensating pg at-rest control FAILS) | CLI/daemon/MCP (`doctor --posture enterprise-federation` / boot gate, POSTGRES backend only) | config | **[#3061, v1.0.0 GA Wave-2 Cluster A]** The OPERATOR-VOUCHED half of the BACKEND-AWARE certified-posture control #15 (at-rest encryption) on a `postgres://` backend. `evaluate` resolves the backend from the store URL the process will open (`store_url::resolve_store_url` / `is_postgres_url`, the #2679 detector): on sqlite it keeps the byte-identical STRUCTURAL sqlcipher predicate (`AI_MEMORY_ENCRYPT_AT_REST` #37); on postgres — where sqlcipher (a SQLite build feature) cannot encrypt the volume, so #15 was previously UNSATISFIABLE and `all_pass` / the #17 boot gate could never arm a pg node — it swaps to a COMPENSATING control that PASSES iff BOTH the DSN pins TLS `sslmode=verify-full` (machine-checked via `dsn_pins_sslmode_verify_full`, libpq last-wins, fail-closed on absent/weak) AND this env is truthy. This half is OPERATOR-VOUCHED, not machine-proven: the daemon cannot prove its block device is LUKS/dm-crypt/cloud-volume/TDE-encrypted, so the attestation is recorded verbatim in the posture output and labelled conspicuously in `docs/compliance/ENTERPRISE-FEDERATION-CERTIFICATION.md` §1.4 (pg-native TDE as a machine-checked control is NOT claimed for GA). `PostureCheck.pass` stays a bool (the compensating control genuinely PASSES on pg — no tri-state). Direct-read env knob (mirrors the require-mode knobs #98/#109/#124), so no `config_precedence` entry. Source: `src/enterprise_federation_posture.rs::{ENV_PG_AT_REST_ATTESTED,dsn_pins_sslmode_verify_full,evaluate}`. |
| 168 | `AI_MEMORY_MIGRATION_REQUIRE_CORE_TABLES` | bool (`1`/`true`/`yes`/`on`) | unset (= report-only: WARN + `doctor` signal) | CLI/daemon/MCP (every sqlite `db::open` → `migrations::migrate`) | config | **[#3113, v1.0.0]** Whether a MISSING core relation REFUSES the schema-version stamp. Several sqlite ladder arms are gated on an existence probe and SKIP when the relation is absent (v34 `signed_events` chain columns, v66 `governance_rules` severity-CHECK widening, v73 `signed_events.cause_hash`) — correct for a test fixture that stamped a high version over a `SCHEMA`-only database, but the tail of `migrate` then stamps `CURRENT_SCHEMA_VERSION` regardless, so a POPULATED database that LOST a core relation (corruption, partial file-level restore, operator `DROP`) “upgraded successfully” with the integrity controls that stamp implies never applied — fail-OPEN. Detection is UNCONDITIONAL (a structured `WARN` under `ai_memory::storage::schema_integrity` naming each missing relation, the version that introduced it, the control not in force, and the live-corpus row count that separates a populated database from an empty fixture; plus a `Critical` `core_relations` fact in `ai-memory doctor`). This knob only escalates that report to a REFUSAL. The check is sited INSIDE the migrate transaction and BEFORE the stamp, so a refusal rolls the ladder back and leaves the database EXACTLY as found — old version, every row intact, still readable — and is fully REVERSIBLE by unsetting the knob (pinned by `tests/migration_core_relation_refusal_3113.rs`). Default-off is deliberate: defaulting to refuse would turn every existing high-stamp fixture and archive-less deployment into a hard boot failure (a fleet-wide availability regression) for no data-integrity gain, since the check issues NO DDL and NO DML and so can neither lose nor corrupt data. Unset / unparseable → report-only. Direct-read knob (the ladder runs before the boot-seeded config globals exist), no `config_precedence` entry. Source: `src/config.rs::{ENV_MIGRATION_REQUIRE_CORE_TABLES,migration_require_core_tables}` + `src/storage/schema_integrity.rs`. |
| 173 | `AI_MEMORY_ALLOW_LINEAGE_REGRESSION` | bool (`1`/`true`/`yes`/`on`, case-insensitive) | unset (= FAIL CLOSED: refuse the open on a detected append-only lineage regression) | CLI/daemon/MCP (every sqlite `db::open` + every postgres `connect`) | config | **[#3172, v1.0.0]** Operator ACKNOWLEDGEMENT for the SCHEMA-masks-DATA-LOSS gate on append-only bootstrap relations (`agent_lineage`). `agent_lineage` ships in the bootstrap `SCHEMA`/`INIT_SCHEMA`, which both backends replay on EVERY open, silently re-creating a DROPPED table (operator `DROP`, partial restore, corrupt page) EMPTY before the ladder runs — the v80 rebuild then "succeeds" over zero rows and the identity-lineage chain is lost with NO skip logged, UNDETECTABLE by the #3113 table-existence probe. A durable `lineage_integrity_watermark` side table records the max row count ever observed; `schema_integrity::enforce_lineage_watermarks` (sqlite, in `db::open` after `migrate`) and its `PostgresStore::enforce_lineage_watermarks` twin (on every connect after the ladder) compare the live count to the mark. Because the relation is append-only, a live count BELOW the mark is UNAMBIGUOUS loss, so — UNLIKE the ambiguous #3113 knob (which is opt-IN to refuse) — this gate FAILS CLOSED by DEFAULT: it emits a loud `WARN` under `ai_memory::storage::schema_integrity`/`store::postgres` and REFUSES the open, leaving the database UNCHANGED and readable. A never-observed relation (absent/zero mark) can never trip it, so fresh and archive-less deployments are never bricked. This knob is the reverse-polarity ESCAPE HATCH: set it to acknowledge a real loss (e.g. a deliberate purge), which RESETS the mark to the current count and proceeds — not reversible. Direct-read knob (the gate runs before the boot-seeded config globals exist), no `config_precedence` entry. Source: `src/config.rs::{ENV_ALLOW_LINEAGE_REGRESSION,allow_lineage_regression}` + `src/storage/schema_integrity.rs` + `src/store/postgres.rs`. Regression: `tests/lineage_schema_masks_loss_3172.rs`. |
| 169 | `AI_MEMORY_PERMISSIONS_REQUIRE_GOVERNED_NAMESPACE` | bool (`1`/`true`/`yes`/`on`, case-insensitive) | `false` (legacy opt-in-enforcement posture: ungoverned namespaces are ALLOWED) | CLI/daemon/MCP (`storage::enforce_governance` + the postgres `enforce_governance_action` twin) | **operator-advisory** (config) | **[v1.0.0 fail-open remediation]** OPT-IN strict admission posture. When truthy AND `permissions.mode = "enforce"`, a `store`/`delete`/`promote` into a namespace whose governance chain resolves NO policy is REFUSED (`GovernanceRefusal`, 403 `GOVERNANCE_REFUSED`) instead of allowed — closing the allow-on-silence admission hole where a typo'd or newly-branched namespace skips the approval / owner / `required_scope` gates. DEFAULT OFF and byte-identical to the legacy behaviour, deliberately: `ungoverned subtrees remain opt-in` is a CUTLINE-PROTECTED ship-gate contract (`tests/ship_gate_governance_inheritance.rs`) that holds even while a sibling subtree IS governed, and `AppConfig::effective_permissions_mode` resolves an unconfigured `[permissions]` block to `enforce`, so a default flip would refuse every write on a fresh install. `permissions.mode = "advisory"`/`"off"` are UNAFFECTED in every case. Pair it with a substrate-wide default policy on the `*` namespace (`memory_namespace_set_standard`). Certified enterprise-federation deployments SHOULD engage it. |
| 170 | `AI_MEMORY_CAPABILITY_FILE` | path | unset (no file channel; `--capability` argv only) | CLI (`store` / `delete` / `promote`, the governed verbs that accept a capability token) | config | **[#1927-class, v1.0.0]** NON-argv channel for the G10.1 (#1827) macaroon capability token: a `0600` file whose sole contents are the `cap1:` token. A token passed on `--capability` lands in world-readable `/proc/<pid>/cmdline`, `ps auxww`, shell history and systemd unit files, where any co-tenant local UID can lift it and REPLAY it within its caveats to flip a governance `Deny`/`Ask` to `Allow` — the same argv-exposure class #1927 closed for the Postgres DSN (`AI_MEMORY_STORE_URL_FILE`, #158), which the capability path was simply missing. Resolution order (first hit wins): `--capability-file <path>` > this env > `--capability <token>` (still accepted, now WARNs). The two flags CONFLICT at clap parse. A NAMED-but-unreadable / lax-mode / empty / set-but-non-UTF-8 file is a hard error (`var_os` + `PathBuf`; `Path` needs no UTF-8), never a silent downgrade to argv or "no token presented". The file is opened ONCE and `fstat`ed on that handle (no TOCTOU re-open window — #1790 finding 2). Direct-read knob (not clap-bound as an env), so no `config_precedence` entry. Source: `src/governance/capability.rs::{CAPABILITY_FILE_ENV,capability_from_file,resolve_capability}`. |
| 171 | `AI_MEMORY_CAPABILITY_FILE_ALLOW_LAX_PERMS` | bool (`1`/`true`) | `false` (require 0600-or-tighter) | CLI (governed verbs reading `AI_MEMORY_CAPABILITY_FILE` / `--capability-file`) | **operator-advisory** (config) | **[#1927-class, v1.0.0]** Escape hatch for the strict-permission check on the capability-token file (#170). When truthy, `capability_from_file` accepts the file even with bits set in `mode & 0o077` (group / world readable) and logs a WARN on every fire. Default `false` refuses with `capability file <path> has lax permissions`. Exactly mirrors `AI_MEMORY_STORE_URL_FILE_ALLOW_LAX_PERMS` (#122) and `AI_MEMORY_PASSPHRASE_FILE_ALLOW_LAX_PERMS` (#40). Set this only if a custom secret-injection workflow fights the chmod step. Source: `src/governance/capability.rs::{CAPABILITY_FILE_ALLOW_LAX_PERMS_ENV,capability_from_file}`. |
| 172 | `AI_MEMORY_EMBED_DIM` | u32 (positive) | unset (falls through to `[embeddings].dim` > the compiled `KNOWN_EMBEDDING_DIMS` table = the model's NATIVE width) | CLI/daemon/MCP (embedder init + postgres schema bootstrap) | config | **[#2626, v1.0.0]** Env form of `[embeddings].dim`, and the highest-precedence layer of the dim ladder (env > config file > compiled table). Pre-fix the dim was CONFIG-FILE-ONLY while every sibling embeddings knob (`AI_MEMORY_EMBED_BACKEND` / `_MODEL` / `_BASE_URL` / `_API_KEY`) had an env form, so an env-configured deployment — containers, systemd, CI — could not reproduce a file-configured fleet's vector space even when the operator knew the value: `google/gemini-embedding-2` resolved to its native 3072 from the compiled table while the fleet's pgvector columns are `vector(768)`, both nodes logging an identical successful embedder load. For a Matryoshka-capable API model the value is sent as the wire `dimensions` param, so the vendor truncates server-side and the vectors fit the fleet schema. A value that is not a POSITIVE integer FAILS CLOSED — the dim resolves as UNKNOWN (an API embedder does not load; recall degrades to keyword) rather than falling through to the compiled table, because falling through would silently resolve the exact value the operator was overriding. The resolved dim AND its source are logged at boot (`embedder loaded (… , dim source: …)`). Source: `crate::config::{ENV_EMBED_DIM,resolve_embed_dim_pin,EmbedDimSource}`. |
| 174 | `AI_MEMORY_COST_MICRO_USD_PER_1K_TOKENS` | u64 (micro-USD per 1,000 tokens) | `10000` (= $10 / 1M tokens) | CLI/daemon/MCP (cost accounting) | config | **[#3323, v1.0.0]** Advisory override for the token→cost rate in per-lineage/per-namespace cost accounting (#3323); micro-USD per 1,000 tokens, default 10000 (= $10 / 1M tokens). Direct-read knob (not clap-bound / not a `[section]` field), so no `config_precedence` entry. Source: `src/cost/mod.rs`. |
| 175 | `AI_MEMORY_EMBED_BACKFILL_ON_BOOT` | enum (`background`/`blocking`/`off`; aliases `async`/`sync`/`none`) | `background` | CLI/daemon/MCP (embedder serve-boot) | config | **[#3329, v1.0.0]** Controls WHEN the boot embedding-backfill runs relative to the MCP `initialize` handshake. `background` (default): the MCP tool surface is exposed IMMEDIATELY and the backlog drains on a detached thread (unembedded rows stay keyword-recallable meanwhile), so a corpus with an embedding backlog (or a concurrent `reembed`) can no longer time out an MCP client's handshake (the pre-#3329 bug: server up advertising 0 tools). `blocking`: opt-in — drain BEFORE completing the handshake, bounded by the 20s `BOOT_BACKFILL_BLOCKING_BUDGET`, then the remainder drains in the background. `off`: skip the boot backfill entirely (fill later via `ai-memory reembed`). DATA INTEGRITY is unaffected in every mode — only the derived vector is deferred; the durable memory TEXT is untouched. Precedence: env > `[embeddings].backfill_on_boot`. Source: `crate::config::{ENV_EMBED_BACKFILL_ON_BOOT,BackfillOnBoot,resolve_embed_backfill_on_boot}`. |
| 176 | `AI_MEMORY_SKILLS_EXPORT_ROOT` | path | unset (= a `skills-export` directory BESIDE THE RESOLVED STORE — the parent of the `--db` path this process opened — created `0700` on first export) | MCP / HTTP / CLI (`memory_skill_export` `target_folder`) | config | **[#3357, v1.0.0]** Operator-configured export JAIL root for skill export; the write-side twin of `AI_MEMORY_SKILLS_IMPORT_ROOT` (#1923). `memory_skill_export` is an UNPRIVILEGED MCP tool that writes `SKILL.md` + a `resources/` subtree, so the caller-supplied `target_folder` MUST canonically resolve INSIDE the export root or the export is refused BEFORE any filesystem I/O (fail-closed): a `..` component is rejected lexically, a relative target is anchored at the root, and the deepest existing ancestor is canonicalized so symlink traversal out of the root collapses and is caught; containment is re-asserted after `create_dir_all`, before `SKILL.md` is written. When SET the root must ALREADY exist and is never created — an env typo REFUSES rather than silently minting a jail nobody meant. When UNSET the store-adjacent default is created `0700` at `mkdir(2)` time (`DirBuilder::mode`, so no group-writable window; same posture as the #3198 key directory). If neither rung resolves to an existing directory — e.g. an in-memory store with no root configured — the export is REFUSED; there is no unconfined fallback, and the process working directory is deliberately NOT one (a daemon's CWD is arbitrary, frequently `/` or `$HOME`, which is not a jail). Source: `src/mcp/tools/skill_export.rs::SKILLS_EXPORT_ROOT_ENV` + `DEFAULT_EXPORT_DIR_NAME`. |
| 177 | `AI_MEMORY_AGENT_KEY_REFRESH_SECS` | u64 seconds (`0` disables) | `15` (`DEFAULT_AGENT_KEY_REFRESH_SECS`) | daemon (`serve`, both backends) | **operator-advisory** (security window) | **[#3418, v1.0.0]** Cadence at which the daemon re-reads the per-agent api-key enrolment set from the store, so an enrolment — and, the reason this is security-relevant, a **REVOCATION** — takes effect without a daemon restart. The value IS the worst-case revocation window: a key revoked at T stops authenticating by T+N. `0` disables the refresh entirely and pins the registry to a boot snapshot — an explicitly degraded posture in which a revoked key keeps authenticating until restart; boot logs which posture is in force either way (`refresh_posture_note`). A non-numeric value is NOT honoured: it WARNs and falls back to the 15s default rather than silently widening the window. A failed refresh pass keeps the last known good registry (degrade, never corrupt) instead of disarming the gate. Source: `crate::handlers::identity_binding::{ENV_AGENT_KEY_REFRESH_SECS,DEFAULT_AGENT_KEY_REFRESH_SECS,resolve_agent_key_refresh_interval}`. |
| 178 | `AI_MEMORY_TEST_3550_CHILD_ROLE` | enum `writer` / `crash` | unset (not a child) | tests (`cli::backup::tests::publish_3550`, re-exec'd test binary) | **test-only** | **[#3550, v1.0.0]** Marks a re-exec of the test binary as a restore-publish child: `writer` tries to open the target mid-restore (must be refused at every locked step), `crash` runs a restore and `abort()`s at the step named by #181. Never read outside `#[cfg(test)]`; no production reader.
| 179 | `AI_MEMORY_TEST_3550_CHILD_DB` | path | unset | tests (`publish_3550` child) | **test-only** | **[#3550]** The target database path the child acts on.
| 180 | `AI_MEMORY_TEST_3550_CHILD_SNAPSHOT` | path | unset | tests (`publish_3550` child) | **test-only** | **[#3550]** The snapshot the `crash` child restores from.
| 181 | `AI_MEMORY_TEST_3550_CHILD_STEP` | `PublishStep` name | unset | tests (`publish_3550` child) | **test-only** | **[#3550]** The publish step at which the `crash` child `abort()`s (no destructors, no unwind, no SQLite close), so the crash matrix proves old-or-new-never-a-mix at all seven steps.
| 182 | `AI_MEMORY_TEST_3550_CHILD_MARKER` | path | unset | tests (`publish_3550` child) | **test-only** | **[#3550]** File the child touches to report which step it reached before aborting.
| 183 | `AI_MEMORY_UNSTAMPED_MUTATION` | enum (`warn`/`refuse`, case-insensitive) | `warn` (every funnel keeps its pre-#3124 outcome; admissions WARN) | CLI/daemon/MCP (every caller-scoped mutation funnel, both backends) | config | **[#3124, v1.0.0; T9 5-agent vote 3–2 migrate-then-refuse]** The ONE cross-backend policy for caller-scoped mutations of an UNSTAMPED (legacy-unowned) row. ONE definition (`src/identity/owner_stamp.rs::OwnerStamp`): a row is *stamped* iff `metadata.agent_id` is a non-empty JSON string; missing / JSON null / `""` is *unstamped*; a present non-string value is *malformed* — never matched as an owner and never admitted as unstamped, on either backend (pre-#3124 the sqlite Rust gates read it through `as_str()` and let anyone mutate it, while postgres `->>` let a caller of the same spelling match it). `warn` (default) changes NO funnel's allow/refuse outcome on either backend — the postgres #1412/#1628 funnels (trait `update` / `delete`, If-Match `PUT`, `archive_by_ids`) keep REFUSING, so the default is never a fail-open relaxation — and every mutation it does admit on an unstamped row emits a WARN (`target: authz.unstamped`) plus `ai_memory_unstamped_mutation_allowed_total{backend,funnel}`. `refuse` refuses an unstamped row on EVERY caller-scoped mutation funnel of both backends (the inbox recipient never rescues an unstamped row — the #1628 shape); the refusal reason names `ai-memory reown`. The grammar is exactly `warn` | `refuse` (trimmed, case-insensitive; empty / unset = `warn`): any other token REFUSES boot, naming the knob, the token and the grammar (`owner_stamp::validate_boot_token`, called pre-runtime for every verb but `doctor`, which reports the bad value instead); a library caller that skips the boot check still resolves it fail-closed to `refuse` (FBL-14 — never widen). Independent of the knob, an update can never WRITE an owner onto a row that has none (`identity::preserve_update_provenance_keys` drops the caller's `metadata.agent_id` with a WARN — the pre-#3124 silent first-writer claim). `ai-memory doctor` reports the census (`Unstamped owners (#3124)`: live unstamped / malformed / archived-unstamped counts, both backends; Warning when non-zero, Critical when non-zero under `refuse`). Nothing auto-stamps a row (vote constraint 1). The compiled default flips to `refuse` in v1.x once the census reads 0 (vote step 4). PINNED to `refuse` by `asi-hard` (#130). Admin / operator lanes (`CallerContext::bypass_visibility`) and the sqlite `daemon` principal are unaffected. READ paths keep their pre-#3124 predicate. Direct-read knob (read per mutation, never boot-seeded), so no `config_precedence` entry — the grammar tests are co-located in `src/identity/owner_stamp.rs`. Source: `src/identity/owner_stamp.rs::{ENV_UNSTAMPED_MUTATION,UnstampedMutationMode,OwnerStamp,metadata_admits_mutation,unlink_admitted,admit_unstamped}`. |
| 184 | `AI_MEMORY_BACKUP_POSTURE_ATTESTATION` | exact string `attested` (any other value, including unset, is inert) | unset | daemon/HTTP write receipts (`src/write_receipt.rs`) | config | #3555 — an OPERATOR POSTURE ASSERTION, not evidence. It upgrades a write receipt's `durability_class` to `replicated+backup` ONLY when the write also received a remote acknowledgement (`quorum_acks > 1`); with a single ack it stays `local-only`. It never proves an asynchronous backup contains this write. |
| 185 | `AI_MEMORY_AGENT_API_KEY_FILE` | path | unset | CLI (`ai-memory agents bind-api-key`) | config | **[#3781]** Non-argv channel for the per-agent HTTP api-key token that `bind-api-key` enrolls. The token on `--token` (argv) is REFUSED because argv is world-readable via `/proc/<pid>/cmdline` and `ps auxww`; supply it instead via `--token-file <0600 path>` (the flag) or this env var, whose value is a PATH to a `0600` file whose sole contents are the token (`/proc/<pid>/environ` is owner-only). Resolution: `--token-file` > this env > REFUSE argv `--token`. Only `sha256(token)` is ever persisted (`src/handlers/identity_binding.rs`), never the raw token. The file is read via `read_api_key_token_file` (mode `& 0o077 == 0` enforced; empty-after-trim refused). Source: `src/cli/agents.rs::{AGENT_API_KEY_FILE_ENV,resolve_bind_api_key_token,read_api_key_token_file}`. |
| 186 | `AI_MEMORY_AGENT_API_KEY_FILE_ALLOW_LAX_PERMS` | bool (`1`/`true`) | `false` (require 0600-or-tighter) | CLI (`ai-memory agents bind-api-key` reading `AI_MEMORY_AGENT_API_KEY_FILE` / `--token-file`) | **operator-advisory** (config) | **[#3781]** Escape hatch for the strict-permission check on the api-key token file (#185). When truthy, `read_api_key_token_file` accepts the token file even with bits set in `mode & 0o077` (group / world readable) and logs a WARN on every fire. Default `false` refuses with `api-key token file <path> has lax permissions`. Exactly mirrors `AI_MEMORY_STORE_URL_FILE_ALLOW_LAX_PERMS` (#122), `AI_MEMORY_CAPABILITY_FILE_ALLOW_LAX_PERMS` (#171) and `AI_MEMORY_PASSPHRASE_FILE_ALLOW_LAX_PERMS` (#40); parses through the one #3200 truthy grammar (`crate::security_profile::is_truthy`). Set this only if a custom secret-injection workflow fights the chmod step. Source: `src/cli/agents.rs::{AGENT_API_KEY_FILE_ALLOW_LAX_PERMS_ENV,read_api_key_token_file}`. |
...
archive_on_gc     = true
# Data-integrity master switches (env twins in the table above). Shown
# here because their OFF states change what a destructive op does:
# append_only = false                    # #1823 G6 signed-revision spine.
#                                        # Env: AI_MEMORY_APPEND_ONLY (#97).
# lineage_dag = true                     # #1859 provenance-DAG + acyclicity
#                                        # guard. Env: AI_MEMORY_LINEAGE_DAG
#                                        # (#110); compiled default TRUE.
# consolidate_tombstone_sources = true   # tracks lineage_dag. When FALSE,
#                                        # consolidation HARD-DELETES its
#                                        # source memories instead of
#                                        # tombstoning them. Env:
#                                        # AI_MEMORY_CONSOLIDATE_TOMBSTONE_SOURCES
#                                        # (#111).
# age_projection_mode = "sync"           # postgres+AGE only. Env:
#                                        # AI_MEMORY_AGE_PROJECTION_MODE (#80).
# db_mmap_size_bytes  = 268435456        # Env: AI_MEMORY_DB_MMAP_SIZE (#70).
# archive_max_days    = 90
# max_memory_mb       = 4096
...
[limits]
# Operator-tunable resource caps. Precedence per field:
# AI_MEMORY_MAX_* env > this section > compiled default.
# Absent / non-positive / unparseable values fall through to the
# compiled default — EXCEPT max_inflight_requests, whose tri-state
# resolver honours an explicit 0 as DISABLED (see below and env #79).
...
# max_inflight_requests            # #1733 HTTP admission cap, TRI-STATE since
                                   # #2032 M3 — LEAVE IT UNSET for the secure
                                   ...
                                   # An EXPLICIT 0 = DISABLED — writing `0`
                                   # here turns the DoS admission guard OFF;
                                   # it is not "the default". See env row #79.
...
`max_storage_bytes`, `max_links_per_day` (all `i64`),
`max_page_size` (`usize`), and — added after this paragraph was first
written — `max_inflight_requests`, `vector_index_capacity` and
`vector_index_hard_fail_at_cap`, each with a `ConfigSource` provenance tag
...
`max_page_size` can never clamp every list response to empty).
**`max_inflight_requests` is the deliberate exception to that filter**
(#2032 M3): its resolver is `env_tristate_usize`, which has NO `> 0`
filter, so an explicit `0` is honoured as DISABLED and never falls
through, while UNSET resolves to the CPU-scaled
`resolve_default_max_inflight_requests`. Reading the blanket
"non-positive falls through" sentence as covering this knob is exactly
the mistake that turns the admission guard off — see env row #79. The
...
full`, 102 production HTTP route registrations / 88 unique URL paths, 99 CLI subcommands (97 in the default build) at v1.0.0 (post FX-12/ARCH-3 + FX-C3 batch2 + #1389 L2 `RecoverPreviousSession` + #1443 `Expand` + #1598 `Reembed` + #1727 `UndoEdit` + #1978 `Watch`), tens of
