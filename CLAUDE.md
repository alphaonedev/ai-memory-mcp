...
| 37 | `AI_MEMORY_ENCRYPT_AT_REST` | bool (`1`/`true`/`yes`/`on`, case-insensitive; `[encryption].at_rest = true` in config.toml is the equivalent seed) | `false` (content stored verbatim) | CLI/daemon/MCP, **default build and every build**, both backends (sqlite + postgres) | config | **[#228, #1956; doc corrected by #3720]** Engages **application-level per-record content encryption — NOT SQLCipher.** When truthy, every memory write seals `content` into `memories.encrypted_envelope` (a `0x03` envelope: a per-record ChaCha20-Poly1305 data-encryption key wrapped under the per-agent X25519 key via ECDH + HKDF-SHA256; `content` column left empty; **title / tags / metadata stay plaintext**), on sqlite and postgres alike, and reads unseal it. Keys: `<agent_id>.x25519.pub` (0644) / `.priv` (0600) under the key dir (`AI_MEMORY_KEY_DIR`, else the default key dir), **minted on first use, persisted, no rotation and no escrow at v1.0.0 (#3717)** — a lost `.priv` makes every row sealed under it unrecoverable, and `crypto_erase` rewrites an envelope to the `0x04` erased form. No passphrase is involved and none is required. `doctor` reports `at_rest = ENCRYPT_AT_REST` at Info; the boot posture treats it as healthy. **SQLCipher is a different control:** whole-database page encryption keyed by the passphrase channel (rows #2 / #40, `--db-passphrase-file` / `AI_MEMORY_DB_PASSPHRASE`) on a `--features sqlcipher` build only; a passphrase on a non-sqlcipher build is a boot refusal, this knob is not. The two compose. Known open gap (#3720, T3 ruling pending): posture control #15 still reads this knob as a SQLCipher predicate. |
