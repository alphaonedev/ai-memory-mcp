...
| 183 | `AI_MEMORY_TEST_3669_CHILD_MARKER` | path | unset (not a child) | tests (`tests/test_scratch_exit_cleanup_3669.rs`, re-exec'd test binary) | **test-only** | **[#3669, v1.0.0]** Marks a re-exec of the test binary as the exit-cleanup child: it creates the key-directory sandbox and a `test_scratch::process_lifetime_dir`, writes their paths to this file and exits normally, so the parent can assert both directories were removed at process exit. No production reader.
