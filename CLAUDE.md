...
| 183 | `AI_MEMORY_READ_AUDIT_STRICT` | bool (shared `1`/`true`/`yes`/`on` grammar; `0`/`false`/`no`/`off` or unset = off; any other token arms it fail-closed with a warning) | unset (best-effort) | MCP sqlite read gate (`governance::agent_action::gate_read`) | **operator-advisory** (config) | **[#3660, v1.0.0]** Enterprise read-audit policy. An engaged `read_action` decision is chain-logged by a DIRECT `signed_events` append with no DLQ, spool or retry behind it (the pre-#3660 "DLQ-backed" comment was false); by default an append failure is counted as a permanent evidence gap (`ai_memory_governance_read_audit_evidence_gap_total{residence}`, `/health` `governance.read_audit_delivery`) and the read proceeds. When armed, a read whose decision cannot be chain-logged is REFUSED with a governance-refusal wire shape instead. Not part of the `asi-hard` floor — it trades read availability for audit completeness. Direct-read knob (`governance::read_audit::strict_policy_from_env`), no `config_precedence` entry. |
