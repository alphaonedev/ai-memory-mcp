...
| 34 | `AI_MEMORY_LLM_MODEL` | string | tier-/vendor-specific (e.g. `gemma3:4b` on ollama, `grok-4` on xai, `gpt-4o` on openai, `claude-sonnet-4` on anthropic, `deepseek-chat`, `qwen-max`, …) | CLI/daemon/MCP | config | **[#1067, v0.7.0]** Model identifier passed through verbatim to the chat / embed endpoint. Vendor-specific — see vendor docs for the canonical name. |
