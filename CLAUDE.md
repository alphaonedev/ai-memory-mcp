...
| — | `RUST_LOG` | tracing filter | unset (= `info` for EVERY target) | all | config | Standard `tracing-subscriber` directives (e.g. `RUST_LOG=ai_memory=debug`), LAYERED on the default by `crate::logging::build_log_filter`: the console and every `[logging]` sink start from a bare level (`info` / `[logging].level`), add the caller's extras, then apply each `RUST_LOG` directive LAST so the operator's directive wins for the target it names. Not an `AI_MEMORY_*` var — listed for completeness. **[TRUTH-FIX #3650, v1.0.0]** this row said `unset (= info)` while the shipped console default was the directive `ai_memory=info`: RUST_LOG unset put every other target at ERROR, the shipped units' `RUST_LOG=ai_memory=info` turned them OFF, and the appended default reset an operator's `ai_memory=debug` back to `info`. **Post-#1562 (2026-06-09):** the postgres SAL adapter emits under the literal targets `store::postgres` / `store::postgres::kg` (and `schema-init` under `schema_init`), so an `ai_memory=debug` directive does NOT raise those events — add `store::postgres=debug` explicitly (the default `info` already covers them). |
