...
| 23 | `AI_MEMORY_FED_PEER_ATTESTATION` | JSON object (peer ID to authorship/namespace scope) | unset | federation | config | Separate from peer key enrollment, which is default-required. Default-required namespace checks refuse inbound writes with no allowlist (#3582); Standard retains an explicit require-scope `0` opt-out (row 147). A configured empty/invalid map fails closed even at `=0`. With outbound peers or inbound enrollment/bindings configured, Standard warns and `asi-hard` refuses boot without a valid, nonempty map. Ordinary doctor remains runnable; local outbound/listener argv is unobservable, while capabilities v2/v3 and remote doctor carry the daemon boot snapshot. Source: `src/federation/peer_attestation.rs::PeerAttestationConfig`, `src/federation/peer_posture.rs`. |
