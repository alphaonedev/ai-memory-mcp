...
**Boolean grammar (#3200).** Every security-relevant boolean knob
(each MANDATE that tightens a control and each HATCH that loosens one)
reads through ONE parser, `src/env_flag.rs`: `1`/`true`/`yes`/`on`
enable and `0`/`false`/`no`/`off` disable, case-insensitive and trimmed;
an empty value counts as UNSET. An unrecognised token on a registered
knob REFUSES boot, naming the knob, the token and the accepted grammar
(a mandate AND a hatch alike, so a typo is neither a silent widen nor a
silent no-op), and a token whose meaning changed versus its pre-#3200
reader gets a one-shot boot WARN. The `asi-hard` floors ask the same
registry entry as the live readers. NEUTRAL knobs not yet migrated are
tracked in `scripts/qc-allowlists/truthy-grammar-neutral-ledger.txt`
(burned down by #3622) and gated by `scripts/check-truthy-grammar.sh`.
...
| 37 | `AI_MEMORY_ENCRYPT_AT_REST` | bool (shared grammar `1`/`true`/`yes`/`on`, #3200) | `false` (plain sqlite) | CLI/daemon (sqlcipher build only) | config | When truthy AND the binary was built with `--features sqlcipher`, every DB open path requires an `AI_MEMORY_DB_PASSPHRASE` (or `--db-passphrase-file`); plain-sqlite opens are refused. Operational nuance: switching this flag on against an existing plain DB does NOT encrypt it — operators must `ai-memory export → encrypted-init → import` or use the sqlcipher CLI `ATTACH … KEY` recipe. **[#3200, v1.0.0]** Reads through the ONE shared boolean grammar (`src/env_flag.rs`): `1`/`true`/`yes`/`on` enable, `0`/`false`/`no`/`off` disable (case-insensitive, trimmed), an empty value is unset, and an unrecognised token REFUSES boot (a lazy read of one keeps this mandate ON). A token whose meaning changed versus the pre-#3200 reader gets a one-shot boot WARN. **#3621:** the pre-#3200 reader did not trim, so a padded ` 1` left at-rest encryption silently OFF. |
...
| 39 | `AI_MEMORY_GOVERNANCE_FAIL_OPEN_ON_ERROR` | bool (shared grammar `1`/`true`/`yes`/`on`, #3200) | `false` (fail-CLOSED, v0.7.0 secure default) | CLI/daemon/MCP | **operator-advisory** (config) | **[#1054, v0.7.0]** Escape hatch for the governance fail-CLOSED posture. When truthy, transient rule-consultation errors (rule-provider timeouts, missing rule rows, etc.) revert to v0.6.x permissive behaviour where the write proceeds. Default `false` (fail-CLOSED) blocks writes on any rule-consultation error to prevent silent governance bypass. Set this only if you operate a custom rule provider that errors transiently AND you accept the risk of writes that bypass policy during the error window. PINNED CLOSED by `asi-hard` (#130 / #3168) — a live-grammar token that arms fail-OPEN (`1`/`true`) refuses boot. **[#3200, v1.0.0]** Reads through the ONE shared boolean grammar (`src/env_flag.rs`): `1`/`true`/`yes`/`on` enable, `0`/`false`/`no`/`off` disable (case-insensitive, trimmed), an empty value is unset, and an unrecognised token REFUSES boot (a lazy read of one keeps this hatch CLOSED). A token whose meaning changed versus the pre-#3200 reader gets a one-shot boot WARN. |
| 40 | `AI_MEMORY_PASSPHRASE_FILE_ALLOW_LAX_PERMS` | bool (shared grammar `1`/`true`/`yes`/`on`, #3200) | `false` (require 0400-or-tighter) | CLI/daemon (sqlcipher build) | **operator-advisory** (config) | **[#1055, v0.7.0]** Escape hatch for the strict-permission passphrase-file check. When truthy, the daemon accepts `--db-passphrase-file` even if the file has bits set in `mode & 0o077` (group / world readable). Default `false` rejects lax permissions with `passphrase file <path> has lax permissions` — operators upgrading from v0.6.x must `chmod 0400 ./passphrase.txt` before first start. Set this only if you have a custom secret-injection workflow that fights the chmod step. **[#3200, v1.0.0]** Reads through the ONE shared boolean grammar (`src/env_flag.rs`): `1`/`true`/`yes`/`on` enable, `0`/`false`/`no`/`off` disable (case-insensitive, trimmed), an empty value is unset, and an unrecognised token REFUSES boot (a lazy read of one keeps this hatch CLOSED). A token whose meaning changed versus the pre-#3200 reader gets a one-shot boot WARN. |
| 41 | `AI_MEMORY_SSRF_GUARD_ALLOW_DNS_FAIL` | bool (shared grammar `1`/`true`/`yes`/`on`, #3200) | `false` (fail-CLOSED, v0.7.0 secure default) | daemon (webhook dispatch + federation push) | **operator-advisory** (config) | **[#1053, v0.7.0]** Escape hatch for the SSRF-guard fail-CLOSED posture. When truthy, webhook + federation dispatches whose target URL fails DNS resolution proceed (TCP connect will then fail explicitly). Default `false` (fail-CLOSED) refuses the dispatch on DNS failure so a sketchy resolver cannot trick the daemon into bypassing the loopback / private-range guard. Set this only if you operate against an internal resolver with intermittent failures AND you accept the SSRF risk. **[#3200, v1.0.0]** Reads through the ONE shared boolean grammar (`src/env_flag.rs`): `1`/`true`/`yes`/`on` enable, `0`/`false`/`no`/`off` disable (case-insensitive, trimmed), an empty value is unset, and an unrecognised token REFUSES boot (a lazy read of one keeps this hatch CLOSED). A token whose meaning changed versus the pre-#3200 reader gets a one-shot boot WARN. |
...
| 48 | `AI_MEMORY_REQUIRE_AGENT_ATTESTATION` | tri-state (`0`/`false` global permissive; `1`/`true` global strict; unset → surface-scoped) | unset → **HTTP-direct REQUIRED, MCP/CLI PERMISSIVE** (surface-scoped, #1985) | CLI/daemon/MCP (store write paths, classified by API surface) | config | **[#626 Layer-3 C7, v0.7.0; #1751 v0.9.0; SURFACE-SCOPED by #1985, v1.0.0]** Tri-state with a per-surface compiled default. With the env UNSET, an unsigned direct-store write is fail-CLOSED (`403 ATTESTATION_FAILED`) ONLY on the HTTP direct-write surface (`POST /api/v1/memories` + `/memories/bulk` — `WriteSurface::HttpDirect`); the MCP `memory_store` and CLI `store` surfaces are the operator-as-actor path (`WriteSurface::Mcp`/`Cli`, #1621/#1675) and land `attest_level="claimed"` unsigned. `1`/`true` forces strict on EVERY surface (the v0.9.0 posture); `0`/`false`/`no`/`off` (case-insensitive) forces permissive on every surface (the v0.8 posture); **[#3200 / #3618, v1.0.0]** the knob reads through the ONE shared boolean grammar (`src/env_flag.rs`): `0`/`false`/`no`/`off` are the permissive opt-out, `1`/`true`/`yes`/`on` are global strict, an empty value keeps the surface-scoped default, and an unrecognised token REFUSES boot (a lazy read of one is strict on every surface). Before #3200 only `1`/`true` armed global strict while the `asi-hard` floor accepted `yes`/`on`, so under `asi-hard` `=yes` met the floor and unsigned MCP/CLI writes still landed (#3618). Scope is by **API surface, NEVER transport/bind**: a postgres deployment proxying MCP through the HTTP daemon still classifies those writes as MCP. When a `signature` IS presented it is verified against the agent's bound public key regardless of this flag (valid → `agent_attested`; forged → `403` UNCONDITIONALLY on every surface); this flag only governs the unsigned-write disposition. **Why surface-scoped:** the v0.9.0 GA #1751 required attestation on every surface, but no MCP host can construct/sign the canonical `SignableWrite` envelope, making the default unsatisfiable on MCP (the #1981 break); the reference deployment had validated the `=0` opt-out path, not the shipped default. **Remediation:** sign writes (`ai-memory store --sign` with a keypair bound via `ai-memory agents bind-key`) or set `AI_MEMORY_REQUIRE_AGENT_ATTESTATION=0`. **Scope:** the STORE/direct-write path ONLY — the federation receive path attests via the peer-authorship allowlist (`resolve_inbound_attribution`, #1464), NOT this flag; curator/autonomy self-writes go through the SAL `store()` surface (`CallerContext::for_admin`) and are exempt. Source: `src/identity/attest.rs::require_agent_attestation_for` + `WriteSurface` (`AI_MEMORY_REQUIRE_AGENT_ATTESTATION`). |
...
| 56 | `AI_MEMORY_REQUIRE_API_KEY` | bool (shared grammar `1`/`true`/`yes`/`on`, #3200) | `false` | daemon | config | **[#1458, v0.7.0]** Strict keyless-bind refusal: when truthy the daemon hard-refuses to start without an `api_key` on ANY bind host, including loopback — the hardened posture for reverse-proxy / `--network=host` / `socat` deployments where the loopback host string does not reflect off-host reachability. Source: `src/daemon_runtime.rs::require_api_key_strict`. **[#3200, v1.0.0]** Reads through the ONE shared boolean grammar (`src/env_flag.rs`): `1`/`true`/`yes`/`on` enable, `0`/`false`/`no`/`off` disable (case-insensitive, trimmed), an empty value is unset, and an unrecognised token REFUSES boot (a lazy read of one keeps this mandate ON). A token whose meaning changed versus the pre-#3200 reader gets a one-shot boot WARN. Before #3200 only `1`/`true` enabled it, so `=yes` silently left the mandate off. **PINNED `1` by `asi-hard`** (#130): a keyless `serve` bind is refused on every host under the posture, so set top-level `api_key` first. |
...
| 69 | `AI_MEMORY_ADMIN_HEADER_TRUST` | bool (shared grammar `1`/`true`/`yes`/`on`, #3200) | `false` (secure default) | daemon | **operator-advisory** (config) | **[#1570, v0.7.0]** Legacy escape hatch for header-only admin trust. Default OFF: when admin ids are configured and the daemon has NO request authentication (no `api_key`), a bare self-asserted `X-Agent-Id` naming an admin id is REFUSED admin-role resolution (boot emits a WARN naming this flag). Set truthy only to restore the pre-#1570 trust-the-header posture on isolated/mTLS-fronted deployments. Source: `src/handlers/admin_role.rs::ENV_ADMIN_HEADER_TRUST`. **[#3200, v1.0.0]** Reads through the ONE shared boolean grammar (`src/env_flag.rs`): `1`/`true`/`yes`/`on` enable, `0`/`false`/`no`/`off` disable (case-insensitive, trimmed), an empty value is unset, and an unrecognised token REFUSES boot (a lazy read of one keeps this hatch CLOSED). A token whose meaning changed versus the pre-#3200 reader gets a one-shot boot WARN. |
...
| 132 | `AI_MEMORY_FED_REQUIRE_POLICY_CURRENT` | bool (`1`/`0`/true/false) | `1` (fail-closed for a DETECTED-stale value, v1.0.0 secure default) | federation (`/sync/push` receive) | config | **[#1947 FED-RQ-03, v1.0.0]** Cross-node governance `policy_version` REFUSE-STALE gate. When truthy (default) the receive path refuses an inbound federated push whose advertised `sender_policy_seq` is STRICTLY BEHIND the local committed governance policy (`crate::governance::policy_version::current_policy_version().seq`) with a typed `409 stale_policy_version` — a receive-path refusal (reject-before-apply) that touches NO `MemoryStore` checkpoint-apply path (postgres-clean, independent of #1990) and fails closed IDENTICALLY on sqlite + postgres. **Fail-closed means DETECTED-stale only:** an ABSENT / undeterminable epoch (a peer that does not advertise `sender_policy_seq`, or a transient local policy-read fault) is fail-OPEN (accepted — staleness cannot be determined), so existing federation is NOT hard-refused by this new gate (there is no prior v0.10.0 WARN carrier). Reserve refusal for a strictly-lower advertised seq. Set a falsy token (`0`/`false`/`no`/`off`) to accept stale-policy pushes during a deliberate heterogeneous-governance-policy rollout window. Mirrors the fail-closed escape-hatch shape of `AI_MEMORY_FED_REQUIRE_CHECKPOINT_SIG` (#125) / `AI_MEMORY_FED_REQUIRE_TRANSITION_SIG` (#87) via the #3200 shared grammar (`src/env_flag.rs`). The authoritative ATTESTED epoch (the signed `SignableEpochManifest` `(policy_seq, policy_digest_hex)`) rides the DEFERRED epoch-manifest-doc federation (ADR-002); this gate uses a minimal ADDITIVE unsigned `sender_policy_seq` wire field to refuse an HONEST stale peer that advertises. Direct-read knob (not clap-bound / not a `[section]` field), so no `config_precedence` entry (mirrors #84/#85/#87/#88/#94/#96/#123/#125). Vote `wd8wtmg0n` (A_MINIMAL). PINNED ON by `asi-hard` (#130 / #3168). Source: `src/federation/receive_auth.rs::{REQUIRE_POLICY_CURRENT_ENV,require_policy_current_enabled,evaluate_inbound_policy_freshness}` + `src/handlers/federation_receive.rs::refuse_if_stale_policy`. |
| 133 | `AI_MEMORY_ALLOW_PLAINTEXT_NONLOOPBACK` | bool (shared grammar `1`/`true`/`yes`/`on`, #3200) | `false` | daemon (bind guard) | **operator-advisory** (config) | **[#2032 M2, v1.0.0; WIRED at tranche 3, 5-agent vote `4d3ea1c5`]** "TLS terminated upstream" acknowledgement escape hatch: when truthy the operator asserts a reverse proxy / service-mesh sidecar terminates TLS in front of the daemon, so a non-loopback bind without in-process TLS is an accepted posture. **Tranche 3 WIRES the consuming `tls_bind_guard`:** a plaintext non-loopback bind (no `--tls-cert`/`--tls-key`) now emits a HARD boot WARN naming the cleartext-off-host exposure + the escape hatches; setting this env truthy SILENCES that WARN (accepts the plaintext bind). Loopback binds are exempt. Binding is NOT refused this release — the REFUSE flip for unacknowledged plaintext non-loopback binds lands v1.1.0. Default `false`. Mirrors the truthy grammar + escape-hatch shape of `AI_MEMORY_REQUIRE_API_KEY` (#56). Direct-read knob (not clap-bound / not a `[section]` field), so no `config_precedence` `[limits]`-style entry — the precedence test lives alongside the resolver. Source: `src/daemon_runtime.rs::{ENV_ALLOW_PLAINTEXT_NONLOOPBACK,allow_plaintext_nonloopback_enabled,tls_bind_guard}`. **[#3200, v1.0.0]** Reads through the ONE shared boolean grammar (`src/env_flag.rs`): `1`/`true`/`yes`/`on` enable, `0`/`false`/`no`/`off` disable (case-insensitive, trimmed), an empty value is unset, and an unrecognised token REFUSES boot (a lazy read of one keeps this hatch CLOSED). A token whose meaning changed versus the pre-#3200 reader gets a one-shot boot WARN. **PINNED CLOSED by `asi-hard`** (#130): a truthy value refuses boot, and under `asi-hard` `tls_bind_guard` REFUSES a NON-loopback plaintext bind outright (this hatch is not honoured); the loopback exemption stays for a same-host TLS-terminating proxy. |
| 134 | `AI_MEMORY_REQUIRE_TLS` | bool (shared grammar `1`/`true`/`yes`/`on`, #3200) | `false` | daemon (bind guard) | config | **[#2032 M2, v1.0.0; WIRED at tranche 3, 5-agent vote `4d3ea1c5`]** Fail-closed-now TLS opt-in: when truthy the operator demands in-process TLS on every bind and a plaintext bind is REFUSED. **Tranche 3 WIRES the consuming `tls_bind_guard`:** when this is truthy and no `--tls-cert`/`--tls-key` pair is configured, `bootstrap_serve` refuses to start (fail-closed-now) on ANY host, including loopback. Default `false`. Mirrors the truthy grammar of `AI_MEMORY_REQUIRE_API_KEY` (#56). Direct-read knob; precedence test lives alongside the resolver. Source: `src/daemon_runtime.rs::{ENV_REQUIRE_TLS,require_tls_enabled,tls_bind_guard}`. **[#3200, v1.0.0]** Reads through the ONE shared boolean grammar (`src/env_flag.rs`): `1`/`true`/`yes`/`on` enable, `0`/`false`/`no`/`off` disable (case-insensitive, trimmed), an empty value is unset, and an unrecognised token REFUSES boot (a lazy read of one keeps this mandate ON). A token whose meaning changed versus the pre-#3200 reader gets a one-shot boot WARN. Before #3200 only `1`/`true` enabled it, so `=yes` silently left the TLS mandate off. Deliberately NOT pinned by `asi-hard` (#3200 R3): a pin would also refuse a LOOPBACK plaintext bind and break the same-host TLS-terminating proxy topology; the posture instead refuses non-loopback plaintext (row #133). |
...
| 183 | `AI_MEMORY_AUTH_FAILURE_BACKOFF` | bool (shared grammar `1`/`true`/`yes`/`on`, #3200) | `true` (ON) | daemon (`serve`, HTTP transport auth) | config | **[#2502, v1.0.0]** Per-source backoff after repeated HTTP authentication failures. Before #2502 `api_key_auth` recorded nothing, so one source could present wrong keys forever at whatever rate the daemon answered; admission control (#79) bounds concurrency, not attempts over time. The layer sits directly outside `api_key_auth` and counts only that gate's `401`s (no key, or a key that is neither the shared key nor an enrolled per-agent key); `/health`, the mTLS `/sync/*` bypass, a `403` identity mismatch and handler-level refusals are never counted. After `FREE_FAILURES` (10) rejections each further rejection puts the source in backoff for 1 s doubling to a 300 s cap; a source in backoff gets `429 {"error":"auth_backoff"}` + `Retry-After` BEFORE its key is examined, the right key included, so a guesser learns nothing. A success does NOT reset the count (a caller holding one valid key cannot interleave guesses at another): the count decays by time only, and a source with no rejection for 15 min starts over. The source is the TCP peer address, IPv6 collapsed to its /48 so one allocation cannot supply a fresh source per address. `X-Forwarded-For` is read ONLY from a peer declared in `AI_MEMORY_AUTH_BACKOFF_TRUSTED_PROXIES` (#184), never inferred from the peer being loopback: on a shared host any local process is loopback, and trusting its header would let it pick a fresh source per request or name another client's address to lock that client out (Conductor ruling on #2502). The table is bounded at 10 000 sources and drops the least-recently-failed source first, but NEVER one in backoff (dropping it would let a guesser clear its own backoff by failing from other addresses); when every eviction candidate is in backoff the new source is not counted and one WARN is logged. A request without a peer address passes through and a poisoned lock is recovered, so the layer degrades to no backoff, never to deny. The guessing bound is 10 per source, not a global total. Known trade-off: behind a proxy that is not declared, every client shares the proxy's source. Observability: `ai_memory_auth_failures_total`, `ai_memory_auth_backoff_refusals_total`, and one WARN (target `http::auth`) when a source crosses the threshold. A falsy token disables the layer; an unrecognised token refuses boot (#3200). **PINNED `1` by `asi-hard`** (#130): a falsy value refuses boot under the posture. Registry knob (`src/env_flag.rs`), no `config_precedence` entry. Source: `src/handlers/auth_backoff.rs::{ENV_AUTH_FAILURE_BACKOFF,FREE_FAILURES,MAX_BACKOFF,IDLE_RESET,source_for,auth_backoff_layer}` + `src/handlers/transport.rs::{api_key_auth,auth_exempt}`. |
| 184 | `AI_MEMORY_AUTH_BACKOFF_TRUSTED_PROXIES` | comma-separated IP addresses | unset (no proxy is trusted; every peer is its own source) | daemon (`serve`, HTTP transport auth) | **operator-advisory** (config) | **[#2502, v1.0.0]** The reverse proxies whose `X-Forwarded-For` the auth-failure backoff (#183) reads. When the TCP peer is in this set, the source is the rightmost `X-Forwarded-For` hop that is not itself a listed proxy (the nginx `set_real_ip_from` + `real_ip_recursive` shape); when it is not, the header is ignored. An absent or unparseable hop stops the walk and the last listed proxy is the source, which can only widen a bucket, never let a client choose one; the walk stops after 32 hops. An entry that is not an IP address is logged at WARN and not trusted, so a typo narrows trust rather than widening it. The proxy must APPEND the client address (nginx `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;`): a listed proxy that forwards the client's header unchanged hands the key choice to the client. Listing `127.0.0.1` is an operator statement that loopback is the proxy; on a host where other local processes can reach the daemon, bind the proxy to a distinct address instead. Only read while #183 is on. Direct-read knob (not clap-bound / not a `[section]` field), so no `config_precedence` entry; the parser tests are co-located. Source: `src/handlers/auth_backoff.rs::{ENV_AUTH_BACKOFF_TRUSTED_PROXIES,TrustedProxies,source_for}`. |
...
Thirteen numbered script-based lint gates run in CI alongside the four
...
HARD-BLOCK. Twelve are wired into `.github/workflows/c8-precheck.yml`,
whose TWENTY-FIVE jobs are `c8-precheck`, `vendor-literal-gate`,
`l3-boundary-gate`, `hardcoded-literal-gate`, `const-name-literal-gate`,
`truthy-grammar-gate` (gate **12** below, #3200), `docs-vs-ssot-drift`,
...
gate, not one of the numbered lint gates below),
`external-pr-operator-approval-gate`, plus the two test-guard jobs
above. (This job list re-synced at #2915 and again at #3200: it had
silently rotted from sixteen to twenty-one jobs since #2636, and then
omitted `const-name-literal-gate` and `external-pr-operator-approval-gate`
— the same prose-rot class rule (f) of gate 7 blocks mechanically, one
...
**12. Truthy-grammar SSOT ratchet** (#3200; Conductor rulings R1-R5 on
the 5-agent vote `dabe9197`) — `scripts/check-truthy-grammar.sh`. Before
#3200 the binary carried more than fifty hand-written boolean env parsers
in seven shapes. Two readers of one knob disagreed
(`PASSPHRASE_FILE_ALLOW_LAX_PERMS`, #3620), a mandate ignored `yes`
(`REQUIRE_TLS=yes` bound plaintext with no warning), and `asi-hard` floors
accepted tokens their live readers ignored: under `asi-hard`,
`REQUIRE_AGENT_ATTESTATION=yes` met the floor and unsigned CLI stores still
landed (#3618; `FED_QUARANTINE_UNATTRIBUTED=TRUE` likewise, #3619). #3200
moved every MANDATE and HATCH reader and every boolean floor onto the knob
registry in `src/env_flag.rs`. This gate HARD-BLOCKS a production line
outside that file that spells a boolean token comparison (`== "1"`,
`eq_ignore_ascii_case("true")`, a `"yes" | "on"` arm, `Some("1")`) inside a
function with no row in the dated, issue-linked ledger
`scripts/qc-allowlists/truthy-grammar-neutral-ledger.txt`
(classes NEUTRAL / ENUM / PROMPT / QUERY; security knobs may never appear
there). A STALE row FAILS, so the ledger only shrinks (#3622 burns down
the NEUTRAL rows). Its production-vs-test boundary is deliberately
STRICTER than the older gates' (#3623): a file is a test file only when
its basename carries `test`/`tests` as a whole `_`-delimited word (the
`*test*.rs` glob skipped `attest.rs` and five other attestation files),
and it skips each `#[cfg(test)]` ITEM's extent to the closing brace at the
same indentation rather than everything after the first test module (which
hid `src/config.rs` below line 5329). `--self-test` plants a new reader, an
`attest.rs`-named reader, a reader below a mid-file test module, a stale
row and a malformed row. Run against the pre-#3200 tree it FAILS with 57
unledgered units, including all three #3200 knobs and the attestation
readers.
...
full`, 100 production HTTP route registrations / 86 unique URL paths, 98 CLI subcommands (96 in the default build) at v1.0.0 (post FX-12/ARCH-3 + FX-C3 batch2 + #1389 L2 `RecoverPreviousSession` + #1443 `Expand` + #1598 `Reembed` + #1727 `UndoEdit` + #1978 `Watch`), tens of
