...
| `daemon_runtime.rs` | clap top-level `Command` enum (58 subcommands at v0.7.0 with `--features sal-postgres`; 56 in the default build — the 2-variant gap is `Migrate` + `SchemaInit`, both sal-gated), HTTP daemon `serve` bootstrap, MCP `mcp` dispatch |
...
full`, 87 production HTTP route registrations / 73 unique URL paths, 58 CLI subcommands at v0.7.0, tens of
