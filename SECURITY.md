...
Every MCP `tools/call` and every HTTP request resolves ONE
`identity::authority::Authority { principal, binding, admin }` at its dispatch
chokepoint (the `TOOL_DISPATCH_TABLE` arm in `src/mcp/mod.rs`; the
`authority_layer` middleware composed inside `api_key_auth` in `src/lib.rs`)
BEFORE any handler runs. The constructor is private, so a handler cannot mint
a wider authority than the chokepoint resolved. The resolver is fail-closed on
an unauthenticated principal: an unusable configured `AI_MEMORY_AGENT_ID`
refuses every MCP tool call, and a present but malformed / reserved
`X-Agent-Id` is refused with a typed `400` on every HTTP route. It carries
**no decision**: which row, namespace or owner a call may touch is the
handler's object predicate, and every read/list funnel applies the ONE
predicate `visibility::is_readable_on_query`. The boundary is proven by a
positive inventory — `tests/authority_boundary_structural_3549.rs` over every
table entry and every route registration, with
`tests/authority_boundary_3549_allowlist.txt` as the frozen exemption spec.
Federation (`/api/v1/sync/*`) stays its own boundary (`receive_auth` /
`signing_check`), authenticated by the peer key, not by this resolver.

**Ruling on #3125 (recorded verbatim for this issue).** The Standard-posture
default is stated explicitly: the effective permissions mode with no
`[permissions]` configuration is **`enforce`** (`resolve_v07_default_mode`),
and with **no rule matched the default decision is `Allow`**
(`mode_default_for`) — a fresh install denies nothing by design, because rules
opt in to deny. Ungoverned namespaces stay allow-on-silence (a
cutline-protected contract, `tests/ship_gate_governance_inheritance.rs`);
a blanket refusal would refuse every write on every fresh install. The
fail-closed posture is **posture-gated, not blanket**: the certified
enterprise-federation profile MUST set
`AI_MEMORY_PERMISSIONS_REQUIRE_GOVERNED_NAMESPACE=1` so a write into a
namespace whose governance chain resolves no policy is refused on the tier
that claims governance (3×7 adversarial panel, 2026-08-30, unanimous; the
default flip is deferred to a major boundary behind a first-run governance
bootstrap).

**Ruling on F13 — stdio is a single trust domain (standard §0.1).** On
`ai-memory mcp` the caller IS the launcher: the principal is the launcher's
`AI_MEMORY_AGENT_ID` (`Binding::Env`) or, when unset, the durable host-scoped
stamp with trust-all reads (`Binding::LocalOperator`). Any process with a
shell can set that variable, so every MCP caller-owns gate on stdio is
**defence in depth**, and the T7 principal matrix (unenrolled / revoked / old
key) is satisfiable **only over HTTP with per-agent api keys**
(`Binding::ApiKey`, #2044) or under an orchestrator that provably controls
...
Last updated: 2026-09-11 (#3549 caller-authority boundary + the #3125 / F13 rulings). Previous: 2026-08-16 (v1.0.0 federation secure-default flips #2448/#2477/#1801→#1954/#1936/#1947/#2447 documented; federation-is-plaintext (#1968 OPEN) posture note added; `asi-hard` 28-knob hardened profile cross-referenced; honest tamper-evidence-not-proof / rollback-estimable-not-attestable boundary added; SQLCipher whole-DB at-rest build-flag caveat added. 2026-07-11: v1.0.0 supply-chain CycloneDX SBOM, #1973).
