...
`@alphaone/ai-memory/wake` is a minimal client for the same-host
`ai-memory wake-hub` wake plane. Instead of polling `GET /api/v1/inbox` on a
timer, keep one authenticated session on the hub's Unix domain socket and read
the inbox when there is something to read:

```ts
import { AiMemoryClient } from "@alphaone/ai-memory";
import { DelegationBundle, WakeListener } from "@alphaone/ai-memory/wake";

const bundle = DelegationBundle.load(
...
const listener = new WakeListener(
  ...
  async () => {
    ...
    const { messages } = await client.inbox({ agent_id: bundle.agentId, unread_only: true });
...
inbox row is the record, and `digestHex` is the SHA-256 you can verify what
you read against without the hub ever having seen it.
...
nothing else. A `pollIntervalMs` above that bound is REFUSED rather than
clamped. Reconnects use jittered exponential backoff capped at the same bound.

Node only: the module needs `node:net` and `node:fs`, which is why it lives in
its own `./wake` subpath rather than the browser-safe root export.
