...
- **Erasure-coded archive cold tier (opt-in) + `DurabilityModel::ErasureCodedColdTier`** ([#2064](https://github.com/alphaonedev/ai-memory-mcp/issues/2064), closing the [#1830](https://github.com/alphaonedev/ai-memory-mcp/issues/1830) TRACT-gap G16 subsystem tracker). The finite-field codec is the **operator-authorized** (2026-07-18, sole-authority dependency rule) `reed-solomon-simd` crate (Leopard-RS GF(2^16), O(n log n), SIMD on x86-64 + AArch64; MIT AND BSD-3-Clause; zero RustSec advisories) — the #1830 2×5 adversarial vote (`4d3ea1c5`) UNANIMOUSLY rejected hand-rolling a codec, so `src/erasure/` contains **zero** finite-field math of its own and the shard on-disk format is the vetted crate's codec format (resolving the vote's T4 freeze-hostile concern).
  ...
  - **Destruction intent flows through.** The purge funnels (`purge_archive`, `purge_archive_for_caller`, gc auto-purge) remove the bundles of the rows they delete — keyed on the bundle DIRECTORY existing, not the enable flag — so forgotten-then-purged content cannot be resurrected from the redundancy layer even after the feature is disabled. **Orphan reconciliation (pre-merge durability-audit fix, F1):** a bundle that survives a crash between the purge `DELETE` and the best-effort bundle removal is reaped by the gc-tick orphan-reconciliation pass (`reconcile_and_scrub` — no archived AND no live row past a grace window → removed; a successful restore also drops the now-stale bundle in-flow), closing the `archive restore`-resurrection window; the false "caught by the next purge" self-healing claim was corrected.
  - **Bundle write durability + scrub (audit F2/F3).** The sweep counts ATTEMPTS (not just successes) toward its per-tick pacing bound and persists a keyset resume frontier so an already-bundled prefix is never re-probed; the bundle temp-dir publish now `fsync`s each shard + the manifest + the parent directory before/after the rename (a torn shard behind a surviving manifest can no longer be silently trusted after a power loss); a bounded rotating scrub lane hash-verifies current bundles and re-mints any torn one from the durable archived row before a reconstruct needs it. Crashed `.tmp-*` assembly dirs are reaped. A non-finite (`NaN`/`±Inf`) REAL column now fails LOUD on encode instead of silently becoming JSON `null` (F4).
