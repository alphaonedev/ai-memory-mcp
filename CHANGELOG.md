...
### Fixed (federated DELETE lane namespace confinement, both backends)
...
- **The federated `deletions[]` lane was broken in OPPOSITE directions on the two backends; neither was correct and they disagreed, so a mixed-backend mesh could not converge on what exists** ([#2488](https://github.com/alphaonedev/ai-memory-mcp/issues/2488) CWE-284 / GA-blocker, [#2491](https://github.com/alphaonedev/ai-memory-mcp/issues/2491) data-integrity). Fixed together because they are the same two lines on the two funnels, and fixing either alone leaves them disagreeing. Design decided by a Fable 5 1x7 adversarial vote (`4d3ea1c5`), 7/7 unanimous.
  ...
  - **sqlite — silently LOSSY (#2491).** The `#1934` gate called `peer_attestation::namespace_allowed` unconditionally, with no enrolled-posture wrapper. Its `scope_for(peer) == None` arm falls through to `sync_trust_peer_bypass()` (`AI_MEMORY_FED_SYNC_TRUST_PEER`, default **false**), so in the ZERO-CONFIG posture — and on any header-absent push that reached the loop — EVERY federated deletion was refused and counted `skipped` inside an **HTTP 200**. Not DLQ'd, so never retried: replicas diverged permanently while the origin was told the erasure had propagated. On a GDPR-erasure or credential-rotation path that is a **wrong result reported as success**, which the North Star ranks above the confidentiality direction. `receive_auth.rs` had documented this exact trap for the write lane since `#2447`; the delete lane never got the `has_allowlist()` guard.
  - **The fix.** Both funnels are now guarded on the ENROLLED posture (`PeerAttestationConfig::has_allowlist()`) and route through the shared `receive_auth::inbound_by_id_namespace_authorized`, so all four lanes (`memories[]` / `deletions[]` / `archives[]` / `restores[]`) reach the same verdict on all four peer shapes (zero-config, enrolled-scoped, enrolled-unscoped, not-in-a-non-empty-allowlist) on both backends, and the empty-scope disposition honours `AI_MEMORY_FED_REQUIRE_PUSH_NAMESPACE_SCOPE` (env row 147) instead of hard-coding it. The stored-namespace probe stays ELIDED on the Layer-2-only shapes, where the stored namespace provably cannot change the verdict — so the fix costs **zero extra reads** versus the broken code.
