...
"""Internal turn-capture transport for the ai-memory Anthropic shim.
...
(``docs/rfc/RFC-0001-mcp-turn-capture.md``). This is an importable port of
the reference host-adapter shim
(``clients/host-adapter-shim/python/capture_turn.py``) — same JSON-RPC
framing, same dedup contract (``host_session_id`` + ``host_turn_index``).

**Non-wedging invariant:** every function here is total — it NEVER raises.
A capture failure (missing binary, timeout, substrate error) emits a stderr
WARN and returns ``False`` so the caller's LLM call is never disturbed.
"""
...
# The tools/call request id we scan the substrate stdout for.
_CALL_ID = 2


def _frames(capture_request: dict[str, Any]) -> str:
    """Build the newline-delimited JSON-RPC frames: initialize, initialized,
    then the ``memory_capture_turn`` tools/call."""
    init = {
        "jsonrpc": "2.0",
        "id": 1,
        "method": "initialize",
...
def _pick_call_response(stdout_text: str) -> dict[str, Any] | None:
    """Return the JSON-RPC response whose id matches the tools/call request."""
